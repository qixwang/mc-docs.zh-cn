...
* 支持 [resourceId()](template-functions-resource.md#resourceid) 函数。 可以使用它获取在订阅级部署中使用的资源的资源 ID。 例如，使用 `resourceId('Microsoft.Authorization/roleDefinitions/', parameters('roleDefinition'))` 获取策略定义的资源 ID。 也可使用 [subscriptionResourceId()](template-functions-resource.md#subscriptionresourceid) 函数获取订阅级别资源的资源 ID。
