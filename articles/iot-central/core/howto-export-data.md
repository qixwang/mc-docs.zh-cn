...
1. [在 Azure 门户中创建新的存储帐户](https://ms.portal.azure.cn/#create/Microsoft.StorageAccount-ARM)。 可以详细了解如何创建新的 [Azure Blob 存储帐户](https://aka.ms/blobdocscreatestorageaccount)或 [Azure Data Lake Storage v2 存储帐户](../../storage/blobs/data-lake-storage-quickstart-create-account.md)。 数据导出只能将数据写入支持块 Blob 的存储帐户。 下面列出了已知的兼容类型的存储帐户： 
...
4. 在下拉列表框中，选择自己的**事件中心命名空间**、**服务总线命名空间**、**存储帐户命名空间**，或**输入连接字符串**。
    ...
    - 对于使用免费定价计划创建的应用，只能通过连接字符串配置连续数据导出。 免费定价计划的应用没有关联的 Azure 订阅。
...
快照中的每条消息或记录表示自上次导出消息以来对设备及其属性和云属性所做的一项或多项更改。 这包括：

- IoT Central 中设备的 `id`
- 设备的 `displayName`
- `instanceOf` 中的设备模板 ID
- `simulated` 标志。如果设备为模拟设备，则此项为 true
- `provisioned` 标志。如果设备已预配，则此项为 true
- `approved` 标志。如果已批准设备发送数据，则此项为 true
- 属性值
- `properties`，包括设备和云的属性值
...
每条消息或快照记录表示自上次导出消息以来对发布的设备模板所做的一项或多项更改。 在每条消息或记录中发送的信息包括：
...
- 适用于设备的 `@id` 已删除，`deviceId` 重命名为 `id` 
- 添加了 `provisioned` 标志，用于说明设备的预配状态
- 添加了 `approved` 标志，用于说明设备的审批状态
- `properties`（包括设备和云的属性）与公共 API 中的实体匹配

对于“设备模板”  ，旧数据格式和新数据格式之间的显著差异包括：

- 设备模板的 `@id` 已重命名为 `id`
- 设备模板的 `@type` 已重命名为 `types`，现在为数组

### <a name="devices-format-deprecated-as-of-3-february-2020"></a>设备（从 2020 年 2 月 3 日开始弃用的格式）
```json
{
  "@id":"<id-value>",
  "@type":"Device",
...
### <a name="device-templates-format-deprecated-as-of-3-february-2020"></a>设备模板（从 2020 年 2 月 3 日开始弃用的格式）
```json
{
  "@id":"<template-id>",
