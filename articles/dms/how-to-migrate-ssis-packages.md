...
> [!NOTE]
> Azure 数据库迁移服务 (DMS) 目前不支持将源 SSISDB 迁移到 Azure SQL Database 服务器，但可以使用以下过程重新部署 SSIS 项目/包。
...
* SSIS 必须在包含 Azure-SSIS Integration Runtime (IR) 的 Azure 数据工厂 (ADF) 中进行预配，并且目标 SSISDB 由 Azure SQL Database 服务器实例托管（如文章[在 Azure 数据工厂中预配 Azure-SSIS Integration Runtime](/data-factory/tutorial-deploy-ssis-packages-azure) 中所述）。

## <a name="assess-source-ssis-projectspackages"></a>评估源 SSIS 项目/包

虽然尚未将源 SSISDB 的评估集成到数据库迁移助手 (DMA) 或 Azure 数据库迁移服务 (DMS) 中，但在将 SSIS 项目/包重新部署到 Azure SQL Database 服务器上托管的目标 SSISDB 时，将对其进行评估/验证。
...
2. 在“登录”选项卡上，指定连接到托管目标 SSISDB 的 Azure SQL Database 服务器所需的信息  。
