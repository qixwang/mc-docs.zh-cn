...
Microfocus Operations Bridge 会自动监视所有混合 IT 资源（任何设备、操作系统、数据库、应用程序或服务）而不管其在何处运行，并将 AIOps 应用于所有数据类型（事件、指标、日志和依赖项）。 它提供了服务质量监视与深度应用程序运行状况分析的独特组合，包括对 Azure 服务的全面的性能和可用性监视。 Operations Bridge 使客户能够以业务和 IT 利益干系人都能理解的方式提供一个单一虚拟管理平台，可在任何具有浏览器的设备上使用。
...
Azure DSM 和 Azure 事件中心协议可从 [IBM 支持网站](https://www.ibm.com/support)下载。 可以[在此处了解 Azure 集成][qradar-doc]。
...
SquaredUp for Azure 使 Azure 应用程序的可视化变得非常简单。 它提供了实时交互式仪表板。 你可以向下钻取到订阅、资源组、标记和单个资源，以查看 CPU、大多数入站连接、Application Insights 响应时间、总成本和运行状况摘要之类的指标，并可通过交叉探查来查看相关数据（如警报、Log Analytics 事件、更详细的指标），或者从所使用的其他工具（例如 ServiceNow、Dynatrace、PagerDuty 或 Pingdom）查看相关数据。  你可以自定义自己的仪表板，将其发布，并与个人共享或在 Intranet 页上共享。 
