...
本文介绍了如何使用 Azure Monitor 中的 Surface Hub 解决方案来监视 Azure Surface Hub 设备。 此解决方案可帮助你跟踪 Surface Hub 的运行状况以及了解其使用情况。

每个 Surface Hub 都安装了 Azure Monitoring Agent。 通过代理，可以将数据从 Surface Hub 发送到 Azure Monitor 中的 Log Analytics 工作区。 日志文件从 Surface Hub 读取，并发送到 Azure Monitor。 类似于服务器处于脱机状态、日历不同步，或设备帐户无法登录 Skype 等这些问题都会显示在 Azure Monitor 中的 Surface Hub 仪表板中。 通过使用仪表板中的数据，可以确定未运行或遇到其他问题的设备，并潜在解决检测到的问题。
...
Intune 是 Azure 的一个产品，它允许集中管理应用于一个或多个设备的 Log Analytics 工作区配置设置。 请按照以下步骤通过 Intune 配置设备：
