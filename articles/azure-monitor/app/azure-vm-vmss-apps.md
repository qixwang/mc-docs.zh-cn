...
在 [Azure 虚拟机](/virtual-machines/)上运行的基于 .NET 的 Web 应用程序上启用监视。 获取使用 Application Insights 的所有权益，不需修改代码。
...
* 通过 Application Insights 代理执行的“无代码”  方法
    ...
    * Application Insights 代理自动收集与 .NET SDK 相同的、现成的依赖项信号。 若要了解详细信息，请参阅[依赖项自动收集](/azure-monitor/app/auto-collect-dependencies#net)。
        > [!NOTE]
...
> [!NOTE]
> 在安装 Application Insights 代理之前，需要一个检测密钥。 请[创建新的 Application Insights 资源](/azure-monitor/app/create-new-resource)，或复制现有 Application Insights 资源中的检测密钥。
...
> [!NOTE]
> 验证安装的方法是：单击与你在部署 Application Insights 代理扩展时使用的检测密钥相关联的 Application Insights 资源中的“实时指标流”。 如果从多个虚拟机发送数据，请在“服务器名称”下选择目标 Azure 虚拟机。 可能需要等待长达一分钟的时间数据才会开始流动。
    ...
    "instrumentationKeyMap"= @{
      "filters"= @(
        @{
          ...
          "machineFilter"= ".*";
          "instrumentationSettings"= @{
            "instrumentationKey"= "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"; # Application Insights Instrumentation Key, create new Application Insights resource if you don't have one.
          }
        }
      )
    }
