...
不管使用哪种内核，使 Notebook 一直保持运行都会消耗群集资源。  使用这些内核时，由于上下文是预设的，仅退出 Notebook 并不会终止上下文，因此会继续占用群集资源。 合理的做法是在使用完 Notebook 后，使用 Notebook “文件”菜单中的“关闭并停止”选项来终止上下文，然后退出 Notebook。       
