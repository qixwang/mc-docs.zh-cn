...
1. 通过将 `SPARKCLUSTER` 替换为 Spark 群集的名称来编辑 URL `https://SPARKCLUSTER.azurehdinsight.cn/jupyter`。 然后在 Web 浏览器中输入已编辑的 URL。 出现提示时，请输入群集的群集登录凭据。
...
1. 在 Jupyter 笔记本的空单元格中粘贴以下代码，然后按 **SHIFT + ENTER** 运行这些代码。 该代码导入此方案所需的类型：
