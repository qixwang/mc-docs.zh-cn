...
5. 转换数据并将其载入目标。  在转换和加载期间，可通过多种方式使用 Hive：
    ...
    * 使用 Hive 查询和准备数据，并将其以 CSV 格式存储在 Azure Blob 存储中。  然后，使用 SQL Server Integration Services (SSIS) 等工具获取这些 CSV，并将数据载入 SQL Server 等目标关系数据库。
...
* JSON 文档存储，例如 <a href="https://docs.azure.cn/zh-cn/cosmos-db/">CosmosDB</a>。
