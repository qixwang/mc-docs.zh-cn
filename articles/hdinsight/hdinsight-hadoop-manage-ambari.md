...
2. 使用“操作”  菜单选择要执行的操作：
    ...
    |启用维护模式|隐藏主机的警报。 如果你正在执行生成了警报的操作，则应当启用此模式。 例如，停止和启动服务。|
    |关闭维护模式|使主机恢复正常警报。|
...
Ambari 视图允许开发人员使用 [Apache Ambari 视图框架](https://cwiki.apache.org/confluence/display/AMBARI/Views)将 UI 元素插入 Ambari Web UI。 HDInsight 为 Hadoop 群集类型提供了以下视图：
