...
若要为注册表启用内容信任，请先在 Azure 门户中导航到注册表。 在“策略”  下选择“内容信任”   > “启用”   >   “保存”。 也可以在 Azure CLI 中使用 [az acr config content-trust update][az-acr-config-content-trust-update] 命令。
...
> [!NOTE]
> 无法将受信任的映像推送权限授予 Azure 容器注册表的[管理员帐户](container-registry-authentication.md#admin-account)。
...
> [!IMPORTANT]
> 在任何角色更改后，运行 `az acr login` 以刷新 Azure CLI 的本地标识令牌，以便新角色生效。 若要了解如何验证标识的角色，请参阅[使用 RBAC 和 Azure CLI 管理对 Azure 资源的访问权限](../role-based-access-control/role-assignments-cli.md)和[排查 Azure 资源的 RBAC 问题](../role-based-access-control/troubleshooting.md)。
...
<!--Not Available on [Azure Pipelines](https://docs.microsoft.com/azure/devops/pipelines/build/content-trust)-->
...
<!-- Update_Description: new article about container registry content trust -->
<!--NEW.date: 04/06/2020-->