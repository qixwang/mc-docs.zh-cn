...
运行 [az configure][az-configure] 命令，使用你的 Azure 容器注册表的名称配置 Azure CLI 默认值。 在以下示例中，请将 `<acrName>` 替换为注册表的名称：
...
列出已下载的图表，并记下文件名中包含的 Wordpress 版本。 `helm fetch stable/wordpress` 命令未指定特定版本，因此会获取最新  版本。 所有 Helm 图表都会在遵循 [SemVer 2][semver2] 标准的文件名中包含一个版本号。 在以下示例输出中，Wordpress 图表的版本是 2.1.10  ：
...
<!--Not Available on [use Azure Container Registry Tasks][acr-tasks]-->
