...
|   **使用现有项**   | 将在副本 VM 所在的主机上创建测试 VM。 该 VM 不会添加到云中。<br/><br/>创建与生产网络隔离的 VM 网络。<br/><br/>如果使用基于 VLAN 的网络，建议在 VMM 中创建一个单独逻辑网络（不在生产中使用）以实现此目的。 此逻辑网络用于创建实现测试故障转移所需的 VM 网络。<br/><br/>逻辑网络应至少与所有托管虚拟机的 Hyper-V 服务器中的一个网络适配器关联。<br/><br/>对于 VLAN 逻辑网络，应隔离添加到逻辑网络中的网络站点。<br/><br/>如果使用基于 Windows 网络虚拟化的逻辑网络，Azure Site Recovery 将自动创建隔离的 VM 网络。 |
| **创建网络** |                                                                     根据在“逻辑网络”  及其相关网站中指定的设置，自动创建一个临时测试网络。<br/><br/> 故障转移会检查是否已创建 VM。<br/><br/> 如果恢复计划使用多个 VM 网络，则应使用此选项。<br/><br/> 如果使用 Windows 网络虚拟化网络，此选项可使用副本虚拟机网络中的相同设置（子网和 IP 地址池）自动创建 VM 网络。 测试故障转移完成后，将自动清理这些 VM 网络。<br/><br/> 将在副本虚拟机所在的主机上创建测试 VM。 该 VM 不会添加到云中。                                            |
...
- 测试网络无需与用于测试故障转移的 VMM 逻辑网络类型匹配。 但是，某些组合不起作用：
    ...
    - 如果副本网络使用“无隔离”并且测试网络使用“Windows 网络虚拟化”，则测试故障转移不起作用。 因为无隔离网络不具备创建 Windows 网络虚拟化网络所需的子网。
...
- 如果定义 DHCP，则目标 VMM 服务器会充当 DHCP 服务器，并从为目标 VM 网络定义的池中提供 IP 地址。
- 如果为源服务器定义了使用静态 IP 地址池，则目标 VMM 服务器将从池中分配 IP 地址。 在这两种情况下，如果未定义静态 IP 地址池，则 IP 地址分配会失败。
...
- 若要使用自动创建 VM 网络的选项运行测试故障转移，则需要先将基础结构资源添加到自动创建的网络，然后再运行测试故障转移。 在恢复计划中用于测试故障转移的组 1 前面添加一个手动步骤可以简化此操作。 然后向自动创建的网络添加基础结构资源，之后再运行测试故障转移。
...
5. 完成后，在恢复计划上单击“清理测试故障转移”  。  在“说明”中，记录并保存与测试故障转移相关联的任何观测结果。 此步骤会删除在测试故障转移期间由 Site Recovery 创建的所有 VM 和网络。 

![测试故障转移](./media/hyper-v-vmm-test-failover/TestFailover.png)

> [!TIP]
> 如果 IP 地址可用于测试故障转移网络，则在测试故障转移期间指定给虚拟机的 IP 地址与在运行计划或非计划故障转移时虚拟机收到的 IP 地址相同。 如果同一 IP 地址在测试性故障转移网络中不可用，那么虚拟机会收到可在测试故障转移网络中使用的其他 IP 地址。
...
* 确保在执行测试故障转移时主 VM 已关闭。 如果未关闭，则将同时在同一网络中运行两台具有相同标识的虚拟机。 这可能会导致意外后果。
