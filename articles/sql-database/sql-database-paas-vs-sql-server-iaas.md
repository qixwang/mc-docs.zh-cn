...
- [**SQL 数据库**](sql-database-technical-overview.md)：最适合用于需要使用最新的 SQL Server 稳定功能，且开发与面市时间有限的新式云应用程序。 完全托管的 SQL 数据库引擎，基于最新稳定的 SQL Server Enterprise Edition。 这是托管在 Azure 云中的关系数据库即服务 (DBaaS)，属于“平台即服务 (PaaS)”行业类别  。 SQL 数据库有多个部署选项，每个选项都以 Microsoft 所拥有、托管及维护的标准化硬件和软件为基础。 在 SQL Server 中，可以使用需要进行广泛配置的内置特性和功能（无论是本地的还是 Azure 虚拟机中）。 使用 SQL 数据库时，可以提前付费，并使用纵向或横向缩放选项获得更强大的功能，且不会中断服务。 SQL 数据库具有 SQL Server 所不能提供的其他一些功能，例如，内置的高可用性、智能和管理。
...
无论你是现金不足的新公司，或是在预算有限的情况下运作的已成立公司的小组，资金的限制通常是决定数据库托管方式的主要考虑因素。 在本部分中，将了解 Azure 中有关以下两个关系数据库选项的计费和许可基本概念：SQL 数据库和 SQL 虚拟机。 此外，还介绍如何计算应用程序总成本。
...
- 针对 [SQL](https://www.azure.cn/pricing/details/virtual-machines/#sql) 和 [Windows](https://www.azure.cn/pricing/details/virtual-machines/#windows) 的[虚拟机定价](https://www.azure.cn/pricing/details/virtual-machines/)
...
- 使用 **SQL 虚拟机**时，可以完全掌控操作系统和 SQL Server 实例配置。 使用 VM，可以决定何时更新/升级操作系统与数据库软件，以及何时安装任何其他软件（例如防病毒软件）。 提供的某些自动化功能可以大大简化修补、备份和高可用性。 此外，还可以控制 VM 的大小、磁盘数目及其存储配置。 Azure 允许根据需要更改 VM 的大小。 有关信息，请参阅 [Azure 的虚拟机和云服务大小](../virtual-machines/windows/sizes.md)。
...
- 若要在 Azure VM 上开始使用 SQL Server，请参阅[在 Azure 中预配 SQL Server 虚拟机](../virtual-machines/windows/sql/virtual-machines-windows-portal-sql-server-provision.md)。
- [确定适合本地数据库的 SQL 数据库或 SQL 托管实例 SKU](https://docs.microsoft.com/sql/dma/dma-sku-recommend-sql-db/)。