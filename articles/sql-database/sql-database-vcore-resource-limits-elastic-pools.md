...
ms.date: 04/27/2020
ms.openlocfilehash: 5761519135463b6e63b2eeade07065c2a8e61b26
ms.sourcegitcommit: a4a2521da9b29714aa6b511fc6ba48279b5777c8
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/24/2020
ms.locfileid: "82126851"
---
...
|列存储支持|是|是|是|是|是|是|
|内存中 OLTP 存储 (GB)|不适用|空值|空值|不适用|不适用|不适用|
...
|每个池的最大数据 IOPS <sup>1</sup> |400|800|1200|1600|2000|2400|
|每个池的最大日志速率 (MBps)|4.7|9.4|14.1|18.8|23.4|28.1|
|每个池的最大并发工作线程数（请求数）<sup>2</sup> |210|420|630|840|1050|1260|
|每个池的最大并发登录数 <sup>2</sup> |210|420|630|840|1050|1260|
...
|副本数|1|1|1|1|1|1|
|Multi-AZ|不适用|空值|不适用|空值|空值|不适用|
|读取横向扩展|空值|空值|不适用|空值|空值|空值|
|随附的备份存储|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|

<sup>1</sup> IO 大小的最大值，范围为 8 KB 到 64 KB。 实际 IOPS 与工作负荷相关。

<sup>2</sup> 如需了解任何单个数据库的最大并发辅助角色数（请求数），请参阅[单一数据库资源限制](sql-database-vcore-resource-limits-single-databases.md)。 例如，如果弹性池使用 Gen5 且每个数据库的最大 vCore 数设置为 2，则最大并发辅助角色数目值为 200。  如果每个数据库的最大 vCore 数设置为 0.5，则最大并发辅助角色数目值为 50，因为 Gen5 上每个 vCore 的最大并发辅助角色数为 100。 对于每个数据库的最大 vCore 设置小于 1 个 vCore 或更少的其他情况，最大并发辅助角色数会相应重新缩放。
...
|列存储支持|是|是|是|是|是|是|
|内存中 OLTP 存储 (GB)|空值|空值|不适用|空值|空值|空值|
|最大数据大小 (GB)|2048|2048|2048|2048|3584|4096|
|最大日志大小 (GB)|614|614|614|614|1075|1229|
|TempDB 最大数据大小 (GB)|224|256|288|320|512|768|
|存储类型|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|
|每个池的最大数据 IOPS <sup>1</sup>|2800|3200|3600|4000|6400|9600|
|每个池的最大日志速率 (MBps)|32.8|37.5|37.5|37.5|37.5|37.5|
|每个池的最大并发工作线程数（请求数）<sup>2</sup>|1470|1680|1890|2100|3360|5040|
|最大并发登录池（请求数）<sup>2</sup>|1470|1680|1890|2100|3360|5040|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
...
|副本数|1|1|1|1|1|1|
|Multi-AZ|空值|空值|空值|不适用|不适用|不适用|
|读取横向扩展|空值|不适用|空值|不适用|空值|空值|
|随附的备份存储|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|
...
|列存储支持|是|是|是|是|是|是|是|
|内存中 OLTP 存储 (GB)|不适用|不适用|空值|不适用|不适用|空值|不适用|
...
|每个池的最大数据 IOPS <sup>1</sup>|800|1600|2400|3200|4000|4800|5600|
|每个池的最大日志速率 (MBps)|9.4|18.8|28.1|37.5|37.5|37.5|37.5|
|每个池的最大并发工作线程数（请求数）<sup>2</sup>|210|420|630|840|1050|1260|1470|
|每个池的最大并发登录数（请求数）<sup>2</sup>|210|420|630|840|1050|1260|1470|
...
|副本数|1|1|1|1|1|1|1|
|Multi-AZ|不适用|不适用|空值|空值|不适用|空值|不适用|
|读取横向扩展|空值|空值|空值|空值|空值|空值|不适用|
...
|存储类型|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|
|每个池的最大数据 IOPS <sup>1</sup> |6,400|7,200|8,000|9,600|12,800|16,000|32,000|
|每个池的最大日志速率 (MBps)|37.5|37.5|37.5|37.5|37.5|37.5|37.5|
|每个池的最大并发工作线程数（请求数）<sup>2</sup>|1680|1890|2100|2520|3360|4200|8400|
|每个池的最大并发登录数（请求数）<sup>2</sup>|1680|1890|2100|2520|3360|4200|8400|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|30,000|
...
|副本数|1|1|1|1|1|1|1|
|Multi-AZ|空值|空值|空值|不适用|空值|空值|不适用|
|读取横向扩展|空值|不适用|不适用|不适用|空值|空值|空值|
|随附的备份存储|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|
...
|每个池的最大数据 IOPS <sup>1</sup>|9,000|13,500|18,000|22,500|27,000|
|每个池的最大日志速率 (MBps)|20 个|30|40|50|60|
|每个池的最大并发工作线程数（请求数）<sup>2</sup>|420|630|840|1050|1260|
|每个池的最大并发登录数（请求数）<sup>2</sup>|420|630|840|1050|1260|
...
|每个池的最大数据 IOPS <sup>1</sup>|31,500|36,000|40,500|45,000|72,000|96,000|
|每个池的最大日志速率 (MBps)|70|80|80|80|80|80|
|每个池的最大并发工作线程数（请求数）<sup>2</sup>|1470|1680|1890|2100|3360|5040|
|每个池的最大并发登录数（请求数）<sup>2</sup>|1470|1680|1890|2100|3360|5040|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
|每个数据库的最小/最大弹性池 vCore 选项|0, 0.25, 0.5, 1...7|0, 0.25, 0.5, 1...8|0, 0.25, 0.5, 1...9|0, 0.25, 0.5, 1...10|0, 0.25, 0.5, 1...10, 16|0, 0.25, 0.5, 1...10, 16, 24|
|副本数|4|4|4|4|4|4|
...
|存储类型|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|
|IO 延迟（近似）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|
|每个池的最大数据 IOPS <sup>1</sup>|18,000|27,000|36,000|45,000|54,000|63,000|
|每个池的最大日志速率 (MBps)|60|90|120|120|120|120|
|每个池的最大并发工作线程数（请求数）<sup>2</sup>|420|630|840|1050|1260|1470|
|每个池的最大并发登录数（请求数）<sup>2</sup>|420|630|840|1050|1260|1470|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
...
|每个池的最大数据 IOPS <sup>1</sup>|72,000|81,000|90,000|108,000|144,000|180,000|256,000|
