...
| [Always On 可用性组](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/always-on-availability-groups-sql-server) | 每个数据库都保证 [99.99-99.995% 可用性](sql-database-high-availability.md)。 [简要介绍如何使用 Azure SQL 数据库确保业务连续性](sql-database-business-continuity.md)中对灾难恢复进行了讨论 | 每个数据库都保证 [99.99.% 可用性](sql-database-high-availability.md)，并且[不能由用户管理](sql-database-managed-instance-transact-sql-information.md#availability)。 [简要介绍如何使用 Azure SQL 数据库确保业务连续性](sql-database-business-continuity.md)中对灾难恢复进行了讨论。 使用[自动故障转移组](sql-database-auto-failover-group.md)在另一个区域中配置辅助 Always On 托管实例。 不能将其他 SQL Server 实例和单一数据库用作托管实例的辅助数据库。 |
...
| [Azure Active Directory (AAD) 身份验证](sql-database-aad-authentication.md) | 是的。 仅限 AAD 用户。 | 是的。 包括服务器级 AAD 登录名。 |
...
| [Service Broker](https://docs.microsoft.com/sql/database-engine/configure-windows/sql-server-service-broker) | 否 | 是，但仅限在实例中。 如果使用远程 Service Broker 路由，请在迁移期间尝试将多个分布式 SQL Server 实例中的数据库整合到一个托管实例中，并只使用本地路由。 请参阅 [Service Broker 差异](sql-database-managed-instance-transact-sql-information.md#service-broker) |
...
| **平台功能** | **单一数据库和弹性池** | **托管实例** |
| --- | --- | --- |
| [活动异地复制](sql-database-active-geo-replication.md) | 是 - 除超大规模之外的所有服务层级 | 否，请参阅用作替代方案的[自动故障转移组](sql-database-auto-failover-group.md) |
...
| [Azure 资源运行状况](/service-health/resource-health-overview) | 是 | 否 |
...
| 公共 IP 地址 | 是的。 访问权限可以使用防火墙或服务终结点来限制。  | 是的。 需要显式启用，且必须在 NSG 规则中启用端口 3342。 可根据需要禁用公共 IP。 有关更多详细信息，请参阅[公共终结点](sql-database-managed-instance-public-endpoint-securely.md)。 | 
...
| [SQL Server Integration Services (SSIS)](https://docs.microsoft.com/sql/integration-services/sql-server-integration-services) | 是，使用 Azure 数据工厂 (ADF) 环境中的托管 SSIS ，其中程序包存储在由 Azure SQL 数据库承载的 SSISDB 中并在 Azure SSIS 集成运行时 (IR) 上执行，请参阅[在 ADF 中创建 Azure-SSIS IR](/data-factory/create-azure-ssis-integration-runtime)。 | 否。 |
...
| VNet 全球对等互连 | 是，使用[专用 IP 和服务终结点](sql-database-vnet-service-endpoint-rule-overview.md) | 否，由于 [VNet 全球对等互连中的负载均衡器约束](../virtual-network/virtual-network-manage-peering.md#requirements-and-constraints)，[不支持托管实例](../virtual-network/virtual-networks-faq.md#what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers)。
...
| 托管实例 | **联机：** [事务复制](sql-database-managed-instance-transactional-replication.md) <br/> **脱机：** [BACPAC 文件（导入）](https://docs.microsoft.com/sql/relational-databases/data-tier-applications/import-a-bacpac-file-to-create-a-new-user-database)、BCP、[快照复制](sql-database-managed-instance-transactional-replication.md) | **联机：** [事务复制](sql-database-managed-instance-transactional-replication.md) <br/> **脱机：** 跨实例时间点还原（[Azure PowerShell](https://docs.microsoft.com/powershell/module/az.sql/restore-azsqlinstancedatabase?#examples) 或 [Azure CLI](https://techcommunity.microsoft.com/t5/Azure-SQL-Database/Cross-instance-point-in-time-restore-in-Azure-SQL-Database/ba-p/386208)）、[本机备份/还原](/sql-database/sql-database-managed-instance-get-started-restore)、[BACPAC 文件（导入）](https://docs.microsoft.com/sql/relational-databases/data-tier-applications/import-a-bacpac-file-to-create-a-new-user-database)、BCP、[快照复制](sql-database-managed-instance-transactional-replication.md) |
