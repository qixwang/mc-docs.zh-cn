...
| action_name | action_name_s | 操作的名称 | 不适用 | string |
...
| class_type_desc | class_type_description_s | 发生审核的可审核实体的说明 | 不适用 | string |
...
| host_name | 不适用 | 客户端主机名 | string | 不适用 |
| is_column_permission | is_column_permission_s | 指示是否为列级权限的标志。 1 = true，0 = false | bit | string |
| 不适用 | is_server_level_audit_s | 指示此项审核是否在服务器级别发生的标志 | 不适用 | string |
...
| 不适用 | securable_class_type_s | 映射到正在审核的 class_type 的安全对象 | 不适用 | string |
