...
| [为弹性池配置故障转移组](scripts/sql-database-add-elastic-pool-to-failover-group-powershell.md) | 此 PowerShell 脚本创建一个数据库，将其添加到弹性池，将弹性池添加到故障转移组，并测试到辅助服务器的故障转移。 | 
|**缩放单一数据库和弹性池**||
...
|**配置故障转移组**||
| [为托管实例配置故障转移组](scripts/sql-database-add-managed-instance-to-failover-group-powershell.md) | 此 PowerShell 脚本创建两个托管实例，将其添加到故障转移组，然后测试从主托管实例到辅助托管实例的故障转移。 | 
|||
