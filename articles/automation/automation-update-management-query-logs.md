...
ms.date: 05/25/2020
ms.topic: conceptual
ms.openlocfilehash: 14fb563072adb4960125dce41c2fb8372e3722f8
ms.sourcegitcommit: 981a75a78f8cf74ab5a76f9e6b0dc5978387be4b
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/22/2020
ms.locfileid: "83801887"
---
# <a name="query-update-records-for-update-management-in-azure-monitor-logs"></a>在 Azure Monitor 日志中查询更新管理的更新记录

除了更新管理解决方案中提供的详细信息以外，你还可以针对 Log Analytics 工作区中存储的日志进行搜索。 在解决方案页的左侧窗格中，选择“日志”。 此时会打开“日志搜索”页。

你还可以了解如何自定义查询，或者从不同的客户端使用查询。 请参阅 [Log Analytics 搜索 API 文档](https://dev.loganalytics.io/)。
...
将创建一条 `RequiredUpdate` 类型的记录用于表示计算机所需的更新。 这些记录的属性在下表中列出：
...
| TenantId | 表示 Azure Active Directory 组织实例的唯一标识符。 | 
...
将创建一条 `Update` 类型的记录，用于表示可用的更新及其在计算机上的安装状态。 这些记录的属性在下表中列出：
...
| MSRCSeverity | 漏洞的严重性分级。 值为：<br> 关键<br> 重要<br> 中等<br> 低 |  
| KBID | Windows 更新的知识库文章 ID。 |
| ManagementGroupName | Operations Manager 管理组或 Log Analytics 工作区的名称。 |
| UpdateID | 软件更新的唯一标识符。 |
...
将创建一条 `UpdateAgent` 类型的记录，用于提供计算机上的更新代理的详细信息。 这些记录的属性在下表中列出：
...
将创建一条 `UpdateRunProgress` 类型的记录，用于按计算机提供已计划的部署的更新部署状态。 这些记录的属性在下表中列出：
...
| InstallationStatus | 客户端计算机上可能的更新安装状态。<br> `NotStarted` - 作业尚未触发。<br> `FailedToStart` - 无法在计算机上启动作业。<br> `Failed` - 作业已启动但失败，并且发生异常。<br> `InProgress` - 作业正在进行。<br> `MaintenanceWindowExceeded` - 执行尚未完成，但已达到维护时段间隔。<br> `Succeeded` - 作业成功。<br> `InstallFailed` - 无法成功安装更新。<br> `NotIncluded`<br> `Excluded` |
| KBID | Windows 更新的知识库文章 ID。 | 
| ManagementGroupName | Operations Manager 管理组或 Log Analytics 工作区的名称。 |
...
| SourceComputerId | 表示源计算机的唯一标识符。 | 
| SourceSystem | 记录的源系统。 该值为 `OperationsManager`。 |
| StartTime | 要按计划安装更新的时间。 |
| SubscriptionId | Azure 订阅的唯一标识符。 | 
| SucceededOnRetry | 该值指示是否在首次尝试执行更新时失败，以及当前操作是否为重试。 |
| TimeGenerated | 创建记录的日期和时间。 |
| 标题 | 更新的标题。 |
...
将创建一条 `UpdateSummary` 类型的记录，用于按计算机提供更新摘要。 这些记录的属性在下表中列出：
...
| OldestMissingSecurityUpdateBucket | 最早的缺失安全桶的说明符。 值为：<br> 值小于 30 天时表示最近<br> 30 天前<br> 60 天前<br> 90 天前<br> 120 天前<br> 150 天前<br> 180 天前<br> 值大于 180 天时表示较早。 | 
| OldestMissingSecurityUpdateInDays | 检测为适用的最早更新的未安装总天数。 |
| OsVersion | 操作系统的版本。 |
...
如果代理无法与 Azure Monitor 日志通信且已配置为通过防火墙或代理服务器与 Internet 通信，请确认防火墙或代理服务器是否已正确配置。 若要了解如何验证防火墙或代理服务器是否已正确配置，请参阅 [Windows 代理的网络配置](../azure-monitor/platform/agent-windows.md)或 [Linux 代理的网络配置](../azure-monitor/learn/quick-collect-linux-computer.md)。
...
对于某些 Linux 分发版，Azure 资源管理器中的 VMUUID 值与 Azure Monitor 日志中存储的 VMUUID 值存在[字节序](https://en.wikipedia.org/wiki/Endianness)不匹配的情况。 以下查询可检查任一字节序的匹配情况。 使用 GUID 的 big-endian 和 little-endian 格式替换 VMUUID 值可正常地返回结果。 在 Azure Monitor 日志中运行以下查询可找到应使用的 VMUUID：`Update | where Computer == "<machine name>"
...
* 在 [Azure Monitor 日志](..//azure-monitor/log-query/log-query-overview.md)中使用日志搜索查看详细的更新数据。
