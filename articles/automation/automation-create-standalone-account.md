...
ms.date: 05/25/2020
ms.topic: conceptual
ms.openlocfilehash: bb91dc7458d8ad5c6e26feda2c49494aa11a2cb5
ms.sourcegitcommit: 981a75a78f8cf74ab5a76f9e6b0dc5978387be4b
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/22/2020
ms.locfileid: "83801107"
---
# <a name="create-a-standalone-azure-automation-account"></a>创建独立的 Azure 自动化帐户

本文介绍如何在 Azure 门户中创建 Azure 自动化帐户。 可以使用门户自动化帐户来评估和了解自动化，无需使用其他管理功能，也无需与 Azure Monitor 日志集成。 可以在以后随时添加管理功能或者与 Azure Monitor 日志集成，对 Runbook 作业进行高级监视。
...
有了系统创建的此帐户，就可以快速构建和部署 Runbook 来支持自动化需求。

## <a name="permissions-required-to-create-an-automation-account"></a>创建自动化帐户所需的权限

若要创建或更新自动化帐户，并完成本文所述的任务，必须具有以下特权和权限：

* 若要创建自动化帐户，必须将你的 Azure AD 用户帐户添加到其权限与 `Microsoft.Automation` 资源的“所有者”角色相当的角色。 有关详细信息，请参阅 [Azure 自动化中基于角色的访问控制](automation-role-based-access-control.md)。
...
如果在添加到订阅的全局管理员/共同管理员角色之前，你不是订阅的 Active Directory 实例的成员，则你将作为来宾添加到 Active Directory。 在这种情况下，你会在“添加自动化帐户”窗格中看到以下消息：`You do not have permissions to create.`

如果用户已添加到全局管理员/协同管理员角色，则可以先从订阅的 Active Directory 实例中删除该用户。 可以将该用户重新添加到 Active Directory 中的用户角色。
...
1. 在下一个屏幕上选择“新建”。
   ...
   > [!NOTE]
   > 如果“添加自动化帐户”窗格中显示以下消息，则表示你的帐户不是订阅管理员角色和订阅的共同管理员的成员。
   >
   > ![添加自动化帐户警报](media/automation-create-standalone-account/create-account-without-perms.png)

1. 在“添加自动化帐户”窗格的“名称”字段中，输入新自动化帐户的名称。 选择此名称后，将无法更改它。 

    > [!NOTE]
    > 自动化帐户名称在每个区域和资源组中是唯一的。 已删除的自动化帐户的名称可能不会立即可用。

1. 如果有多个订阅，请使用“订阅”字段指定要用于新帐户的订阅。
...
默认情况下，创建 Azure 自动化帐户时，将不再创建经典运行方式帐户。 如果仍需要经典运行方式帐户，请执行以下操作：

1. 在你的自动化帐户中，选择“帐户设置”下的“运行方式帐户”。
2. 选择“Azure 经典运行方式帐户”。
...
* 若要开始使用 Python 2 runbook，请参阅[我的第一个 Python2 runbook](automation-first-runbook-textual-python2.md)。
