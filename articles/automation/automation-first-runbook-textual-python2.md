...
ms.date: 05/11/2020
ms.topic: conceptual
ms.openlocfilehash: a2df2a7e7e3d4334c4e248e88a76edbea24efba6
ms.sourcegitcommit: 7443ff038ea8afe511f7419d9c550d27fb642246
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/09/2020
ms.locfileid: "83001630"
---
...
1. 在“流程自动化”下选择“Runbook”，以打开 Runbook 的列表。  
1. 选择“添加 Runbook”，创建一个新的 Runbook  。
1. 将该 Runbook 命名为 MyFirstRunbook-Python  。
1. 选择“Python 2”作为“Runbook 类型”。  
...
1. Runbook 作业完成后，会显示其输出。 在此示例中，应会看到 `Hello World`。
1. 关闭测试窗格以返回到画布。
...
1. 如果在“Runbook”页中向左滚动来查看该 Runbook，应该会看到“创作状态”为“已发布”    。
1. 向右滚动查看“MyFirstRunbook-Python”  窗格。
   可以通过顶部的选项来启动 Runbook、查看 Runbook，或计划在将来的某个时间启动它。
1. 单击“启动”  ，在“启动 Runbook”边栏选项卡打开后单击“确定”  。
1. 此时会为你创建的 Runbook 作业打开一个“作业”窗格。 可以关闭此窗格，但我们将使其保持打开状态，以便查看作业的进度。
1. 作业状态显示在“作业摘要”中并且与测试该 Runbook 时看到的状态相匹配  。
1. 在 Runbook 状态显示为“已完成”后，单击“输出”  。 此时会打开“输出”窗格，可以在其中看到 `Hello World`。
1. 关闭“输出”窗格。
1. 单击“所有日志”打开 Runbook 作业的“流”窗格  。 在“输出”流中应当只会看到 `Hello World`。 但是，此窗格可以显示 Runbook 作业的其他流，例如，“详细”流和“错误”流（如果 Runbook 向其中写入内容）。
1. 关闭“流”窗格和“作业”窗格以返回到“MyFirstRunbook-Python”窗格。
1. 单击“作业”以打开此 Runbook 的“作业”页  。 此页列出此 Runbook 创建的所有作业。 由于只运行该作业一次，应该只会看到一个列出的作业。
1. 可以单击此作业，打开在启动 Runbook 时查看过的“作业”窗格。 你可以随时返回到此窗格，并查看为特定 Runbook 创建的任何作业的详细信息。
...
因此，脚本必须使用你的自动化帐户的凭据进行身份验证。 若要帮助你，你可以使用 [Azure 自动化实用程序包](https://github.com/azureautomation/azure_automation_utility)，以便更轻松地进行身份验证和与 Azure 资源进行交互。

> [!NOTE]
> 必须使用服务主体功能来创建自动化帐户，以便使用运行方式证书。
> 如果你的自动化帐户不是使用服务主体创建的，则可根据[使用用于 Python 的 Azure 管理库进行身份验证](https://docs.microsoft.com/azure/python/python-sdk-azure-authenticate)中的说明进行身份验证。
...
对于资源组名称和 VM 名称，Runbook 当前使用硬编码的值。
...
- 若要了解如何使用 Python 进行 Azure 开发，请参阅[适用于 Python 开发人员的 Azure](https://docs.microsoft.com/azure/python/)。
