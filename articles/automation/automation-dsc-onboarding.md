...
Azure Automation State Configuration 是一个配置管理服务，适用于任何云或本地数据中心内的 Desired State Configuration (DSC) 节点。
...
> [!NOTE]
> 如果安装了高于 2.70 版的虚拟机 DSC 扩展，则包含使用 State Configuration 管理 Azure VM 的功能而无需额外付费。 有关详细信息，请参阅[**自动化定价页**](https://azure.cn/pricing/details/automation/)。

以下部分概述了如何将每种类型的计算机加入 Azure Automation State Configuration。

> [!NOTE]
>将 DSC 部署到 Linux 节点时，使用 `/tmp` 文件夹，并临时下载 **nxAutomation** 之类的模块进行验证，然后再将其安装在适当的位置。 为确保正确安装模块，适用于 Linux 的 Log Analytics 代理需要对 `/tmp` 文件夹具有读/写权限。 适用于 Linux 的 Log Analytics 代理以 `omsagent` 用户身份运行。 
>
>若要向 `omsagent` 用户授予写入权限，请运行以下命令：`setfacl -m u:omsagent:rwx /tmp`
>
...
在“注册”  下，输入用例所需的[“PowerShell DSC 本地配置管理器”值](https://docs.microsoft.com/powershell/scripting/dsc/managing-nodes/metaConfig)，并选择性地输入要分配给 VM 的节点配置。
...
可通过 PowerShell 使用 [Register-AzAutomationDscNode](https://docs.microsoft.com/powershell/module/az.automation/register-azautomationdscnode) cmdlet 在 Azure 中登记虚拟机。
但是，这当前仅针对运行 Windows 的计算机实现（此 cmdlet 仅触发 Windows 扩展）。
...
还可以将本地或其他云环境中运行的 Windows 服务器加入到 Azure Automation State Configuration，只要它们能够[对 Azure 进行出站访问](automation-dsc-overview.md#network-planning)即可：
...
还可以将本地或其他云环境中运行的 Linux 服务器加入到 Azure Automation State Configuration，只要它们能够[对 Azure 进行出站访问](automation-dsc-overview.md#network-planning)即可：
...
2. 如果 [PowerShell DSC 本地 Configuration Manager 默认值](https://docs.microsoft.com/powershell/scripting/dsc/managing-nodes/metaConfig4)与用例匹配，并且想要将计算机加入为**同时**从 Azure Automation State Configuration 提取并报告信息，则请执行以下操作：
...
若要以一般方式将任何计算机加入 Azure Automation State Configuration，可以生成 [DSC 元配置](https://docs.microsoft.com/powershell/scripting/dsc/managing-nodes/metaConfig)，以告知 DSC 代理从 Azure Automation State Configuration 提取数据和/或向其报告。 Azure Automation State Configuration 的 DSC 元配置可以使用 PowerShell DSC 配置或 Azure 自动化 PowerShell cmdlet 来生成。
...
2. 使用 `Connect-AzAccount -EnvironmentName AzureChinaCloud` 连接到 Azure 资源管理器
...
- 有关 PowerShell cmdlet 参考，请参阅 [Azure Automation State Configuration cmdlet](https://docs.microsoft.com/powershell/module/az.automation#automation)
