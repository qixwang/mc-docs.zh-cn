...
ms.topic: conceptual
manager: digimobile
ms.openlocfilehash: 627a1c9ea52f704cf2425a7ee6dc884961c35f18
ms.sourcegitcommit: 7443ff038ea8afe511f7419d9c550d27fb642246
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/09/2020
ms.locfileid: "83001574"
---
...
### <a name="scenario-event-15011-in-hybrid-runbook-worker"></a><a name="cannot-connect-signalr"></a>场景：混合 Runbook 辅助角色中发生事件 15011
...
### <a name="scenario-windows-azure-vms-automatically-dropped-from-hybrid-worker-group"></a><a name="vm-automatically-dropped"></a>场景：从混合辅助角色组中自动删除 Windows Azure VM
...
启动辅助角色计算机，然后将其重新注册到 Azure 自动化。 在[部署 Windows 混合 Runbook 辅助角色](../automation-windows-hrw-install.md)中查看有关安装 Runbook 环境和连接到 Azure 自动化的说明。
...
### <a name="scenario-error-403-during-registration-of-hybrid-runbook-worker"></a><a name="error-403-on-registration"></a>场景：在注册混合 Runbook 辅助角色期间发生错误 403
...
* 混合 Runbook 辅助角色无法下载配置，导致帐户链接错误。 当 Azure 启用解决方案时，它仅支持特定的区域链接 Log Analytics 工作区和自动化帐户。 此外，还可能在计算机上设置了错误的日期和/或时间。 如果时间比当前时间快/慢 15 分钟，则无法加入。

#### <a name="resolution"></a>解决方法

##### <a name="mistyped-workspace-idkey"></a>错误键入了工作区 ID/密钥
...
可能还需要更新计算机的日期和/或时区。 如果选择自定义时间范围，请确保该范围采用 UTC，它可能与你的本地时区不同。
...
#### <a name="resolution"></a>解决方法
 ...
 输入命令 `ps -ef | grep python`，验证代理是否正在运行。 应会看到如下所示的输出，以及使用 **nxautomation** 用户帐户运行的 Python 进程。 如果未启用 Azure 自动化解决方案，则以下任何进程都不会运行。
...
如果在 **/var/opt/microsoft/omsconfig/omsconfig.log** 中看到错误 `The specified class does not exist..`，则需要更新 Linux 的 Log Analytics 代理。 运行以下命令重新安装代理：
...
如果 Microsoft Monitoring Agent 服务未运行，则表示已阻止混合 Runbook 辅助角色与 Azure 自动化通信。
