...
例如，如果规模集模型中指定了两个扩展（扩展 A 和扩展 B），则可能会出现以下两种预配顺序之一：
-   扩展 A -> 扩展 B
-   扩展 B -> 扩展 A
...
使用 [Add-AzVmssExtension](https://docs.microsoft.com/powershell/module/az.compute/add-azvmssextension) cmdlet 将应用程序运行状况扩展添加到规模集模型定义中。 必须使用 Az PowerShell 1.2.0 或更高版本，才能执行扩展排序。
...
使用 [az vmss 扩展集](/cli/vmss/extension#az-vmss-extension-set)将应用程序运行状况扩展添加到规模集模型定义中。 必须使用 Azure CLI 2.0.55 或更高版本，才能执行扩展排序。
