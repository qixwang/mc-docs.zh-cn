...
默认情况下，将从跨容错域的规模集中均匀地删除虚拟机，以最大限度地提高可用性。 首先删除 ID 最大的 VM。
...
通过 PowerShell 或 Azure CLI，可以为虚拟机规模集创建指标警报。 
...
有关详细信息，请参阅[创建或更新虚拟机规模集](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesets/createorupdate)。
...
有关详细信息，请参阅[创建或更新虚拟机规模集](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesets/createorupdate#linuxconfiguration)。
...
对于预期的输入结构，请参阅[创建或更新虚拟机集](https://docs.microsoft.com/rest/api/compute/virtualmachinescalesets/createorupdate)。
...
若要了解如何部署 Linux VM 的证书，请参阅[将证书从客户管理的 Key Vault 部署到 VM](https://docs.microsoft.com/archive/blogs/kv/deploy-certificates-to-vms-from-customer-managed-key-vault)。
