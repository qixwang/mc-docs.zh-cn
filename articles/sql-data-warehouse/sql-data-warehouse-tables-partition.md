...
下例使用 [CREATE TABLE](https://docs.microsoft.com/sql/t-sql/statements/create-table-azure-sql-data-warehouse) 语句根据 OrderDateKey 列对 FactInternetSales 表进行分区：
...
- 消除 SQL Server [分区方案](https://docs.microsoft.com/sql/t-sql/statements/create-partition-scheme-transact-sql)。
- 将[分区函数](https://docs.microsoft.com/sql/t-sql/statements/create-partition-function-transact-sql)定义添加到 CREATE TABLE。
...
SQL 数据仓库支持分区拆分、合并和切换。 这些函数每个都使用 [ALTER TABLE](https://docs.microsoft.com/sql/t-sql/statements/alter-table-transact-sql) 语句执行。
