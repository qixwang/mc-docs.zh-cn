...
#Create DNS records 
az network private-dns record-set a create --name myserver --zone-name privatelink.database.chinacloudapi.cn --resource-group myResourceGroup  
az network private-dns record-set a add-record --record-set-name myserver --zone-name privatelink.database.chinacloudapi.cn --resource-group myResourceGroup -a <Private IP Address>
```
...
2. 输入“nslookup myserver.database.chinacloudapi.cn” 。你将收到如下消息： 
...
<!-- Update_Description: new article about create private endpoint cli -->
<!--NEW.date: 01/06/2020-->