...
可以使用 [az network application-gateway create](/cli/network/application-gateway#az-network-application-gateway-create) 创建名为 *myAppGateway* 的应用程序网关。 使用 Azure CLI 创建应用程序网关时，请指定配置信息，例如容量、sku 和 HTTP 设置。 
...
使用 *az network application-gateway rule create* 将具有重定向配置的名为 [rule2](/cli/network/application-gateway/rule#az-network-application-gateway-rule-create) 的路由规则添加到应用程序网关。
