...
**原因：** 建立 TCP 连接并完成 SSL 握手（如果启用了 SSL）后，应用程序网关会将探测作为 HTTP GET 请求发送到后端服务器。 如上所述，默认探测针对 \<协议\>://127.0.0.1:\<端口\>/，并将 200 至 399 范围内的响应状态代码视为“正常”。 如果服务器返回任何其他状态代码，则将服务器标记为“不正常”并显示此消息。
...
**原因：** 应用程序网关将会检查后端 HTTP 设置中指定的主机名是否与后端服务器 SSL 证书提供的 CN 的主机名相匹配。 这是 Standard_v2 和 WAF_v2 SKU 的行为。 Standard 和 WAF SKU 的服务器名称指示 (SNI) 设置为后端池地址中的 FQDN。
...
**原因：** 每个证书存在有效范围，除非服务器的 SSL 证书有效，否则 HTTPS 连接不安全。 当前日期必须在**生效**和**失效**日期范围内。 否则，证书将被视为无效，并成为一个安全隐患。在这种情况下，应用程序网关会将后端服务器标记为“不正常”。

**解决方案：** 如果 SSL 证书已过期，请向供应商续订证书，并使用新证书更新服务器设置。 如果它是自签名证书，则必须生成有效的证书，并将根证书上传到应用程序网关的 HTTP 设置。 为此，请执行以下步骤：
