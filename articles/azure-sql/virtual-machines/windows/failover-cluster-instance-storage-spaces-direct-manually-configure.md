---
title: Azure 虚拟机中的 SQL Server FCI
description: 本文介绍如何在 Azure 虚拟机上创建 SQL Server 故障转移群集实例 (FCI)。
...
如需比较 Azure 虚拟机中 SQL Server 的标准预付费套餐和 BYOL 许可，请参阅 [SQL Server VM 入门](sql-server-on-azure-vm-iaas-what-is-overview.md#get-started-with-sql-server-vms)。
...
> [!IMPORTANT]
> 目前，Azure 上的 SQL Server FCI 不支持 [SQL Server IaaS 代理扩展](sql-server-iaas-agent-extension-automate-management.md)。 建议从参与 FCI 的 VM 中卸载此扩展。 此扩展支持自动备份和修补之类的功能，以及适用于 SQL 的某些门户功能。 卸载代理以后，这些功能将不适用于 SQL VM。
...
- 有权在虚拟机和 Active Directory 中创建对象的帐户
...
1. 在可用性集中创建虚拟机。
    ...
    从 Azure 市场中选择映像。 可以使用包含 Windows Server 和 SQL Server，或者只包含 Windows Server 的 Azure 市场映像。 有关详细信息，请参阅 [Azure 虚拟机上的 SQL Server 概述](sql-server-on-azure-vm-iaas-what-is-overview.md)。
...
1. Azure 在创建虚拟机之后，使用远程桌面协议 (RDP) 连接到每个虚拟机。
...
现在请使用存储空间直通配置故障转移群集。 在本部分中，你将完成以下步骤：
...
- 故障转移群集的名称
- 故障转移群集的 IP 地址 <br/>
    可以使用群集节点所在的同一 Azure 虚拟网络和子网中未使用的 IP 地址。
...
1. 为负载均衡器配置以下属性：
    ...
    ![设置负载均衡器](./media/failover-cluster-instance-storage-spaces-direct-manually-configure/30-load-balancer-create.png)
...
<!-- Update_Description: new article about failover cluster instance storage spaces direct manually configure -->
<!--NEW.date: 07/06/2020-->