...
有关动态管理视图的详细信息，请参阅[动态管理视图和功能 (Transact-SQL)](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/system-dynamic-management-views)。

## <a name="permissions"></a>权限

在 Azure SQL 数据库中，查询动态管理视图需要 VIEW DATABASE STATE 权限。 **VIEW DATABASE STATE** 权限返回有关当前数据库中的所有对象的信息。
...
在 Azure SQL 托管实例中，查询动态管理视图需要 VIEW SERVER STATE 权限。 有关详细信息，请参阅[系统动态管理视图](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/system-dynamic-management-views#required-permissions)。

在 SQL Server 实例和 Azure SQL 托管实例中，动态管理视图会返回服务器状态信息。 在 Azure SQL 数据库中，这些视图会返回只与当前逻辑数据库相关的信息。
...
### <a name="identify-high-memory-consuming-statements"></a>识别内存消耗量较高的语句
...
可以使用 [sys.dm_exec_connections](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-connections-transact-sql) 视图检索与特定服务器和托管实例建立的连接的相关信息和每个连接的详细信息。 此外，[sys.dm_exec_sessions](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-sessions-transact-sql) 视图在检索有关所有活动用户连接和内部任务的信息时非常有用。
...
可以使用 [sys.server_resource_stats](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-server-resource-stats-azure-sql-database) 返回 Azure SQL 托管实例的 CPU 使用率、IO 和存储数据。 在五分钟间隔内收集并聚合数据。 其中有一行用于显示每隔 15 秒报告的信息。 返回的数据包括 CPU 使用率、存储大小、IO 利用率和托管实例 SKU。 历史数据保留大约 14 天。
...
> [!NOTE]
> 在 Azure SQL 数据库中，必须连接到 master 数据库，才能查询下面示例中的 sys.resource_stats 。
...
对于弹性池，可以使用本部分中所述的技术来监视池中的单个数据库。 但也可总体监视该池。 有关信息，请参阅[监视和管理弹性池](elastic-pool-overview.md)。

### <a name="maximum-concurrent-requests"></a>最大并发请求数

若要查看并发请求数，请在数据库中运行以下 Transact-SQL 查询：
...
若要分析 SQL Server 数据库的工作负荷，请修改此查询，针对要分析的特定数据库进行筛选。 例如，如果有一个名为 MyDatabase 的本地数据库，则以下 Transact-SQL 查询返回该数据库中并发请求的计数：
...
可以通过查询 [sys.resource_stats](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database) 视图并查看 active_session_count 列获取会话的历史统计信息。
...
[Azure SQL 数据库和 Azure SQL 托管实例简介](sql-database-paas-overview.md)
