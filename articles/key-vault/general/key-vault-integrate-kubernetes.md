...
ms.date: 07/01/2020
ms.openlocfilehash: 99daf3d77881a0fdcfdddcc0bf389fb0b8a5c710
ms.sourcegitcommit: 4f84bba7e509a321b6f68a2da475027c539b8fd3
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/01/2020
ms.locfileid: "85802789"
---
...
> [!div class="checklist"]
> * 创建服务主体或使用托管标识
> * 使用 Azure CLI 部署 Azure Kubernetes 服务群集
...
按照本[指南](https://docs.azure.cn/aks/kubernetes-walkthrough)操作并完成以下部分：“创建资源组”、“创建 AKS 群集”，以及“连接到群集”  。
...
1. 显示使用以下命令创建的 AKS 群集的元数据。 复制 principalId、clientId、subscriptionId 和 nodeResourceGroup   。
    ...
    ![插图](../media/kubernetes-key-vault-2.png) ![插图](../media/kubernetes-key-vault-3.png)
...
按照本[指南](https://docs.azure.cn/key-vault/secrets/quick-create-cli)自行创建 Key Vault 并设置机密。
...
1. 将服务主体分配到现有 Key Vault。 $AZURE_CLIENT_ID 参数是在创建服务主体后复制的 appId 。
    ...
    ![映像](../media/kubernetes-key-vault-5.png)
...
1. 将 Azure Active Directory (AAD) 标识安装到 AKS 中。
    ```azurecli
...
1. 创建 AAD 标识。 复制 clientId 和 principalId 。
    ```azurecli
    az identity create -g $resourceGroupName -n $identityName
    ```

1. 将“读取者”角色分配给刚才为 Key Vault 创建的 AAD 标识。 然后向标识授予从 Key Vault 获取机密的权限。 将使用刚才创建的 Azure 标识中的 clientId 和 principalId 。
    ```azurecli
...
![映像](../media/kubernetes-key-vault-6.png)
...
> [启用软删除](https://docs.azure.cn/key-vault/general/soft-delete-clid)
