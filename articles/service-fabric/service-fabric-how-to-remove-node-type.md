---
title: 删除 Azure Service Fabric 中的节点类型
description: 了解如何从在 Azure 中运行的 Service Fabric 群集删除节点类型。
ms.topic: conceptual
origin.date: 02/14/2019
ms.date: 01/13/2020
ms.author: v-yeche
ms.openlocfilehash: 3b6cb72b7b8ad81306295393c9d93b0ebdce5421
ms.sourcegitcommit: 713136bd0b1df6d9da98eb1da7b9c3cee7fd0cee
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/08/2020
ms.locfileid: "75742506"
---
...
使用 [Remove-AzServiceFabricNodeType](https://docs.microsoft.com/powershell/module/az.servicefabric/remove-azservicefabricnodetype) 删除 Service Fabric 节点类型。

调用 Remove-AzServiceFabricNodeType 时会产生如下三个操作：
1. 删除节点类型后的虚拟机规模集。
2. 从群集中删除节点类型。
...
使用 Remove-AzServiceFabricNodeType 时，安全性优先于速度。 节点类型必须为银级或金级[持续性级别](/service-fabric/service-fabric-cluster-capacity#the-durability-characteristics-of-the-cluster)，原因如下：
- 铜级未提供任何关于保存状态信息的保证。
- 银级和金级持续性阻止对规模集的任何更改。
...
若要删除节点类型，请运行 [Remove-AzServiceFabricNodeType](https://docs.microsoft.com/powershell/module/az.servicefabric/remove-azservicefabricnodetype) cmdlet。  该 cmdlet 需要一些时间才能完成操作。  所有 VM 都消失（表示为“关闭”）后，fabric:/System/InfrastructureService/[nodetype name] 将显示错误状态。
...
然后，可以更新群集资源以删除节点类型。 可以使用 ARM 模板部署，也可以通过 [Azure 资源管理器](https://resources.azure.com)编辑群集资源。 这将启动群集升级，从而删除处于错误状态的 fabric:/System/InfrastructureService/[nodetype name] 服务。

仍然会在 Service Fabric Explorer 中看到节点处于“关闭”状态。 在要删除的每个节点上运行 [Remove-ServiceFabricNodeState](https://docs.microsoft.com/powershell/module/servicefabric/remove-servicefabricnodestate?view=azureservicefabricps)。
