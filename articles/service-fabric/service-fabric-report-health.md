...
只要运行状况报告的设计清晰明了，发送运行状况报告就十分容易。 如果群集不[安全](service-fabric-cluster-security.md)或 Fabric 客户端拥有管理员权限，可以使用 [FabricClient](https://docs.azure.cn/dotnet/api/system.fabric.fabricclient?view=azure-dotnet) 报告运行状况。 可以使用 [FabricClient.HealthManager.ReportHealth](https://docs.azure.cn/dotnet/api/system.fabric.fabricclient.healthclient.reporthealth?view=azure-dotnet) 通过 API 进行报告，或者通过 PowerShell 或 REST 来完成。 配置旋钮 Batch 报告可提升性能。
...
创建 `FabricClient` 时，通过传递 [FabricClientSettings](https://docs.azure.cn/dotnet/api/system.fabric.fabricclientsettings?view=azure-dotnet) 及运行状况相关实体的所需值来指定配置参数。
...
建议保留默认结构客户端设置，将 `HealthReportSendInterval` 设为 30 秒。 此设置确保通过批处理获得最佳性能。 对于必须尽快发送关键报告，请在 [FabricClient.HealthClient.ReportHealth](https://docs.azure.cn/dotnet/api/system.fabric.fabricclient.healthclient.reporthealth?view=azure-dotnet) API 中对 `HealthReportSendOptions` 使用 Immediate `true`。 即时报告会绕过批处理间隔。 请小心使用此标志；我们想尽可能利用运行状况客户端批处理。 结构客户端即将关闭（例如，进程已确定无效状态并需要关闭以预防副作用）时，即时发送也很有用。 它确保尽量发送累积的报告。 如果某个报告添加有“即时”标志，运行状况客户端对自上次发送积累的所有报告进行批处理。
...
* 对于无状态服务，使用 [IStatelessServicePartition.ReportInstanceHealth](https://docs.azure.cn/dotnet/api/system.fabric.istatelessservicepartition.reportinstancehealth?view=azure-dotnet) 报告当前服务实例的运行状况。
* 对于有状态服务，使用 [IStatefulServicePartition.ReportReplicaHealth](https://docs.azure.cn/dotnet/api/system.fabric.istatefulservicepartition.reportreplicahealth?view=azure-dotnet) 报告当前副本的运行状况。
* 使用 [IServicePartition.ReportPartitionHealth](https://docs.azure.cn/dotnet/api/system.fabric.iservicepartition.reportpartitionhealth?view=azure-dotnet) 报告当前分区实体的运行状况。
* 使用 [CodePackageActivationContext.ReportApplicationHealth](https://docs.azure.cn/dotnet/api/system.fabric.codepackageactivationcontext.reportapplicationhealth?view=azure-dotnet) 报告当前应用程序的运行状况。
* 使用 [CodePackageActivationContext.ReportDeployedApplicationHealth](https://docs.azure.cn/dotnet/api/system.fabric.codepackageactivationcontext.reportdeployedapplicationhealth?view=azure-dotnet) 报告当前节点上部署的当前应用程序的运行状况。
* 使用 [CodePackageActivationContext.ReportDeployedServicePackageHealth](https://docs.azure.cn/dotnet/api/system.fabric.codepackageactivationcontext.reportdeployedservicepackagehealth?view=azure-dotnet) 报告当前节点上部署的应用程序的服务包的运行状况。
