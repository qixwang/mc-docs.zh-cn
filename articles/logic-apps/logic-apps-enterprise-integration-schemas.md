...
1. 使用 Azure 帐户凭据登录到 <a href="https://portal.azure.cn" target="_blank">Azure 门户</a>。
...
若要添加更大的架构，可将架构上传到 Azure 存储帐户中的 Azure Blob 容器。 添加架构的步骤因 Blob 容器是否具有公共读取访问权限而异。 因此，首先请执行以下步骤检查 Blob 容器是否具有公共读取访问权限：[为 Blob 容器设置公共访问级别](../vs-azure-tools-storage-explorer-blobs.md#set-the-public-access-level-for-a-blob-container)
...
1. 在 <a href="https://portal.azure.cn" target="_blank">Azure 门户</a>中，找到并打开你的集成帐户（如果尚未打开）。
...
<!-- Update_Description: new article about logic apps enterprise integration schemas -->
<!--NEW.date: 03/30/2020-->