...
1. 保存逻辑应用。
   ...
   1. 在“回调 URL [POST]”下，复制 URL： 
      ...
      ```http
      POST https://management.azure.cn/{logic-app-resource-ID}/triggers/{endpoint-trigger-name}/listCallbackURL?api-version=2016-06-01
      ```
...
> [!IMPORTANT]
> 对于生产和安全系统，我们强烈建议不要直接从浏览器中调用逻辑应用，原因如下：
>
> * URL 中会出现共享访问密钥。
...
**答**：可以，HTTP 终结点支持通过 [Azure API 管理](../api-management/api-management-key-concepts.md)进行更高级的配置。 此服务还提供相应的功能，使你能够以一致的方式管理所有 API（包括逻辑应用）、设置自定义域名和使用其他身份验证方法等等，例如：
