...
ms.date: 07/20/2020
ms.testscope: no
ms.testdate: 10/08/2019
ms.author: v-yeche
ms.openlocfilehash: 2e6bdf52057050fdeceeeac8435964fbee5e4268
ms.sourcegitcommit: 31da682a32dbb41c2da3afb80d39c69b9f9c1bc6
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/16/2020
ms.locfileid: "86414675"
---
...
<!--CORRECT ON Azure services, Microsoft services-->
...
* 逻辑应用支持的电子邮件提供商（例如 Office 365 Outlook 或 Outlook.com）提供的电子邮件帐户。 至于其他提供商，请[查看此处的连接器列表](https://docs.microsoft.com/connectors/)。

    <!--Not Available on , or Gmail-->
    
    此逻辑应用使用 Office 365 Outlook 帐户。 如果使用其他电子邮件帐户，则常规步骤保持不变，但 UI 显示可能稍有不同。

    <!--No Available on Gmail connector-->

* 下载并安装[免费 Azure 存储资源管理器](https://storageexplorer.com/)。 此工具可以用于检查存储容器是否已正确设置。
...
1. 在创建存储容器之前，请先在 Azure 门户中的“基本信息”选项卡上使用以下设置[创建一个存储帐户](../storage/common/storage-account-create.md)：
    ...
    | **位置** | <*Azure-region*> | 用于存储存储帐户相关信息的区域。 此示例使用“中国北部”。 |
    ...
    若要创建存储帐户，也可以使用 [Azure PowerShell](../storage/common/storage-account-create.md) 或 [Azure CLI](../storage/common/storage-account-create.md)。
...
1. 在 Azure 部署存储帐户后，请查找存储帐户并获取存储帐户的访问密钥：
    ...
    若要获取存储帐户的访问密钥，也可以使用 [Azure PowerShell](https://docs.microsoft.com/powershell/module/az.storage/get-azstorageaccountkey) 或 [Azure CLI](https://docs.azure.cn/cli/storage/account/keys?view=azure-cli-latest#az-storage-account-keys-list)。
...
1. 为电子邮件附件创建 Blob 存储容器。
    ...
    若要创建存储容器，也可以使用 [Azure PowerShell](https://docs.microsoft.com/powershell/module/az.storage/new-azstoragecontainer) 或 [Azure CLI](https://docs.azure.cn/cli/storage/container?view=azure-cli-latest#az-storage-container-create)。
...
1. 请先使用以下设置[创建函数应用](../azure-functions/functions-create-function-app-portal.md)，然后即可创建函数：
    ...
    如果函数应用没有在部署后自动打开，请在 [Azure 门户](https://portal.azure.cn)搜索框中找到该应用并选择“函数应用”。 在“函数应用”下选择你的函数应用。
