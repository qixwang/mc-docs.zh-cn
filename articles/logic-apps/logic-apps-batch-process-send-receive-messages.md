...
3. 为批接收方设置以下属性： 
    ...
    此示例显示了所有条件，但是对于你自己的测试，请仅尝试一个条件：
...
4. 现在，添加一个或多个操作用于处理每个批。 
    ...
    2. 在搜索框中，输入“发送电子邮件”作为筛选器。
        ...
        例如，如果你有个人帐户（如 @outlook.com 或 @hotmail.com），请选择 Outlook.com 连接器。 此示例使用 Office 365 Outlook 连接器。
...
6. 为添加的操作设置属性。
    ...
    * 在“主题”框中，当动态内容列表出现时，请选择“分区名称”字段。 
    ...
    * 在“正文”框中，当动态内容列表出现时，请选择“消息 ID”字段。  

        逻辑应用设计器会自动围绕“发送电子邮件”操作添加一个“For each”循环，因为该操作会将上一个操作的输出视为集合而不是批。 
...
3. 设置批发送方的属性：
    ...
    本示例将添加以下表达式，该表达式在发送到批的消息内容中插入当前日期和时间：
