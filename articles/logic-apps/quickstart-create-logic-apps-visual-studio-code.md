...
尽管可以在 [Azure 门户](https://portal.azure.cn)和 Visual Studio 中执行这些相同的任务，但是当你已熟悉逻辑应用程序定义并想要直接使用代码时，可以在 Visual Studio Code 更快地开始使用。 例如，你可以禁用、启用、删除和刷新已创建的逻辑应用。 此外，还可以从运行 Visual Studio Code 的任何开发平台（如 Linux、Windows 和 Mac）处理逻辑应用和集成帐户。
...
1. 打开 Visual Studio Code。 在 Visual Studio Code 工具栏上，选择 Azure 图标。
    ...
    <!--CUSTOMZIE ON MOONCAKE-->
    
    > [!NOTE]
    > 如果这是你首次登录 Azure 中国云或首次切换到其他 Azure 云，则可按照以下步骤进行选择。
    > 1. 在 Visual Studio Code 工具栏上，选择 `View` > `Command Palette`(`Ctrl+Shift+P`)。
    > 1. 键入“`Azure: Sign In to Azure Cloud`”，然后选择“`Azure China`”。 
    >
    
    <!--CUSTOMZIE ON MOONCAKE-->
    
1. 在 Azure 窗口中的“逻辑应用”下，选择“登录到 Azure”。 当 Azure 登录页面出现提示时，请使用 Azure 帐户登录。

    ![选择“登录到 Azure”](./media/quickstart-create-logic-apps-visual-studio-code/sign-in-azure-visual-studio-code.png)

    1. 如果登录花的时间比平时长，Visual Studio Code 会提示你提供设备代码，以便通过 Azure 身份验证网站登录。 若要改为使用代码登录，请选择“使用设备代码”。
...
1. 提供逻辑应用的名称，然后按 Enter。
    ...
    > [!IMPORTANT]
    > 若要重用此示例逻辑应用定义，需要使用 Office 365 组织帐户，例如 @fabrikam.com。 请确保将虚拟电子邮件地址替换为你自己的电子邮件地址。 若要使用其他电子邮件连接器（如 Outlook.com），请将 `Send_an_email_action` 操作替换为 [Azure 逻辑应用支持的电子邮件连接器](../connectors/apis-list.md)提供的类似操作。
    >
    
    <!--Not Available on or Gmail-->
    <!--Not Available Gmail connector-->
...
1. 如果系统提示将逻辑应用上传到 Azure 订阅，请选择“上传”。
    ...
    此步骤将逻辑应用发布到 [Azure 门户](https://portal.azure.cn)，从而使逻辑能够在 Azure 中活动和运行。
...
1. 使用与逻辑应用关联的 Azure 帐户和订阅登录到 [Azure 门户](https://portal.azure.cn)。
...
1. 如果系统提示上传更改并覆盖 Azure 门户中的现有逻辑应用，请选择“上传”。
    ...
    此步骤会将更新发布到 [Azure 门户](https://portal.azure.cn)中的逻辑应用。
