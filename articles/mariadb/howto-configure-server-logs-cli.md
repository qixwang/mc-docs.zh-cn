...
如果“log_output”  配置为“File”，则可以直接从服务器的本地存储访问日志。 若要列出服务器的可用慢查询日志文件，请运行 [az mariadb server-logs list](https://docs.microsoft.com/cli/azure/mariadb/server-logs#az-mariadb-server-logs-list) 命令。
...
如果“log_output”  配置为“File”，则可以使用 [az mariadb server-logs download](https://docs.microsoft.com/cli/azure/mariadb/server-logs#az-mariadb-server-logs-download) 命令从服务器下载单个日志文件。
