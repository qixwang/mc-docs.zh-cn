...
通过 `az mariadb server create` 命令创建服务器时，`--geo-redundant-backup` 参数描述备份冗余选项。 如果为 `Enabled`，则创建异地冗余备份。 如果为 `Disabled`，则创建本地冗余备份。

备份保留期由参数 `--backup-retention` 设置。

有关在创建过程中设置这些值的详细信息，请参阅 [Azure Database for MariaDB 服务器 CLI 快速入门](quickstart-create-mariadb-server-database-using-azure-cli.md)。
...
```azurecli
az mariadb server georestore --resource-group newresourcegroup --name mydemoserver-georestored --source-server "/subscriptions/$<subscription ID>/resourceGroups/$<resource group ID>/providers/Microsoft.DBforMariaDB/servers/mydemoserver" --location chinaeast2 --sku-name GP_Gen5_8
