...
以下示例在资源组 `myresourcegroup` 中的 `chinaeast2` 处创建名为 `mydemoserver` 的 Azure Database for MariaDB 服务器。 该服务器的管理员登录名为 `myadmin`。 它是一台常规用途第 5 代服务器，具有 2 个 vCore。 用自己的值替换 `<server_admin_password>`。
