...
1. 为 `timeSeriesId` 设置一个值。 若要详细了解**时序 ID**，请阅读[时序模型](./time-series-insights-update-tsm.md)。
...
1. 选择“单击可启动”  。 
    ...
    > [!TIP]
    > 风车模拟器还会创建可以作为有效负载与[时序见解 GA 查询 API](https://docs.microsoft.com/rest/api/time-series-insights/ga-query) 配合使用的 JSON。
...
* **输出**：两个事件。 “location”属性复制到每个事件  。
    ...
    |--------|---------------|----------------------|
    |chinaeast|device1|2016-01-08T01:08:00Z|
    |chinaeast|device2|2016-01-08T01:17:00Z|
...
* **输出**：两个事件。
    ...
    |---|---|---|---|---|---|---|---|
    |chinaeast|manufacturer1|chinaeast|device1|2016-01-08T01:08:00Z|压力|psi|108.09|
    |chinaeast|manufacturer1|chinaeast|device2|2016-01-08T01:17:00Z|振动|abs G|217.09|
