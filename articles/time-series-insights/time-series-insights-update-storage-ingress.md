...
默认情况下，时序见解预览版可以在每个时序见解环境中以高达每秒 1 GB (MBps) 的速率引入传入数据。 存在针对[单个中心分区](/time-series-insights/time-series-insights-update-storage-ingress#hub-partitions-and-per-partition-limits)的其他限制。
