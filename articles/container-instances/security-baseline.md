...
* [虚拟网络方案和资源 - Azure 容器实例](/container-instances/container-instances-virtual-network-concepts)

* [将容器实例部署到 Azure 虚拟网络](/container-instances/container-instances-vnet)
...
* [保护你的网络资源](/security-center/security-center-network-recommendations)
...
* [Azure 安全中心实时网络访问控制](/security-center/security-center-just-in-time)
...
* [在虚拟网络中部署 - Azure 容器实例](/container-instances/container-instances-vnet) 
...
* [公开容器组的静态 IP 地址](/container-instances/container-instances-application-gateway)
...
**指导**：还可以使用 Azure 蓝图，通过在单个蓝图定义中打包关键环境项目（例如 Azure 资源管理器模板、RBAC 控制措施和策略），来简化大规模的 Azure 部署。 可将蓝图应用到新的订阅，并通过版本控制来微调控制措施和管理。 
...
* [如何为 Azure 计算资源配置时间同步](/virtual-machines/windows/time-sync)
...
* [使用 Azure Monitor 日志进行容器组和实例日志记录](/container-instances/container-instances-log-analytics)
...
* [使用 Azure Monitor 日志进行容器组和实例日志记录](/container-registry/container-registry-diagnostics-audit-logs)
...
* [Azure 容器注册表管理员帐户](/container-registry/container-registry-authentication#admin-account)
...
* [有关对容器映像进行标记和版本控制的建议](/container-registry/container-registry-image-tag-version)
...
* [在 Azure 容器实例中装载 Azure 文件共享](/container-instances/container-instances-volume-azure-files)
...
**指导**：使用 Azure AD RBAC 控制对 Azure 容器实例数据和资源的访问。 
...
* [使用 Azure 容器实例加密部署数据](/container-instances/container-instances-encrypt-data)
...
* [Azure 容器实例的安全注意事项](/container-instances/container-instances-image-security)

* [Azure 容器注册表与安全中心的集成](/security-center/azure-container-registry-integration)
...
* [关于 Azure 容器注册表任务的基础映像更新](/container-registry/container-registry-tasks-base-images)
...
* [关于 ACR 任务的基础映像更新](/container-registry/container-registry-tasks-base-images)
...
* [使用 Azure Policy 审核 Azure 容器注册表的合规性](/container-registry/container-registry-azure-policy)
...
* [Azure 容器实例的容器监视和扫描安全建议](/container-instances/container-instances-image-security)
...
**指导**：将容器映像存储在 Azure 容器注册表中，并利用 RBAC 确保只有授权用户才能访问这些映像。

* [了解 Azure 中的 RBAC](/role-based-access-control/rbac-and-directory-admin-roles)

* [了解容器注册表的 RBAC](/container-registry/container-registry-roles)
...
* [如何将托管标识与 Azure 容器实例结合使用](/container-instances/container-instances-managed-identity)
...
**指导**：在支持 Azure 服务（例如 Azure 容器实例）的底层主机上已启用 Azure 反恶意软件，但是，该软件不会针对客户内容运行。
...
* [将容器映像导入到容器注册表](/container-registry/container-registry-import-images)
...
* [加密部署数据 - Azure 容器实例](/container-instances/container-instances-encrypt-data)
