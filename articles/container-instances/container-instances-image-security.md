...
<!--Not Avaialble on [Twistlock](https://azuremarketplace.microsoft.com/marketplace/apps/twistlock.twistlock?tab=Overview)-->
...
### <a name="whitelist-files-and-executables-that-the-container-is-allowed-to-access-or-run"></a>将允许容器访问或运行的文件和可执行文件加入白名单 

减少可变因素或未知因素的数量有助于维持稳定可靠的环境。 限制容器以使其只能访问或运行已预先批准的或已加入白名单的文件和可执行文件，是限制风险因素的已证实方法。  

管理从一开始就实施的白名单要容易得多。 白名单提供控制措施和管理功能，因为你知道正常运行应用程序需要哪些文件和可执行文件。 

白名单不仅可以减小受攻击面，而且还能提供异常状况的基线，并防止出现“干扰邻居”和容器入侵情形的用例。 
