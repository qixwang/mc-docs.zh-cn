...
> [!IMPORTANT]
> 将容器组部署到虚拟网络功能仅在以下区域中正式发布：**中国东部 2**。 在提供了此功能的其他区域中，虚拟网络部署当前为预览版，计划在不久的将来正式发布。 需同意[补充使用条款][terms-of-use]才可使用预览版。 
...
<!--Not Available on Microsoft [aci-helloworld]-->
...
> [!NOTE]
> 如果尝试删除网络配置文件时收到错误，请留出 2-3 天时间让平台自动缓解此问题，然后重试删除操作。 如果在删除网络配置文件时仍出现问题，请[开启支持请求](https://support.azure.cn/support/support-azure/)。

此功能当前需要执行几个其他命令才能删除前面创建的网络资源。 如果你在本文的前面几个部分中使用示例命令创建了虚拟网络和子网，则可以使用以下脚本来删除这些网络资源。
