...
ms.date: 05/29/2020
ms.openlocfilehash: 2be97a2bd144c854789e99ac299641370ef60ea1
ms.sourcegitcommit: be0a8e909fbce6b1b09699a721268f2fc7eb89de
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/29/2020
ms.locfileid: "84200117"
---
# <a name="set-up-diagnostic-logs-for-an-azure-event-hub"></a>设置 Azure 事件中心的诊断日志

可以查看两种类型的 Azure 事件中心日志：

* **[活动日志](../azure-monitor/platform/platform-logs-overview.md)** ：通过这些日志，可以了解对作业执行了哪些操作。 这些日志始终启用。 通过在 Azure 门户事件中心命名空间的左侧窗格中选择“活动日志”，可以查看活动日志条目。 例如：“创建或更新命名空间”、“创建或更新事件中心”。

    ![事件中心命名空间的活动日志](./media/event-hubs-diagnostic-logs/activity-log.png)
* **[诊断日志](../azure-monitor/platform/platform-logs-overview.md)** ：通过诊断日志，可以更详细地了解使用 API 或通过语言 SDK 中的管理客户端对命名空间执行了哪些操作。 
    
    下一部分介绍如何为事件中心命名空间启用诊断日志。

## <a name="enable-diagnostic-logs"></a>启用诊断日志
诊断日志默认已禁用。 若要启用诊断日志，请执行以下步骤：

1.  在 [Azure 门户](https://portal.azure.cn)中，导航到你的事件中心命名空间。 
2. 在左窗格中的“监视”下选择“诊断设置”，然后选择“+ 添加诊断设置”  。 

    ![“诊断设置”页面 - 添加诊断设置](./media/event-hubs-diagnostic-logs/diagnostic-settings-page.png)
4. 在“类别详细信息”部分，选择要启用的“诊断日志类型” 。 有关这些类别的详细信息将在本文后面部分讨论。 
5. 在“目标详细信息”部分，设置所需的存档目标（目标）；例如，存储帐户、事件中心或 Log Analytics 工作区。
...
| 操作日志 | 捕获针对 Azure 事件中心命名空间执行的所有管理操作。 针对 Azure 事件中心执行的数据操作量很大，因此不会捕获数据操作。 |
| 自动缩放日志 | 捕获对事件中心命名空间执行的自动膨胀操作。 |
| Kafka 协调器日志 | 捕获与事件中心相关的 Kafka 协调器操作。 |
| Kafka 用户错误日志 | 捕获有关对事件中心调用的 Kafka API 的信息。 |
| 事件中心虚拟网络 (VNet) 连接事件 | 捕获有关向事件中心发送流量的 IP 地址和虚拟网络的信息。 |
...
| Message | 信息性消息，提供有关自动膨胀操作的详细信息。 该消息包含给定命名空间的上一个和当前吞吐量单位值，以及触发 TU 膨胀的原因。 |
...
客户管理的密钥用户日志 JSON 包括下表中列出的元素：

| 名称 | 说明 |
| ---- | ----------- | 
| Category | 消息的类别类型。 为以下任一值：“错误”和“信息”  |
