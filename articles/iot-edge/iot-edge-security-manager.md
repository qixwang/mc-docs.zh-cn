...
IoT Edge 安全管理器旨在保护 IoT Edge 设备和所有固有软件操作的完整性。 安全管理器会转换来自基础硬件信任根（如果可用）的信任，以便启动 IoT Edge 运行时并监视持续进行的操作。  IoT Edge 安全管理器是与安全的硅硬件（如果可用）结合使用的软件，可用于提供最高安全保证。  
...
IoT Edge 安全守护程序使用证明过程来保护此 API。 当模块调用此 API 时，安全守护程序尝试查找标识的注册。 如果成功，则其使用注册的属性来度量该模块。 如果度量过程的结果与注册匹配，则生成新的标识证明。 相应的 CA 证书（信任捆绑包）会返回到模块。  该模块使用此证书来连接到 IoT 中心、其他模块或启动服务器。 当签名令牌或证书即将到期时，模块有责任请求新的证书。
...
IoT Edge 安全守护程序的安装和更新通过操作系统的包管理系统进行管理。 具有硬件信任根的 IoT Edge 设备应该通过安全启动和更新管理系统来管理守护程序的生命周期，进一步增强其完整性。 设备制造商应该根据各自的设备功能来探索这些方法。
