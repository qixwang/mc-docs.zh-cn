...
使用自定义安装程序可以更改默认的操作配置或环境，例如，启动其他 Windows 服务、保存访问凭据以便进行文件共享，或者使用强加密/更安全的网络协议 (TLS 1.2)。 或者，可以在 Azure-SSIS IR 的每个节点上安装其他自定义/第三方组件，例如程序集、驱动程序或扩展。 有关内置/已预安装组件的详细信息，请参阅 [Azure-SSIS IR 上的内置/已预安装组件](/data-factory/built-in-preinstalled-components-ssis-integration-runtime)。
...
无论使用标准还是快速自定义安装，都可安装免费（无许可证）组件和付费（带许可证）组件。 如果你是独立软件供应商 (ISV)，请参阅[为 Azure-SSIS IR 开发付费或带许可证的组件](how-to-develop-azure-ssis-ir-licensed-components.md)。
...
若要使用标准自定义安装来预配或重新配置 Azure-SSIS IR，请完成以下步骤。
...
1. 使用 ADF UI 预配或重新配置 Azure-SSIS IR 时，在“Integration Runtime 安装程序”窗格的“高级设置”页上选中“使用其他系统配置/组件安装来自定义 Azure-SSIS Integration Runtime”复选框，然后在“自定义安装容器 SAS URI”框中输入容器的 SAS URI。
...
若要使用快速自定义安装来预配或重新配置 Azure-SSIS IR，请完成以下步骤。

1. 使用 ADF UI 预配或重新配置 Azure-SSIS IR 时，在“Integration Runtime 安装程序”窗格的“高级设置”页上选中“使用其他系统配置/组件安装来自定义 Azure-SSIS Integration Runtime”复选框。 

1. 选择“新建”打开“添加快速自定义安装”窗格，然后在“快速自定义安装类型”下拉列表中选择一种类型：  
...
1. 完成标准自定义安装并启动 Azure-SSIS IR 之后，可在容器的 main.cmd.log 文件夹中找到 main.cmd 的标准输出和其他执行日志。
