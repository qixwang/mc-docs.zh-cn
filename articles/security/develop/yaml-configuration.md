...
ms.tgt_pltfrm: na
ms.workload: na
ms.openlocfilehash: 0a56abe85401b12c07d46f13ef7b347d05e050b8
ms.sourcegitcommit: 79c99a9ea013b3c74706a1038a505f4eea2aaac4
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/05/2020
ms.locfileid: "84439657"
---
...
| **InputType**      | **类型**     | **适用于**            | **必需** | **默认值**             | **选项（针对 pickList）**                                   | **说明**                                                                                                                                                                                                                                                                           |
|------------|---------------|-----------------------|----------|---------------------------|----------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ServiceEndpointName | connectedService:Generic | 通用 | True |  |  | VSTS 项目上预配置的服务终结点（泛型）的名称，该终结点存储着（你已登录到的）MSRD 实例 URL 和 REST API 访问令牌（它是从帐户设置页生成的，允许对你的帐户进行完全访问）。
| AccountId | string | 通用 | True |  |  | 用于标识帐户的 GUID。 可以从帐户 URL 中检索它。
| BinariesURL | string | 通用 | True |  |  | 一个以分号分隔的 URL 列表，可供模糊计算机用来下载二进制文件。 请确保 URL 公开可用。
| SeedsURL | string | 通用 | False |  |  | 一个以分号分隔的 URL 列表，可供模糊计算机用来下载种子。 请确保 URL 公开可用。
| OSPlatformType | pickList | 通用 | True | Windows | Linux、Windows | 要在其上运行模糊作业的计算机的 OS 平台类型。
| WindowsEdition | string | OSPlatformType = Windows | True | Server 2008 R2 |  | 要在其上运行模糊作业的计算机的 OS 版本。
| LinuxEdition | string | OSPlatformType = Linux | True | Redhat |  | 要在其上运行模糊作业的计算机的 OS 版本。
| PreSubmissionCommand | string | 通用 | False |  |  | 在提交模糊作业之前，需要在测试计算机上运行以安装测试目标程序及其依赖项的脚本。
| SeedDirectory | string | 通用 | True |  |  | 模糊计算机上包含种子的目录的路径。 有关详细信息，请参阅[种子文件目录](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/03-choose-seed-files#seed-file-directory)。
...
| TestDriverExeType | pickList | 通用 | True | x86 | amd64、x86 | 目标可执行文件体系结构。
| TestDriverParameters | string | 通用 | True | "%testfile%" |  | 传递给测试目标可执行文件的命令行参数。 请注意， **"%testfile%"** 符号（包括双引号）将自动替换为测试驱动程序要分析的目标文件的完整路径，并且是必需的。 有关详细信息，请参阅[命令行参数](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/02-choose-exe#command-line-arguments)。
| ClosesItself | boolean | 通用 | True | 是 |  | 如果测试驱动程序在完成后自行终止（例如，测试驱动程序分析输入文件并立即退出），请选中；如果需要强制关闭测试驱动程序（例如，测试驱动程序是一个在解析输入后主窗口保持打开的 GUI 应用程序），请取消选中。 有关详细信息，请参阅[自行终止](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/05-scope-exe-lifetime#self-termination)。
| MaxDurationInSeconds | string | 通用 | True | 5 |  | 测试驱动程序的最大持续时间（秒）。 提供目标程序分析输入文件时所需的最长合理预期时间的估计。 此估计值越准确，模糊运行越高效。 有关详细信息，请参阅[最大预期执行持续时间](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/05-scope-exe-lifetime#maximum-expected-execution-duration)。
| CanRunRepeat | boolean | 通用 | True | 是 |  | 如果测试驱动程序可以在不依赖于持久化/共享全局状态的情况下重复运行，则选中。 有关详细信息，请参阅[从头开始运行](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/04-describe-exe-characteristics#runs-from-scratch)。
| CanTestDriverBeRenamed | boolean | 通用 | True | false |  | 如果测试驱动程序可执行文件可以重命名并且仍能正常工作，则选中。 有关详细信息，请参阅[可重命名和并行化](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/04-describe-exe-characteristics#can-be-renamed-and-parallelized)。
| SingleOsProcess | boolean | 通用 | True | false |  | 如果测试驱动程序在单个操作系统进程下运行，则选中；如果测试驱动程序生成更多进程，则取消选中。 有关详细信息，请参阅[单进程](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/04-describe-exe-characteristics#single-process)。
...
| MSRD | boolean | AllTools = false | True | 是 |  | 发布由 MSRD 生成任务启动的 MSRD 作业的作业信息和作业 URL。 MSRD 作业长时间运行，并提供单独的报告。
...
| MSRD | boolean | AllTools = false | True | false |  | 报告由 MSRD 生成任务启动的 MSRD 作业的作业信息和作业 URL。 MSRD 作业长时间运行，并提供单独的报告。
