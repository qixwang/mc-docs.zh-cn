...
1. 转到“Azure Active Directory” > “属性” > “目录 ID”，找到自己的租户 ID。    在“cmdlet”部分，每当需要提供 resourceId 时，都使用此 ID。

    ![在 Azure AD 组织的属性中找到租户 ID](./media/powershell-for-azure-ad-roles/tenant-id-for-Azure-ad-org.png)

> [!Note]
> 以下部分提供了简单的示例来帮助你顺利完成操作。 可以在 https://docs.microsoft.com/powershell/module/azuread/?view=azureadps-2.0-preview#privileged_role_management 中找到有关以下 cmdlet 的更详细文档。 但是，需要将 providerID 参数中的“azureResources”替换为“aadRoles”。 此外，还需要记得使用 Azure AD 组织的租户 ID 作为 resourceId 参数。

## <a name="retrieving-role-definitions"></a>检索角色定义

使用以下 cmdlet 获取 Azure AD 组织（租户）中的所有内置的 Azure AD 角色和自定义的 Azure AD 角色。 此重要步骤提供角色名称与 roleDefinitionId 之间的映射。 所有这些 cmdlet 中都会使用该 roleDefinitionId 来引用特定的角色。
