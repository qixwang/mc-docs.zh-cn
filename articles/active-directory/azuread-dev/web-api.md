...
1. 如果 Web 应用程序的用户尚未许可允许 Web 应用程序代表自己调用 Web API，则需要用户许可。 应用程序将显示它要求的权限，如果这些权限中有任何一个是管理员级权限，目录中的普通用户无法许可。 此许可适用于单租户和多租户应用程序。 在单租户情况下，管理员可以代表其用户对许可执行管理员同意。 可使用 [Azure 门户](https://portal.azure.cn)中的 `Grant Permissions` 按钮完成此操作。 
