...
1. 要验证邀请的用户是否已添加到 Azure AD，请运行以下命令：
   ...
   ```
3. 查看输出，确保已列出受邀用户，其中用户主体名称 (UPN) 采用 emailaddress#EXT#\@domain 的格式   。 例如，sanda_fabrikam.com#EXT#\@contoso.partner.onmschina.cn  ，其中 contoso.partner.onmschina.cn 是你从中发送邀请的组织。
...
```
例如： `Remove-AzureADUser -ObjectId "sanda_fabrikam.com#EXT#@contoso.partner.onmschina.cn"`
