...
本文介绍来宾用户如何访问你的资源以及他们遇到的同意过程。 如果你向来宾发送邀请电子邮件，邀请中会包含一个链接，来宾可以通过该链接兑换访问应用或门户的权限。 邀请电子邮件只是供来宾获取资源访问权限的一种方式。 作为替代方法，可以将来宾添加到目录，并为来宾提供门户或你想要共享的应用的直接链接。 无论来宾使用哪种方法，系统都会引导他们完成首次同意过程。

将来宾用户添加到目录时，来宾用户帐户的同意状态（可在 PowerShell 中查看）最初设置为 PendingAcceptance  。 在来宾接受邀请并同意你的隐私策略和使用条款之前，会一直保留此设置。 此后，同意状态将更改为“已接受”，且不再向来宾显示同意页  。

## <a name="redemption-through-the-invitation-email"></a>通过邀请电子邮件兑换

使用 Azure 门户将来宾用户添加到目录的过程中，会向来宾发送邀请电子邮件。 在[使用 PowerShell](/active-directory/b2b/b2b-quickstart-invite-powershell) 将来宾用户添加到目录时，也可以选择发送邀请电子邮件。 下面是来宾兑换电子邮件中的链接时的体验说明。
...
作为邀请电子邮件的替代方法，可以向来宾提供应用或门户的直接链接。 首先需要通过 Azure 门户或 [PowerShell](/active-directory/b2b/b2b-quickstart-invite-powershell) 将来宾用户添加到目录。 当来宾使用直接链接而不是邀请电子邮件时，系统仍会引导他们完成首次同意体验。

> [!IMPORTANT]
> 直接链接必须特定于租户。 换句话说，它必须包含租户 ID 或已验证的域，以便能够在共享应用所在的租户中对来宾进行身份验证。 常见的 URL（如 https://account.activedirectory.windowsazure.cn/r#/applications ）对来宾不起作用，因为它会重定向到来宾的主租户进行身份验证。 下面是使用租户上下文的直接链接的一些示例：
 > - 应用访问面板： https://account.activedirectory.windowsazure.cn/r#/applications/?tenantid=&lt ;tenant id&gt; 
 > - 已验证域的应用访问面板： https://account.activedirectory.windowsazure.cn/r#/applications/&lt ;verified domain&gt;
 > - Azure 门户： https://portal.azure.cn/&lt ;tenant id&gt;
...
当来宾首次登录并访问合作伙伴组织中的资源时，系统会引导他们在以下页中完成操作。 

1. 来宾查看“查看权限”页，其中描述了邀请方组织的隐私声明  。 用户必须接受根据邀请方组织的隐私策略使用其信息才能继续  。
...
3. 除非另有指定，否则来宾将重定向到应用访问面板，其中列出了来宾可以访问的应用程序。
