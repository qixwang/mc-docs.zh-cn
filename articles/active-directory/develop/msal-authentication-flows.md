...
ms.reviewer: saeeda
ms.custom: aaddev
ms.openlocfilehash: 1380538242cf2bea7614e81b94ccc3b77c52276b
ms.sourcegitcommit: 92b9b1387314b60661f5f62db4451c9ff2c49500
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/09/2020
ms.locfileid: "86164972"
---
...
根据客户端的生成方式，客户端可以使用 Microsoft 标识平台支持的一种或多种身份验证流。 这些流可以生成多种类型的令牌以及授权代码，并需要不同的令牌使其正常工作。
...
MSAL 可以以交互方式提示用户输入其凭据，以使用这些凭据登录并获取令牌。

![交互式流示意图](./media/msal-authentication-flows/interactive.png)

有关以交互方式获取特定平台上的令牌的详细信息，请参阅：

- [MSAL.js 交互式请求](msal-js-prompt-behavior.md)
...
MSAL 支持 [OAuth 2 隐式授权](v2-oauth2-implicit-grant-flow.md)流。 此流允许应用从 Microsoft 标识平台获取令牌，而无需执行后端服务器凭据交换。 此流使应用能够让用户登录、维护会话，并获取客户端 JavaScript 代码中所有其他 Web API 的令牌。
...
MSAL 支持 [OAuth 2 授权代码授予](v2-oauth2-auth-code-flow.md)流。 可在设备上安装的应用中使用此流以访问受保护的资源，如 Web API。 这样，就可以添加对移动应用和桌面应用的登录与 API 访问权限。
...
MSAL 支持 [OAuth 2 设备代码流](v2-oauth2-device-code.md)，可让用户登录到智能电视、IoT 设备和打印机等输入受限的设备。 使用 Azure AD 的交互式身份验证需要 Web 浏览器。 如果设备或操作系统不提供 Web 浏览器，设备代码流可让用户使用另一台设备（例如计算机或手机）以交互方式登录。
...
MSAL 支持 [OAuth 2 资源所有者密码凭据授予](v2-oauth-ropc.md) (ROPC)，后者允许应用程序通过直接处理用户的密码来登录用户。 在桌面应用程序中，可以使用用户名/密码流以静默方式获取令牌。 使用应用程序时无需 UI。
...
### <a name="azure-ad-b2c-and-ropc"></a>Azure AD B2C 和 ROPC
