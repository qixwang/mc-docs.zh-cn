...
接下来，运行 CLI `az storage` 命令并使用存储访问密钥进行身份验证，然后将文件上传到 blob 容器。 对于此步骤，需要在 VM 上[安装最新的 Azure CLI](/cli/install-azure-cli)（如果尚未安装）。
...
>[管理存储访问密钥](/storage/common/storage-create-storage-account)
