...
应将条件访问视为一个用于控制对组织中应用和资源的访问的框架，而不是一个独立的功能。 因此，某些条件访问设置需要配置附加的功能。 

如果需要附加的功能，则还可能需要获取相关的许可证。 例如，当条件性访问是 Azure AD Premium P1 功能时。
...
除了活动策略，还建议实现在服务中断/紧急情况下充当辅助性的弹性访问控制措施的已禁用策略。 应急策略的命名标准应当包括更多的一些项： 
...
使用条件访问策略可以自动响应从可能已泄密的标识进行登录的活动。 
