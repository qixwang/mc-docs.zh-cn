---
title: 如何加密 Azure 中的 Linux 虚拟机 | Azure
...
ms.date: 08/12/2019
ms.author: v-yeche
ms.openlocfilehash: 1cf08b0badcf1750bdb626f52d6eb01a657f0108
ms.sourcegitcommit: c1ba5a62f30ac0a3acb337fb77431de6493e6096
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/17/2020
ms.locfileid: "68913026"
---
...
如果选择在本地安装并使用 CLI，本文要求运行 Azure CLI 2.0.30 或更高版本。 运行 `az --version` 即可查找版本。 如果需要进行安装或升级，请参阅[安装 Azure CLI](https://docs.azure.cn/cli/install-azure-cli?view=azure-cli-latest)。
...
在 Azure 订阅中使用 [az provider register](https://docs.azure.cn/cli/provider?view=azure-cli-latest#az-provider-register) 启用 Azure Key Vault 提供程序，并使用 [az group create](https://docs.azure.cn/cli/group?view=azure-cli-latest#az-group-create) 创建一个资源组。 以下示例在  *位置创建名为 myResourceGroup*`chinaeast` 的资源组：
...
包含加密密钥和关联的计算资源（例如存储和 VM 本身）的 Azure 密钥保管库必须位于同一区域。 使用 [az keyvault create](https://docs.azure.cn/cli/keyvault?view=azure-cli-latest#az-keyvault-create) 创建 Azure Key Vault，并启用 Key Vaul，将其用于磁盘加密。 指定 keyvault_name  的唯一 Key Vault 名称，如下所示：
...
可以使用软件保护来存储加密密钥。 由于创建的是标准 Key Vault，以下示例使用受软件保护的密钥。
...
对于这种保护模型，在启动 VM 以解密虚拟磁盘时，需要向 Azure 平台授予访问权限，使之能够请求加密密钥。 使用 [az keyvault key create](https://docs.azure.cn/cli/keyvault/key?view=azure-cli-latest#az-keyvault-key-create) 在 Key Vault 中创建加密密钥。 以下示例创建名为 myKey  的密钥：
...
使用 [az vm create](https://docs.azure.cn/cli/vm?view=azure-cli-latest#az-vm-create) 创建 VM 并附加一个 10Gb 数据磁盘。 只有特定市场映像才支持磁盘加密。 以下示例使用 Ubuntu 16.04 LTS 映像创建一个名为 myVM 的 VM   ：
...
<!--MOONCAKE: WE APPEND  *$sp_id* and *$sp_password* TO SUIT-->

使用 [az vm encryption enable](https://docs.azure.cn/cli/vm/encryption?view=azure-cli-latest#az-vm-encryption-enable) 加密 VM。 以下示例使用之前的 *az ad sp create-for-rbac* 命令中的 *$sp_id* 和 [$sp_password](https://docs.azure.cn/cli/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac) 变量：
...
需要一段时间才能完成磁盘加密过程。 使用 [az vm encryption show](https://docs.azure.cn/cli/vm/encryption?view=azure-cli-latest#az-vm-encryption-show) 监视过程状态：
...
> [!NOTE]
> 在运行命令 [az ad sp create-for-rbac](https://docs.azure.cn/cli/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac) 之前，应将 <sp_id> 和 <sp_password> 替换为上一步中的输出值。
