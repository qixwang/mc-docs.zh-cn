...
ms.author: v-johya
ms.reviewer: cynthn
ms.openlocfilehash: a72bc3f4f4381aef4995f34a856bd29d98df2e18
ms.sourcegitcommit: b5794af488a336d84ee586965dabd6f45fd5ec6d
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/01/2020
ms.locfileid: "87508866"
---
...
Azure 支持将 VHD 引入到 Azure 平台，将其用作[托管磁盘](/virtual-machines/windows/faq-for-disks#managed-disks)或用作映像的源。 
...
- [Azure Linux 代理](/virtual-machines/extensions/agent-linux)
- [cloud-init](/virtual-machines/linux/using-cloud-init)

有关创建映像的先决条件，请参阅[此处](/virtual-machines/linux/create-upload-generic)。


### <a name="specialized-images"></a>专用映像
此类映像经过完全配置且不需用 VM 参数和特殊参数。该平台仅打开 VM，你需要通过设置主机名等方式确保 VM 内的唯一性来避免同一 VNET 中存在 DNS 冲突。 

虽然此类映像无需预配代理，但是，你可能需要一些扩展处理功能。 为此，可以安装 Linux 代理，但请禁用预配选项。 即使无需预配代理，此类映像也必须满足 Azure 映像的[先决条件](/virtual-machines/linux/create-upload-generic)。
...
Azure 支持 Hyper-V 第 1 代 (Gen1) 和第 2 代 (Gen2)。Gen2 是最新一代的 Hyper-V，与 Gen1 相比提供了更多功能， 例如更大的内存、Intel Software Guard Extensions (Intel SGX) 和虚拟化持久性内存 (vPMEM)。 在本地运行的第 2 代 VM 具有 Azure 中尚不支持的一些特性。 有关详细信息，请参阅“特性和功能”部分。 有关详细信息，请参阅此[文章](/virtual-machines/windows/generation-2)。 如果需要其他功能，请创建 Gen2 映像。

如果仍需要创建自己的映像，请确保映像符合[映像先决条件](/virtual-machines/linux/create-upload-generic)，并将其上传到 Azure。 以下是分发特定要求：
