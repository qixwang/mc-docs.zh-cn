...
2. 在“配置 SQL 数据库”对话框中，单击“SQL Server”旁的“新建”。
   ...
   此时会生成唯一的服务器名称。 此名称用作服务器 `<server_name>.database.chinacloudapi.cn` 的默认 URL 的一部分。 它在 Azure SQL 的所有服务器中必须是唯一的。 可以更改服务器名称，但就本教程来说，请保留生成的值。

3. 添加管理员用户名和密码。 有关密码复杂性要求，请参阅[密码策略](https://docs.microsoft.com/sql/relational-databases/security/password-policy)。
