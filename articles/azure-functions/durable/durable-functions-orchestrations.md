...
业务流程的实例 ID 是大多数[实例管理操作](durable-functions-instance-management.md)的必需参数。 它们对于诊断也很重要。 出于此原因，建议将生成的实例 ID 保存到可在今后方便引用的某个外部位置（例如数据库或应用程序日志中）。
...
> [!NOTE]
> 如果业务流程协调程序函数发出日志消息，重播行为可能导致发出重复的日志消息。 
