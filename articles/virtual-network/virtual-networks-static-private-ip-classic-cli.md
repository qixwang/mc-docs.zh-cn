...
1. 如果从未使用过 Azure CLI，请参阅[安装和配置 Azure CLI](https://docs.azure.cn/cli/install-classic-cli?view=azure-cli-latest)，并按照说明进行操作，直到选择 Azure 帐户和订阅。
...
3. 运行 **azure create vm** 命令以创建 VM。 请注意静态专用 IP 地址的值。 在输出后显示的列表说明了所用的参数。
    ...
    * **-l（或 --location）** 。 将在其中创建 VM 的 Azure 区域。 在本方案中为 *chinaeast*。
