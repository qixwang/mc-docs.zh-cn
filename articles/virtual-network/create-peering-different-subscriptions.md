...
<!--MOONCAKE: Not Available on Active Directory b2b-->
...
23. 注销 UserA 的门户登录，然后以 UserB 的身份登录。
24. 针对 myVnetB 再次完成步骤 17-21。 在步骤 21 中，将对等互连命名为 *myVnetBToMyVnetA*，为“虚拟网络”选择“myVnetA”，并在“资源ID”框中输入步骤 10 中的 ID    。
...
- 需要 Azure CLI 2.0.4 或更高版本。 若要查找版本，请运行 `az --version`。 如果需要进行升级，请参阅[安装 Azure CLI](https://docs.azure.cn/cli/install-azure-cli?toc=%2fvirtual-network%2ftoc.json?view=azure-cli-latest)。
- 可以在 Bash shell 中使用。 有关在 Windows 客户端上运行 Azure CLI 脚本的选项，请参阅[在 Windows 上安装 Azure CLI](https://docs.azure.cn/cli/install-azure-cli-windows?view=azure-cli-latest)。
...
3. 使用 `az logout` 命令注销 UserA 的 Azure 登录，然后以 UserB 的身份登录 Azure。 用于登录的帐户必须拥有创建虚拟网络对等互连的必要权限。 有关权限列表，请参阅[虚拟网络对等互连权限](virtual-network-manage-peering.md#permissions)。
...
1. 确认你使用的是 Azure PowerShell 1.0.0 或更高版本。 可以通过运行 `Get-Module -Name Az` 来执行此操作（我们建议你安装最新版本的 PowerShell [Az 模块](https://docs.microsoft.com/powershell/azure/install-az-ps)）。 如果不熟悉 Azure PowerShell，请参阅 [Azure PowerShell 概述](https://docs.microsoft.com/powershell/azure/overview?toc=%2fvirtual-network%2ftoc.json)。 
2. 启动 PowerShell 会话。
3. 在 PowerShell 中，输入 `Connect-AzAccount -Environment AzureChinaCloud` 命令以 UserA 的身份登录 Azure。 用于登录的帐户必须拥有创建虚拟网络对等互连的必要权限。 有关权限列表，请参阅[虚拟网络对等互连权限](virtual-network-manage-peering.md#permissions)。
...
5. 注销 UserA 的 Azure 登录，然后以 UserB 的身份登录。 用于登录的帐户必须拥有创建虚拟网络对等互连的必要权限。 有关权限列表，请参阅[虚拟网络对等互连权限](virtual-network-manage-peering.md#permissions)。
...
2. 注销 UserA 的 Azure 登录，然后以 UserB 的身份登录。
3. 运行以下命令：
