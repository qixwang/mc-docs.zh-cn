...
若要从 Internet 访问虚拟机，需要为 VM 创建 IPv4 和 IPv6 公共 IP 地址。 使用 [New-AzPublicIpAddress](https://docs.microsoft.com/powershell/module/az.network/new-azpublicipaddress) 创建公共 IP 地址。 以下示例在 *dsStd_ILB_RG* 资源组中创建名为 *RdpPublicIP_1* 与 *RdpPublicIP_2* 的 IPv4 和 IPv6 公共 IP 地址：
...
使用 [New-AzVirtualNetwork](https://docs.microsoft.com/powershell/module/az.network/new-azvirtualnetwork) 创建虚拟网络，使用 [New-AzVirtualNetworkSubnetConfig](https://docs.microsoft.com/powershell/module/az.network/new-azvirtualnetworksubnetconfig) 创建双堆栈子网配置。 以下示例创建包含 *dsSubnet* 的名为 *dsVnet* 的虚拟网络：
...
使用 [New-AzLoadBalancer](https://docs.microsoft.com/powershell/module/az.network/new-azloadbalancer) 创建标准负载均衡器。 以下示例使用前面步骤中创建的 IPv4 和 IPv6 前端 IP 配置、后端池和负载均衡规则创建名为 *myInternalLoadBalancer* 的公共标准负载均衡器：
...
使用 [New-AzAvailabilitySet](https://docs.microsoft.com/powershell/module/az.compute/new-azavailabilityset) 创建一个可用性集。 以下示例创建名为 *dsAVset* 的可用性集：
...
使用 [New-AzNetworkSecurityRuleConfig](https://docs.microsoft.com/powershell/module/az.network/new-aznetworksecurityruleconfig) 创建网络安全组规则以允许通过端口 80 进行 Internet 连接。
