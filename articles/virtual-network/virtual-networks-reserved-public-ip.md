---
title: 管理 Azure 保留 IP 地址（经典）| Azure
...
> [!IMPORTANT]
> Azure 具有用于创建和处理资源的两个不同的部署模型：[Resource Manager 和经典](../azure-resource-manager/resource-manager-deployment-model.md)。 本文介绍使用经典部署模型。 Azure 建议大多数新部署使用 Resource Manager 模型。 了解如何使用 [Resource Manager 部署模型](virtual-network-ip-addresses-overview-arm.md)保留静态公共 IP 地址。
...
- 保留 IP 是否收费？
    有时。 有关定价详细信息，请参阅[保留 IP 地址定价详细信息](https://www.azure.cn/pricing/details/ip-addresses/)。
- 如何保留某个 IP 地址？
...
在使用保留 IP 之前，必须先将其添加到订阅。 从中国北部位置提供的公共 IP 地址池创建保留 IP，如下所示  ：
...
示例：
 ```azurecli
 azure network reserved-ip create MyReservedIP chinaeast
 ```
...
- 使用 Azure CLI（经典）：
    ...
    ```azurecli
      azure network reserved-ip create MyReservedIP chinaeast -r TestService -d asmtest8942
    ```
