...
尝试将 Azure 文件共享装载到你创建的目录中。 本文假定你已部署了 Ubuntu 的最新版本。 如果使用的是 Ubuntu 的早期版本，请参阅[在 Linux 上装载](../storage/files/storage-how-to-use-files-linux.md?toc=%2fvirtual-network%2ftoc.json)来了解有关装载文件共享的其他说明。 在运行以下命令之前，将 `<storage-account-name>` 替换为在[创建存储帐户](#create-a-storage-account)中检索到的帐户名称，将 `<storage-account-key>` 替换为检索到的密钥：
