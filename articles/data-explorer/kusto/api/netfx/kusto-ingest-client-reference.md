---
title: Kusto.Ingest 客户端接口和工厂类 - Azure 数据资源管理器
description: 本文介绍 Azure 数据资源管理器中的 Kusto.Ingest 客户端接口和工厂类。
...
ms.date: 07/01/2020
ms.openlocfilehash: d5fde46aea7a436b6b32204baec98c2273721b80
ms.sourcegitcommit: 9bc3e55f01e0999f05e7b4ebaea95f3ac91d32eb
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 07/10/2020
ms.locfileid: "86226356"
---
# <a name="kustoingest-client-interfaces-and-classes"></a>Kusto.Ingest 客户端接口和类

Kusto.Ingest 库中的主要接口和类包括：

* [接口 IKustoIngestClient](#interface-ikustoingestclient)：主引入接口。
* [类 ExtendedKustoIngestClient](#class-extendedkustoingestclient)：主引入接口的扩展。
* [类 KustoIngestFactory](#class-kustoingestfactory)：引入客户端的主工厂。
* [类 KustoIngestionProperties](#class-kustoingestionproperties)：用于提供常见引入属性的类。
* [类 IngestionMapping](#class-ingestionmapping)：用于描述引入的数据映射的类。
* [枚举 DataSourceFormat](#enum-datasourceformat)：支持的数据源格式（例如，CSV、JSON）
* [接口 IKustoQueuedIngestClient](#interface-ikustoqueuedingestclient)：一个接口，描述仅适用于排队引入的操作。
* [类 KustoQueuedIngestionProperties](#class-kustoqueuedingestionproperties)：仅适用于排队引入的属性。

## <a name="interface-ikustoingestclient"></a>接口 IKustoIngestClient
...
## <a name="class-extendedkustoingestclient"></a>类 ExtendedKustoIngestClient

* IngestFromSingleBlob - 已弃用。 请改用 `IKustoIngestClient.IngestFromStorageAsync`。
* IngestFromSingleBlobAsync - 已弃用。 请改用 `IKustoIngestClient.IngestFromStorageAsync`。
* IngestFromDataReader - 已弃用。 请改用 `IKustoIngestClient.IngestFromDataReaderAsync`。
* IngestFromDataReaderAsync
* IngestFromSingleFile - 已弃用。 请改用 `IKustoIngestClient.IngestFromStorageAsync`。
* IngestFromSingleFileAsync - 已弃用。 请改用 `IKustoIngestClient.IngestFromStorageAsync`。
* IngestFromStream - 已弃用。 请改用 `IKustoIngestClient.IngestFromStreamAsync`。
* IngestFromStreamAsync
...
## <a name="class-kustoingestfactory"></a>类 KustoIngestFactory
...
## <a name="class-kustoingestionproperties"></a>类 KustoIngestionProperties

KustoIngestionProperties 类包含基本引入属性，可用于精细控制引入过程以及 Kusto 引擎处理该过程的方式。

|属性   |含义    |
|-----------|-----------|
|DatabaseName |要将数据引入到其中的数据库的名称 |
|TableName |要将数据引入到其中的表的名称 |
|DropByTags |每个盘区都会有的标记。 DropByTags 是永久性的，可以按如下方式使用：`.show table T extents where tags has 'some tag'` 或 `.drop extents <| .show table T extents where tags has 'some tag'` |
|IngestByTags |按盘区写入的标记。 以后可以与 `IngestIfNotExists` 属性一起使用，以免引入相同的数据两次 |
|IngestionMapping|保存对现有映射的引用或列映射列表|
|AdditionalTags |根据需要附加的标记 |
|IngestIfNotExists |不希望再次引入的标记的列表（按表） |
|ValidationPolicy |数据验证定义。 有关详细信息，请参阅 [TODO] |
|格式 |要引入的数据的格式 |
|AdditionalProperties | 其他将作为[引入属性](../../../ingestion-properties.md)传递给引入命令的属性。 这些属性会被传递，因为并非所有引入属性都以此类的独立成员的形式表示|
...
## <a name="class-ingestionmapping"></a>类 IngestionMapping

保存对现有映射的引用或列映射列表。

|属性   |含义    |
|-----------|-----------|
|IngestionMappings | 列映射，每个列映射均描述目标列数据及其源 |
|IngestionMappingKind | IngestionMappings 属性中描述的映射的类型 - 以下类型之一：Csv、Json、Avro、Parquet、SStream、Orc、ApacheAvro 或 W3CLogFile |
|IngestionMappingReference | 预先创建的映射名称 |
...
## <a name="enum-datasourceformat"></a>枚举 DataSourceFormat
...
## <a name="example-of-kustoingestionproperties-definition"></a>KustoIngestionProperties 定义的示例
...
## <a name="interface-ikustoqueuedingestclient"></a>接口 IKustoQueuedIngestClient

IKustoQueuedIngestClient 接口添加了用于跟踪引入操作结果的跟踪方法，并为引入客户端公开 RetryPolicy。
...
## <a name="class-kustoqueuedingestionproperties"></a>类 KustoQueuedIngestionProperties

KustoQueuedIngestionProperties 类使用几个可用于微调引入行为的控制旋钮扩展了 KustoIngestionProperties。

|属性   |含义    |
|-----------|-----------|
|FlushImmediately |默认为 `false`。 如果设为 `true`，将绕过数据管理服务的聚合机制 |
|IngestionReportLevel |控制引入状态报告的级别（默认为 `FailuresOnly`）。 为了获得良好的性能和存储使用率，建议你不要将 IngestionReportLevel 设为 `FailuresAndSuccesses` |
|IngestionReportMethod |控制引入状态报告的目标。 可用选项包括：Azure 队列和/或 Azure 表。 默认为 `Queue`。
