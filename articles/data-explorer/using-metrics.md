...
ms.date: 02/17/2020
ms.openlocfilehash: f3528c4391981ae2fc127876afb9910495d631f5
ms.sourcegitcommit: 5c4141f30975f504afc85299e70dfa2abd92bea1
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/06/2020
ms.locfileid: "77037943"
---
...
1. 选择“添加指标”按钮可以查看同一图表中绘制的多个指标。 
1. 选择“+ 新建图表”按钮可在一个视图中查看多个图表。 
...
**指标** | **单位** | **聚合** | **指标说明**
|---|---|---|---|
| 缓存利用率 | 百分比 | Avg、Max、Min | 群集当前使用的已分配缓存资源百分比。 缓存是为用户活动分配的、符合定义的缓存策略的 SSD 大小。 80% 或更低的平均缓存利用率可以维持群集的正常状态。 如果平均缓存利用率超过 80%时，则群集应[纵向扩展](manage-cluster-vertical-scaling.md)到存储优化的定价层级，或[横向扩展](manage-cluster-horizontal-scaling.md)为更多实例。 也可以调整缓存策略（减少缓存的天数）。 如果缓存利用率超过 100%，则根据缓存策略缓存的数据大小将大于群集上的缓存总大小。 |
...
| 盘区总数 | 计数 | Avg、Max、Min、Sum | 群集中的数据盘区总数。 更改此项指标可能会更改大规模数据的结构并在群集上施加较高的负载，因为合并数据盘区是 CPU 密集型活动。 |
| | | | |
...
**指标** | **单位** | **聚合** | **指标说明**
|---|---|---|---|
连续导出的记录数    | 计数 | Sum | 从群集导出的记录总数。 |
连续导出最大滞后分钟数 |    计数   | Max   | 导出的记录的最大分钟数值。|
连续导出挂起计数 | 计数 | Max   | 挂起的导出操作的最大值。
连续导出结果    | 计数 |   计数   | 连续导出操作总数，按结果列出。 指标包含连续导出名称和数据库。 
导出利用率 |    百分比 | Max   | 用于导出操作的已定义槽的使用率。
| | | | |
...
**指标** | **单位** | **聚合** | **指标说明**
|---|---|---|---|
| 处理的事件数（适用于事件中心/IoT 中心） | 计数 | Max、Min、Sum | 从事件中心读取的以及由群集处理的事件总数 事件划分为群集引擎拒绝的事件和接受的事件。 |
| 引入延迟 | 秒 | Avg、Max、Min | 引入数据的延迟，根据从群集中收到数据，到数据可供查询的时间来测得。 引入延迟周期决于引入方案。 |
| 引入结果 | 计数 | 计数 | 失败和成功的引入操作总数。 使用“应用拆分”可以创建成功和失败结果桶，并分析维度（**值** > **状态**）。 |
| 引入量 (MB) | 计数 | Max、Sum | 引入到群集中的数据在压缩前的总大小 (MB)。 |
| | | | |  
...
**指标** | **单位** | **聚合** | **指标说明**
|---|---|---|---|
| 查询持续时间 | 毫秒 | Avg、Min、Max、Sum | 收到查询结果之前所花费的总时间（不包括网络延迟）。 |
| 并发查询总数 | 计数 | Avg、Max、Min、Sum | 群集中并行运行的查询数。 使用此指标可以很好地评估群集上的负载。 |
| 受限制的查询总数 | 计数 | Avg、Max、Min、Sum | 群集中受限制（被拒绝）的查询数。 允许的最大并发（并行）查询数在并发查询策略中定义。 |
| | | | |
