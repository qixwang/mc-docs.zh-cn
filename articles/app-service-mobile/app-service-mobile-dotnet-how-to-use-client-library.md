...
md.date: 03/23/2020
ms.author: v-tawe
ms.openlocfilehash: f1b1bce574aeb2ae2275e91648641c41f3820a71
ms.sourcegitcommit: c1ba5a62f30ac0a3acb337fb77431de6493e6096
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/17/2020
ms.locfileid: "80151741"
---
...
### <a name="how-to-work-with-debug-symbols-in-visual-studio"></a><a name="symbolsource"></a>如何：在 Visual Studio 中使用调试符号
[SymbolSource][10] 上提供了 Microsoft.Azure.Mobile 命名空间的符号。  要将 SymbolSource 与 Visual Studio 集成，请参阅 [SymbolSource 说明][11]。

## <a name="create-the-mobile-apps-client"></a><a name="create-client"></a>创建移动应用客户端
...
### <a name="how-to-create-a-table-reference"></a><a name="instantiating"></a>如何：创建表引用
...
返回的对象使用类型化序列化模型。 也支持非类型化的序列化模型。 下例[创建对非类型化表的引用]：
...
### <a name="how-to-query-data-from-your-mobile-app"></a><a name="querying"></a>如何：从移动应用中查询数据
...
### <a name="how-to-sort-returned-data"></a><a name="sorting"></a>如何：对返回的数据进行排序
以下代码演示了如何通过在查询中包含 [OrderBy] 或 [OrderByDescending] 函数来为数据排序。 该代码将返回 `todoTable` 中已按 `Text` 字段升序排序的项。
...
### <a name="how-to-return-data-in-pages"></a><a name="paging"></a>如何：返回多页的数据
...
### <a name="how-to-select-specific-columns"></a><a name="selecting"></a>如何：选择特定列
...
### <a name="how-to-look-up-data-by-id"></a><a name="lookingup"></a>如何：按 ID 查找数据
...
### <a name="how-to-execute-untyped-queries"></a><a name="untypedqueries"></a>如何：执行非类型化查询
...
### <a name="how-to-insert-data-into-a-mobile-app-backend"></a><a name="inserting"></a>如何：将数据插入移动应用后端
...
如果插入期间没有在 `todoItem` 中包括唯一的自定义 ID 值，服务器会生成 GUID。
在调用返回后，可通过检查对象来检索生成的 ID。
...
### <a name="how-to-modify-data-in-a-mobile-app-backend"></a><a name="modifying"></a>如何：修改移动应用后端中的数据
...
### <a name="how-to-delete-data-in-a-mobile-app-backend"></a><a name="deleting"></a>如何：删除移动应用后端中的数据
...
### <a name="how-to-use-optimistic-concurrency-for-conflict-resolution"></a><a name="optimisticconcurrency"></a>如何：使用乐观并发解决冲突
...
### <a name="how-to-bind-mobile-apps-data-to-a-windows-user-interface"></a><a name="binding"></a>如何：将移动应用数据绑定到 Windows 用户界面
...
使用通过调用 `ToCollectionAsync` 或 `ToCollection` 创建的集合时，可以获取可绑定到 UI 控件的集合。  此集合具有分页感知功能。  该集合从网络加载数据，因此加载有时候会失败。 若要处理这种故障，可以重写 `MobileServiceIncrementalLoadingCollection` 中的 `OnException` 方法，处理调用 `LoadMoreItemsAsync` 后发生的异常。

假设表包含许多字段，但只想在控件中显示其中的某些字段。 可以使用上面“[选择特定的列](#selecting)”部分中的指导，选择要在 UI 中显示的特定列。

### <a name="change-the-page-size"></a><a name="pagesize"></a>更改页面大小
...
## <a name="work-with-offline-tables"></a><a name="#offlinesync"></a>使用脱机表
...
### <a name="syncing-an-offline-table"></a><a name="syncoffline"></a>同步脱机表
...
## <a name="work-with-a-custom-api"></a><a name="#customapi"></a>使用自定义 API
...
## <a name="authenticate-users"></a><a name="authentication"></a>对用户进行身份验证
...
### <a name="client-managed-authentication"></a><a name="clientflow"></a>客户端托管的身份验证
...
* [Microsoft 帐户](#client-microsoftaccount)

#### <a name="authenticate-users-with-the-active-directory-authentication-library"></a><a name="adal"></a>使用 Active Directory 身份验证库对用户进行身份验证
...
#### <a name="single-sign-on-using-a-token-from-microsoft-account"></a><a name="client-microsoftaccount"></a>使用来自 Microsoft 帐户的令牌进行单一登录
可以对 Microsoft 帐户使用此代码片段中所示的客户端流。
...
### <a name="server-managed-authentication"></a><a name="serverflow"></a>服务器托管的身份验证
注册标识提供者后，使用提供者的 [MobileServiceAuthenticationProvider] 值对 [MobileServiceClient] 调用 [LoginAsync] 方法。 例如，以下代码使用 Microsoft 帐户启动服务器流登录。
...
如果使用的标识提供者不是 Microsoft 帐户，请将 [MobileServiceAuthenticationProvider] 的值更改为提供者的值。
...
### <a name="caching-the-authentication-token"></a><a name="caching"></a>缓存身份验证令牌
在某些情况下，存储提供者提供的身份验证令牌可避免在首次成功身份验证后调用登录方法。  Microsoft Store 和 UWP 应用可以使用 [PasswordVault] 在成功登录后缓存当前身份验证令牌，如下所示：
...
Xamarin 应用使用 [Xamarin.Auth] 将证书安全存储在 **Account** 对象中。 有关使用这些 API 的示例，请参阅 [ContosoMoments 照片分享示例](https://github.com/azure-appservice-samples/ContosoMoments)中的 [AuthStore.cs] 代码文件。
...
## <a name="push-notifications"></a><a name="pushnotifications"></a>推送通知
以下主题介绍了推送通知：
...
如果要推送到 WNS，必须[获取 Microsoft Store 包 SID](#package-sid)。  有关 Windows 应用的详细信息，包括如何注册模板，请参阅 [Add push notifications to your app]（将推送通知添加到应用）。

不支持从客户端请求标记。  注册时将静默删除标记请求。
如果想要使用标记注册设备，请创建自定义 API，使用通知中心 API 自动执行注册。  调用自定义 API 而不是 `RegisterNativeAsync()` 方法。

### <a name="how-to-obtain-a-microsoft-store-package-sid"></a><a name="package-sid"></a>如何：获取 Microsoft Store 包 SID
...
### <a name="how-to-register-push-templates-to-send-cross-platform-notifications"></a><a name="register-xplat"></a>如何：注册推送模板以发送跨平台通知
...
## <a name="miscellaneous-topics"></a><a name="misc"></a>其他主题
...
### <a name="how-to-customize-request-headers"></a><a name="headers"></a>如何：自定义请求标头
