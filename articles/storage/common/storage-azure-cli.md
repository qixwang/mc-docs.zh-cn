...
接下来，我们将使用一个小型 shell 脚本，该脚本会发出一些基本的 Azure CLI 命令与 Azure 存储资源进行交互。 该脚本首先在存储帐户中创建新容器，再将现有文件（作为 Blob）上传到该容器。 然后，列出容器中的所有 Blob，最后，将文件下载到指定的本地计算机上的目标。
...
* `--location` [必需]：位置。 例如“China East”。
* `--name` [必需]：存储帐户名称。 名称的长度必须为 3 到 24 个字符，只能包含小写字母数字字符。
