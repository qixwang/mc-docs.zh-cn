...
* <a id="redundancy-options"></a>
  **Azure 文件支持哪些存储冗余选项？**  
    当前，Azure 文件支持本地冗余存储 (LRS) 和异地冗余存储 (GRS)。 将来我们计划支持读取访问权限异地冗余存储 (RA-GRS)，但目前还没有可分享的日程表。
...
**如何确保已静态加密 Azure 件共享？**  
    ...
    是的。 有关详细信息，请参阅 [Azure 存储服务加密](../common/storage-service-encryption.md?toc=%2fstorage%2ffiles%2ftoc.json)。
