...
description: 在具有分层命名空间的存储帐户中使用 Azure CLI 管理目录和文件以及目录访问控制列表 (ACL)。
services: storage
author: WenJason
ms.service: storage
...
本文介绍了如何使用 [Azure 命令行接口 (CLI)](/cli/?view=azure-cli-latest) 在具有分层命名空间的存储帐户中创建和管理目录、文件与权限。 
...
2. 使用以下命令验证安装的 Azure CLI 版本是否为 `2.5.1` 或以上。
   ...
   ```azurecli
    az --version
   ```
   如果你的 Azure CLI 版本低于 `2.5.1`，请安装最新版本。 请参阅[安装 Azure CLI](/cli/install-azure-cli?view=azure-cli-latest)。

## <a name="connect-to-the-account"></a>连接到帐户

1. 如果要在本地使用 Azure CLI，请运行登录命令。
   ...
   否则，请在 [https://aka.ms/deviceloginchina](https://aka.ms/deviceloginchina) 处打开浏览器页，然后输入终端中显示的授权代码。 然后，在浏览器中使用帐户凭据登录。
...
> [!NOTE]
> 本文中提供的示例演示 Azure Active Directory (AD) 授权操作。 若要了解有关授权方法的详细信息，请参阅[使用 Azure CLI 授权访问 blob 或队列数据](../common/authorize-data-operations-cli.md)。

## <a name="create-a-file-system"></a>创建文件系统

文件系统充当文件的容器。 可以使用 `az storage fs create` 命令创建一个文件系统。 
...
可使用 `az storage fs show` 命令将文件系统的属性输出到控制台。
...
使用 `az storage fs file list` 命令列出目录的内容。

此示例列出名为 `my-file-system` 的文件系统的内容。
...
此示例将名为 `my-directory` 的目录添加到位于名为 `mystorageaccount` 的帐户下名为 `my-file-system` 的文件系统中。
...
可使用 `az storage fs directory show` 命令将目录的属性输出到控制台。
...
此示例在同一文件系统中将目录的名称 `my-directory` 重命名为 `my-new-directory`。

```azurecli
az storage fs directory move -n my-directory -f my-file-system --new-directory "my-file-system/my-new-directory" --account-name mystorageaccount --auth-mode login
```

此示例将目录移动到名为 `my-second-file-system` 的文件系统。
...
使用 `az storage fs directory exists` 命令确定文件系统中是否存在特定的目录。
...
使用 `az storage fs file download` 命令从目录中下载文件。
...
可使用 `az storage fs file show` 命令将文件的属性输出到控制台。
...
此示例将文件的名称 `my-file.txt` 重命名为 `my-file-renamed.txt`。
...
此示例针对所有者用户、所有者组或其他用户的目录设置默认 ACL，然后将 ACL 输出到控制台。
...
将 `-permissions` 参数设置为 ACL 的简短形式以更新目录或文件的 ACL。
...
还可将 `--owner` 或 `group` 参数设置为用户的实体 ID 或用户主体名称 (UPN)，来更新目录或文件的所有者用户和组。 
