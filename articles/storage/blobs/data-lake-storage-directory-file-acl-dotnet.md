...
ms.reviewer: prishet
ms.openlocfilehash: bd4132a67b28e43b1da73c0ae14c72d96a055477
ms.sourcegitcommit: 6a8bf63f55c925e0e735e830d67029743d2c7c0a
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/03/2020
ms.locfileid: "75624474"
---
...
> [!IMPORTANT]
> 本文中重点介绍的 [Azure.Storage.Files.DataLake](https://www.nuget.org/packages/Azure.Storage.Files.DataLake/12.0.0-preview.6) NuGet 包目前为公共预览版。

[包 (NuGet)](https://www.nuget.org/packages/Azure.Storage.Files.DataLake/12.0.0-preview.6) | [示例](https://github.com/Azure/azure-sdk-for-net/tree/master/sdk/storage/Azure.Storage.Files.DataLake) | [API 参考](https://azuresdkdocs.blob.core.windows.net/$web/dotnet/Azure.Storage.Files.DataLake/12.0.0-preview.6/api/index.html) | [提供反馈](https://github.com/Azure/azure-sdk-for-net/issues)
...
可以通过调用 **FileSystemClient.CreateDirectoryAsync** 方法来创建目录引用。
...
可以通过调用 **DirectoryClient.RenameAsync** 方法来重命名或移动目录。 以参数形式传递所需目录的路径。 
...
可以通过调用 **directoryClient.GetAccessControlAsync** 方法获取目录的访问控制列表 (ACL)，并通过调用 **DirectoryClient.SetAccessControl** 方法来设置 ACL。
...
首先，通过创建 **DataLakeFileClient** 类的实例，在目标目录中创建文件引用。 通过调用 **DataLakeFileClient.AppendAsync** 方法上传文件。 确保通过调用 **DataLakeFileClient.FlushAsync** 方法完成上传。
...
可以通过调用 **DataLakeFileClient.GetAccessControlAsync** 方法获取文件的访问控制列表 (ACL)，并通过调用 **FileClient.SetAccessControl** 方法来设置 ACL。
...
首先，创建表示要下载的文件的一个 **DataLakeFileClient** 实例。 使用 **FileClient.ReadAsync** 方法，并分析返回值来获取一个 [Stream](https://docs.microsoft.com/dotnet/api/system.io.stream) 对象。 使用任何 .NET 文件处理 API 将来自流的字节保存到文件。 
...
可以通过调用 **FileSystemClient.ListPathsAsync** 方法并枚举结果来列出目录内容。
