---
title: 监视 Azure Cosmos DB | Azure Docs
...
<!--Not Avaialble on ## Azure Monitor for Cosmos DB (Preview)-->
...
Azure Cosmos DB 收集的监视数据及其他 Azure 资源类型与“来自 Azure 资源的监视数据”中所述的类型相同。 请参阅 [Azure Cosmos DB 监视数据参考](monitor-cosmos-db-reference.md)来获取有关 Azure Cosmos DB 创建的日志和指标的详细参考。
...
* 查询已按资源分组的所有用户活动：
    ...
    | summarize count() by Resource
    ```
* 获取与 DataPlaneRequests 和 QueryRunTimeStatistics 中的数据联接的大于 100 RU 的所有查询   。
...
* 获取分区键统计信息，在数据库帐户排名前 3 的分区中评估偏差：
...
<!-- Update_Description: new article about monitor cosmos db -->
<!--NEW.date: 04/27/2020-->
