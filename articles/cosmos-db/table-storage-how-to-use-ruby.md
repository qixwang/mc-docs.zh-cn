...
本文介绍如何创建表、存储数据以及对数据执行 CRUD 操作。 选择 Azure 表服务或 Azure Cosmos DB 表 API。 本文中所述的示例是采用 Ruby 编写的，并使用了[用于 Ruby 的 Azure 存储表客户端库](https://github.com/azure/azure-storage-ruby/tree/master/table)。 涉及的场景包括创建表、删除表、插入实体以及查询表中的实体。
...
## <a name="add-access-to-azure-storage-or-azure-cosmos-db"></a>添加对 Azure 存储或 Azure Cosmos DB 的访问权限
