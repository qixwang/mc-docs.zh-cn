...
Azure Cosmos DB 是可缩放的多区域分布式完全托管型数据库。 它能够确保数据访问延迟较低。 若要详细了解 Azure Cosmos DB，请参阅[概述](introduction.md)文章。 本文说明如何将连接到 Couchbase 的 Java 应用程序迁移到 Azure Cosmos DB 中的 SQL API 帐户。
...
无论指定还是不指定分区键，均可读取文档。 如果不指定分区键，则读取操作将被视为跨分区查询。 考虑以下代码示例。第一个示例使用 ID 和分区键字段执行操作。 第二个示例使用常规字段，且不指定分区键字段。
...
* 无需使用 META 关键字或引用第一级文档。 你可以创建自己的对容器的引用。 在此示例中，我们已将其视为“c”（可以是任何内容）。 此引用用作所有第一级字段的前缀。 例如，c.id、c.country，等等。
