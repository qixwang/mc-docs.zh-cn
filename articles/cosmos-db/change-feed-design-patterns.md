...
使用 Azure Cosmos DB 更改源可以高效处理具有大量写入的大型数据集。 更改源还提供用于查询整个数据集以确定更改内容的替代方法。 本文档重点介绍常用的更改源设计模式、设计优缺点和更改源的限制。
...
Azure Cosmos DB 更改源可以简化需要基于特定事件触发通知或调用 API 的方案。 可以使用[更改源进程库](change-feed-processor.md)自动轮询容器的更改，并在每次发生写入或更新时调用外部 API。

还可以基于特定的条件，有选择性地触发通知或发送对 API 的调用。 例如，如果你要使用 [Azure Functions](change-feed-functions.md) 从更改源中读取数据，可以将逻辑放入函数中，以便仅在满足特定的条件时才发送通知。 尽管在每次进行写入和更新期间都会执行 Azure Function 代码，但只有在满足特定的条件时才会发送通知。

## <a name="real-time-stream-processing"></a>实时流处理

Azure Cosmos DB 更改源可用于为 IoT 进行实时流处理，或者针对操作数据进行实时分析处理。
...
在许多情况下，流处理实现首先会将大量传入数据接收到 Azure 事件中心或 Apache Kafka 等临时消息队列中。 由于 Azure Cosmos DB 能够支持持续较高的数据引入速率，并保证较低的读取和写入延迟，因此，更改源是极佳的替代方案。 与消息队列相比，Azure Cosmos DB 更改源的优势包括：

### <a name="data-persistence"></a>数据暂留

写入到 Azure Cosmos DB 的数据将出现在更改源中，并在删除之前一直会得到保留。 消息队列通常具有最长的保留期。 例如，[Azure 事件中心](https://www.azure.cn/home/features/event-hubs/)提供的最长数据保留期为 90 天。

### <a name="querying-ability"></a>查询能力

除了能够从 Cosmos 容器的更改源中读取数据以外，还可以对 Azure Cosmos DB 中存储的数据运行 SQL 查询。 更改源不是对容器中已有的数据进行复制，而只是一种不同的数据读取机制。 因此，如果从更改源中读取数据，读取结果始终与对同一 Azure Cosmos DB 容器进行查询的结果相一致。
...
处理更改源中的项后，可以生成具体化视图，并将聚合值存回到 Azure Cosmos DB 中。 例如，若要使用 Azure Cosmos DB 来构建游戏，可以使用更改源，根据已完成的游戏的分数实时更新排行榜。
...
* 实现应用程序级数据分层和存档。 例如，可将“热数据”存储在 Azure Cosmos DB 中，并将陈旧的“冷数据”移到 [Azure Blob 存储](../storage/common/storage-introduction.md)等其他存储系统中。

如果必须[使各个分区和容器中的数据非规范化](how-to-model-partition-example.md#v2-introducing-denormalization-to-optimize-read-queries
)，可以从用作此数据复制操作的源的容器更改源中读取数据。 使用更改源的实时数据复制只能保证最终一致性。 可以在 Cosmos 容器的处理更改中[监视更改源处理器的滞后程度](how-to-use-change-feed-estimator.md)。
...
由于 Azure Cosmos DB 在横向可伸缩性和高可用性方面的优势，因此在事件溯源模式中，它是极佳的仅限追加的中心持久数据存储。 此外，更改源处理器库提供[“至少一次”](change-feed-processor.md#error-handling)保证，确保不会遗漏任何事件的处理。

## <a name="current-limitations"></a>当前限制

你应了解更改源的几项重要限制。 尽管 Cosmos 容器中的项始终会保留在更改源中，但更改源并非完整的操作日志。 设计利用更改源的应用程序时，需要考虑到一些重要方面。

### <a name="intermediate-updates"></a>中间更新

更改源中仅包含对给定项所做的最新更改。 处理更改时，将会读取最新可用的项版本。 如果在短时间内对同一项进行了多次更新，可能会遗漏中间更新的处理。 若要跟踪更新并重放以往对某个项的更新，我们建议将这些更新建模为一系列写入操作。

### <a name="deletes"></a>Deletes

更改源不会捕获删除操作。 如果删除容器中的某个项，则该项也会从更改源中删除。 处理此操作的最常用方法是在要删除的项上添加一个软标记。 可以添加名为“deleted”的属性，并在删除时将其设置为“true”。 此文档更新将出现在更改源中。 可以在此项上设置 TTL，以便以后可以自动删除此项。
...
将为每个客户保留当前购物车内容的具体化视图。 此应用程序必须确保按事件的发生顺序处理这些事件。 例如，如果在删除商品 A 之前处理了购物车结帐，则卖家可能已经为客户交付了商品 A，而不是所需的商品 B。为了保证按发生顺序处理这四个事件，这些事件应属于同一分区键值。 如果选择用户名（每个客户都有唯一的用户名）作为分区键，则可以保证这些事件按照它们写入到 Azure Cosmos DB 的相同顺序出现在更改源中。 

## <a name="examples"></a>示例

下面是一些真实的更改源代码示例，这些示例超出了 Azure 文档中所提供的示例的范围：
...
<!-- Update_Description: new article about change feed design patterns -->
<!--NEW.date: 04/27/2020-->