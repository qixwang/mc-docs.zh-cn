...
使用 Azure Cosmos DB 可对事务数据运行多区域分布式的低延迟分析和 AI。 Azure Cosmos DB 原生支持 Apache Spark 和 Jupyter Notebooks，可帮助缩短见解的生成时间。 由于会引入和提供数据，因此将会针对 Azure 区域中的本地数据库副本运行分析。 可以直接对存储在数据分区中的带索引多模型数据执行 Apache Spark 查询。

使用 Azure Cosmos DB 中的内置 Apache Spark 支持可以通过 Apache Spark 针对 Azure Cosmos 帐户中存储的数据运行分析。 Azure Cosmos DB 原生支持在多区域分布式 Cosmos 数据库中直接执行 Apache Spark 作业。 借助这些功能，开发人员、数据工程师和数据科学家可以使用 Azure Cosmos DB 作为灵活、可缩放且高效的数据平台来运行 **OLTP 和 OLAP/HTAP** 工作负荷。
...
Azure Cosmos DB 中的 Apache Spark 支持在 Apache Spark 运行时中提供内置的机器学习支持。 运行时包括 Spark MLLib、适用于 Spark 的 Azure 机器学习、Azure 机器学习和认知服务。 借助这些功能，数据科学家、数据工程师和数据分析师可以在短时间内以较低的成本直接在 Azure Cosmos DB 中生成和操作化机器学习模型。

## <a name="key-benefits"></a>主要优点

### <a name="low-latency-operational-analytics-and-ai"></a>低延迟运营分析和 AI

通过针对多区域分布式 Azure Cosmos 数据库使用 Apache Spark，现在可以在中国各地快速获取实时见解。 Azure Cosmos DB 使用三项关键技术来实现支持弹性缩放的**多区域分布式低延迟运营分析**：

* 由于 Azure Cosmos 数据库是多区域分布的，因此将在数据生成者（例如用户）所在位置本地引入所有数据。 无论数据生成者和使用者位于中国的哪个位置，都会针对最靠近他们的本地副本提供查询服务。 

    <!--MOONCAKE: CORRECT ON in China -->

* 所有分析查询将会直接针对数据分区中存储的索引数据执行，而无需进行任何不必要的数据移动。

* 由于 Spark 与 Azure Cosmos DB 共置在一起，只会发生极少量的中间转换和数据移动，因此可以提高性能和可伸缩性。

* 所有 Azure Cosmos DB 核心功能（例如多主数据库、自动故障转移等）都适用于 Azure Cosmos DB 中的内置 Apache Spark。
...
Apache Spark 作业将会利用 Azure Cosmos DB 的优势，例如，行业领先的综合性 [SLA](https://www.azure.cn/support/sla/documentdb/) (99.999)，且可以避免单独管理各个 Apache Spark 群集所造成的开销。 这些 SLA 涵盖了吞吐量、99% 时间内的延迟、一致性和高可用性。 

### <a name="mixed-operational-and-analytical-workloads-with-complete-isolation"></a>完全隔离的混合运营与分析工作负荷

将 Apache Spark 集成到 Azure Cosmos DB 可以弥合事务与分析功能的隔离状态，而这种隔离是客户在生成多区域规模的云原生应用程序时面临的主要痛点之一。 OLTP 和 OLAP 工作负荷并行运行，不会相互干扰。

### <a name="low-latency-analytical-and-transactional-storage"></a>低延迟分析和事务存储

Azure Cosmos DB 原生将数据存储在两个不同的存储层中：事务存储（行导向）和分析存储（列导向，采用 Apache Parquet 文件格式）。 它在多个区域复制每个层中的数据，并允许用户根据保留策略独立管理这些层中的数据。
...
<!--Update_Description: new articles on spark api introduction -->
<!--ms.date: 10/28/2019-->
