...
<a name="analytical-schema"></a>
#### <a name="automatically-handle-schema-updates"></a>自动处理架构更新
...
* 以不区分大小写的方式进行比较时属性名称是唯一的
    ...
    * 例如，以下项没有完善定义的架构 - 以不区分大小写的方式进行比较时 `{"Name": "fred"} {"name": "john"}` - `"Name"` 和 `"name"` 相同
...
如果你拥有多区域分发的 Azure Cosmos DB 帐户，为容器启用分析存储后，它将适用于该帐户的所有区域。  对操作数据的任何更改都会将多区域复制到所有区域。 你可以高效地对 Azure Cosmos DB 中距离最近的区域的数据副本运行分析查询。
