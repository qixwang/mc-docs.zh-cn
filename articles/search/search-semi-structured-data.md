...
[创建一个 Azure 存储帐户](https://docs.azure.cn/storage/common/storage-quickstart-create-account)，用于存储示例数据。
...
此调用的终结点为 `https://[service name].search.chinacloudapi.cn/datasources?api-version=2019-05-06`。 请将 `[service name]` 替换为搜索服务的名称。 
...
```json
{
    "@odata.context": "https://exampleurl.search.chinacloudapi.cn/$metadata#datasources/$entity",
...
```json
{
    "@odata.context": "https://exampleurl.search.chinacloudapi.cn/$metadata#indexes/$entity",
...
此调用的 URL 为 `https://[service name].search.chinacloudapi.cn/indexers?api-version=2019-05-06`。 请将 `[service name]` 替换为搜索服务的名称。
...
```json
{
    "@odata.context": "https://exampleurl.search.chinacloudapi.cn/$metadata#indexers/$entity",
