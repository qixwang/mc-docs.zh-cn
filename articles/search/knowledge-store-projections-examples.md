...
ms.date: 03/16/2020
ms.openlocfilehash: b68b9414f7b6afcd4ddfab321caf5fa12771dd55
ms.sourcegitcommit: d5eca3c6b03b206e441b599e5b138bd687a91361
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/09/2020
ms.locfileid: "78934928"
---
...
投影是知识存储中扩充的文档的物理表达形式。 有效利用扩充的文档需要具备结构。 本文探讨结构和关系，介绍如何生成投影属性，以及如何关联所创建的投影类型之间的数据。 

若要创建投影，必须使用[整形器技能](cognitive-search-skill-shaper.md)为数据整形以创建自定义对象，或者在投影定义中使用内联的整形语法。 
...
如果你熟悉[投影](knowledge-store-projection-overview.md)的话，则会记得以下三种类型：
...
为了理解数据整形与投影之间的相互关系，我们将使用以下技能集作为探索各种配置的基础。 此技能集处理原始图像和文本内容。 对于我们要支持的方案，将会基于文档的内容和技能的输出定义投影。
...
假设我们要生成一个仪表板，在其中可将从文档中提取的关键短语可视化为字云。 若要创建适当的数据结构，可以在技能集中添加一个整形器技能，以创建包含文档特定的详细信息和关键短语的自定义形状。 该自定义形状在 `document` 根节点上称为 `pbiShape`。
...
创建可投影到表存储的自定义形状。 如果没有自定义形状，投影只能引用单个节点（每个输出对应一个投影）。 创建自定义形状可将各个元素聚合成新的逻辑整体，而该整体又可以投影为单个表，或者在一系列表之间切片和分布。 
...
在继续学习下一个示例之前，让我们回顾一下表投影的各个方面，以理解切片和关联数据的机制。
...
Power BI 依赖于这些生成的键来发现表中的关系。 如果需要以不同的方式命名子表中的列，请在父表中设置 ```referenceKeyName``` 属性。 例如，将 ```generatedKeyName``` 设置为 pbiDocument 表的 ID，并将 ```referenceKeyName``` 设置为 DocumentID。 这会导致包含文档 ID 的 pbiEntities 和 pbiKeyPhrases 表中的列命名为 DocumentID。

## <a name="projecting-to-objects"></a>投影到对象

对象投影不存在表投影那样的限制，更适合用于投影大型文档。 此示例将整个文档投影到对象投影。 对象投影限制为容器中的单个投影，并且无法切片。

为了定义对象投影，我们将在投影中使用 ```objects``` 数组。 可以使用整形器技能或使用对象投影的内联整形来生成新形状。 表示例演示的是创建形状和切片的方法，而本示例演示的是内联整形的用法。 

内联整形是指在投影输入的定义中创建新形状的功能。 内联整形创建一个匿名对象，该对象与整形器技能生成的对象相同（在本例中为 `pbiShape`）。 定义一个你不打算重复使用的形状时，内联整形非常有用。

投影属性是一个数组。 此示例将一个新的投影实例添加到数组，其中，knowledgeStore 定义包含内联投影。 使用内联投影时，可以省略整形器技能。
...
为了生成文件投影，我们将在投影对象中使用 `files` 数组。 此示例将从文档中提取的所有图像投影到名为 `samplefile` 的容器。
...
若要获取这些投影所需的形状，请首先添加一个新的整形器技能用于创建名为 `crossProjection` 的形状对象。 
...
在合并的 crossProjection 对象中，可将对象切片成多个表，将 OCR 输出捕获为 Blob，然后将图像保存为文件（也在 Blob 存储中）。
...
此示例还重点演示了投影的另一项功能。 在同一投影对象内部定义投影的多个类型后，将在不同类型（表、对象、文件）的内部和之间表达一种关系，使你可以从文档的表行开始，在对象投影中的该文档内找到图像的所有 OCR 文本。 
...
在迭代知识存储定义时，请考虑将增量扩充用作下一步。 增量扩充基于缓存，可让你重复使用任何不受技能集修改所影响的扩充。 增量扩充对于包含 OCR 和图像分析的管道特别有用。
