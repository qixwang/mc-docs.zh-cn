...
ms.date: 03/02/2020
ms.openlocfilehash: 94566ec4db9c502da561ab180e2790a574d00ef2
ms.sourcegitcommit: 094c057878de233180ff3b3a3e3c19bc11c81776
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/20/2020
ms.locfileid: "77501429"
---
# <a name="adjust-capacity-in-azure-cognitive-search"></a>在 Azure 认知搜索中调整容量

在[预配搜索服务](search-create-service-portal.md)和锁定特定的定价层之前，请花几分钟时间了解服务中的副本和分区的角色、是否需要按比例增大或加速分区，以及如何为预期负载配置服务。

容量是[所选层](search-sku-tier.md)（层确定硬件特征）以及预期工作负荷所需的副本和分区组合的一个功能因素。 本文重点介绍副本和分区的组合以及交互。
...
单个服务必须具有足够的资源才能处理所有工作负荷（索引和查询）。 没有任何工作负荷在后台运行。 如果查询请求在性质上不频繁，则可以计划索引编制，但如果不这样做，服务也不会排定任务的优先级。
...
在生产服务中，应分配三个副本来满足 SLA 的要求。 如果遇到查询性能缓慢的问题，补救措施之一是添加副本，使更多的索引副本联机，以支持更大的查询工作负荷，并通过多个副本对请求进行负载均衡。
...
[所选的层](search-sku-tier.md)确定了分区大小和速度，每个层已根据一组适合不同方案的特征进行优化。 如果选择更高端的层，所需的分区数可能比使用 S1 时更少。
