...
> [!IMPORTANT]
> 从 2020 年 7 月 15 日起，新建的搜索服务将使用 BM25 排名函数，经证实，在大多数情况下，该函数提供的搜索排名优于当前默认排名，可以更好地达到用户的期望。  除了卓越的排名准确度以外，BM25 还启用了根据文档大小等因素优化结果的配置选项。  
>
> 由于此项变更，你很可能会发现搜索结果的顺序稍有变化。   对于想要测试此项变更所产生的影响的用户，我们在 2019-05-06-Preview API 中提供了对新索引启用 BM25 评分的功能。  

本文介绍如何将 2020 年 7 月 15 日之前创建的服务更新为使用新的 BM25 排名算法。

Azure 认知搜索将使用 Okapi BM25 算法的官方 Lucene 实现 (BM25Similarity)，该实现将取代以前使用的 ClassicSimilarity 实现。   与旧式 ClassicSimilarity 算法一样，BM25Similarity 是类似于 TF-IDF 的检索函数，它使用字词频率 (TF) 和逆向文档频率 (IDF) 作为变量来计算每个文档-查询对的相关性评分，然后可以使用这些评分进行排名。 虽然 BM25 在概念上类似于旧式“经典相似性”算法，但它立足于概率信息检索来改善结果。 BM25 还提供高级自定义选项，例如，允许用户确定如何根据匹配字词的字词频率调整相关性评分。

## <a name="how-to-test-bm25-today"></a>如何立即测试 BM25

创建新索引时，可以设置一个“similarity”属性。 需要使用 2019-05-06-Preview 版本，如下所示。 
...
对于 2020 年 7 月 15 日之前创建的服务：如果 similarity 被省略或设置为 null，则索引将使用旧式经典相似性算法。

对于 2020 年 7 月 15 日之后创建的服务：如果 similarity 被省略或设置为 null，则索引将使用新的 BM25 相似性算法。

还可以显式将 similarity 设置为以下两个值之一：“#Microsoft.Azure.Search.ClassicSimilarity”或“#Microsoft.Azure.Search.BM25Similarity”。  


## <a name="bm25-similarity-parameters"></a>BM25 相似性参数

BM25 相似性添加了两个用户可自定义参数用于控制计算出的相关性评分：
