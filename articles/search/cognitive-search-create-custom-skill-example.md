...
在此示例中，了解如何创建 Web API 自定义技能。 此技能将接受位置、公众人物和组织，并返回其说明。 此示例使用 [Azure Function](https://azure.microsoft.com/services/functions/) 来包装自定义实体搜索 API，以便实现自定义技能接口。
...
### <a name="modify-the-code-to-call-the-custom-entity-search-service"></a>修改代码以调用自定义实体搜索服务
...
请确保根据注册自定义实体搜索 API 时获得的密钥，在 `key` 常量中输入你自己的密钥值  。

为方便起见，此示例在单个文件中包含了所有必需的代码。 可以在 [power skills 存储库](https://github.com/Microsoft/SkillsExtractorCognitiveSearch)中找到该同一技能的结构略好的版本。

当然，你可以将文件从 `Function1.cs` 重命名为 `CustomEntitySearch.cs`。
