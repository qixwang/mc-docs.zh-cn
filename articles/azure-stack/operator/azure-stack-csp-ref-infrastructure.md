...
Azure Stack Hub 包括实时跟踪使用情况所需的基础结构，因此会将使用情况转发到 Azure。 在 Azure 中，Azure Commerce 会处理[使用情况数据并根据使用情况向相应的 Azure 订阅收费](azure-stack-billing-and-chargeback.md)。 此过程的工作方式与在 Azure 云中跟踪使用情况的方式相同。
...
Azure Stack Hub 有本地使用情况计量。 本地使用情况将映射到在 Azure Commerce 中使用的计量。 但是，计量 ID 是不同的。 在本地可用的计量多于 Azure 用于计费的计量。
