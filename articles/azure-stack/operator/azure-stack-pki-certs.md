...
如果打算在部署和配置 Azure Stack Hub 之后部署其他 Azure Stack Hub PaaS 服务（SQL、MySQL 和 应用服务），则需要请求额外的证书来涵盖 PaaS 服务的终结点。

> [!IMPORTANT]
> 用于应用服务、SQL 和 MySQL 资源提供程序的证书需要包含与用于全局 Azure Stack Hub 终结点的证书相同的根颁发机构。

下表描述了 SQL 和 MySQL 适配器以及应用服务所需的终结点与证书。 无需将这些证书复制到 Azure Stack Hub 部署文件夹。 应在安装其他资源提供程序时提供这些证书。
