...
首先，为 Azure 创建网络资源。 以下说明介绍如何使用 [Azure 门户](https://portal.azure.cn/)来创建资源。
...
7. 选择 VNet 的**位置**。  如果使用示例值，请选择“中国东部”，或使用另一个位置。
8. 选择“固定到仪表板”。
9. 选择“创建” 。
...
7. 在“名称”字段中键入 **Azure-GW-PiP**，然后选择“确定”。 
...
## <a name="create-a-custom-ipsec-policy"></a>创建自定义 IPSec 策略
...
现在请在 Azure 中创建一个 VM，然后将其置于虚拟网络的 VM 子网上。
...
服务管理员可以用户身份登录，对其用户可能使用的计划、套餐和订阅进行测试。 请在登录之前[创建用户帐户](../operator/azure-stack-add-new-user-aad.md)（如果尚未这样做）。
...
1. 登录到 Azure 门户。
2. 从左侧导航栏中选择“虚拟机”。
3. 从 VM 列表中找到前面创建的 **Azure-VM**，并选择它。
4. 在 VM 部分选择“连接”。
5. 使用创建 VM 时所配置的帐户登录。
...
11. 获得远程 VM 的答复表示测试成功。 此时可关闭“VM”窗口。
