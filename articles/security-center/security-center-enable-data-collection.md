...
本文介绍如何安装 Log Analytics 代理以及如何设置 Log Analytics 工作区，以便在其中存储收集的数据。 这两项操作都需要启用数据收集。 
...
> - 在 Log Analytics 中存储数据，无论使用的是新工作区还是现有工作区，都可能会产生额外的数据存储费用。 有关详细信息，请参阅[定价页](https://www.azure.cn/pricing/details/security-center/)。
...
> 在 Log Analytics 中存储数据可能会产生额外的数据存储费用。 有关详细信息，请参阅[定价页](https://wwww.azure.cn/pricing/details/security-center/)。
...
5. 选择要在其中设置 Log Analytics 代理的所需工作区的定价层。 <br>若要使用现有工作区，请设置该工作区的定价层。 这会在该工作区中安装一个安全中心解决方案（如果尚不存在）。

    a.  在“安全中心”主菜单下，选择“定价和设置”。
...
选择用于存储数据的工作区时，跨所有订阅的所有工作区可用。 通过跨订阅工作区选择，可以从不同订阅中运行的虚拟机收集数据并将其存储在所选的工作区中。 如果在组织中使用集中式工作区，并想要使用该工作区来收集安全数据，则这种选择非常有用。 有关如何管理工作区的详细信息，请参阅[管理工作区访问权限](https://docs.azure.cn/log-analytics/log-analytics-manage-access)。
...
选择筛选策略的具体步骤：
1. 在“数据收集”页面，选择“安全事件”下的筛选策略 。
2. 选择“保存” 。

   ![选择筛选策略][5]

### <a name="automatic-provisioning-in-cases-of-a-pre-existing-agent-installation"></a>对现有的代理安装进行自动预配<a name="preexisting"></a> 
...
- 存在现有的 VM 扩展<br>
    ...
    - 若要查看现有扩展将数据发送到哪个工作区，请运行测试来[验证与 Azure 安全中心的连接](https://blogs.technet.microsoft.com/yuridiogenes/2017/10/13/validating-connectivity-with-azure-security-center/)。 也可以打开 Log Analytics 工作区，选择一个工作区，再选择 VM，然后查看 Log Analytics 代理连接。 
...
可通过多种方法手动安装 Log Analytics 代理。 手动安装时，请务必禁用自动预配。
