...
ms.date: 04/23/2020
ms.custom: seodec18
ms.openlocfilehash: 82f98dd2e0ccf927ce9c9a1759f4c29386338b32
ms.sourcegitcommit: ebedf9e489f5218d4dda7468b669a601b3c02ae5
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/26/2020
ms.locfileid: "82159197"
---
# <a name="troubleshoot-azure-stream-analytics-outputs"></a>Azure 流分析输出的故障排除

本文介绍了 Azure 流分析输出连接的常见问题、如何排查输出问题，以及如何解决这些问题。 许多故障排除步骤都需要为流分析作业启用诊断日志。 如果未启用诊断日志，请参阅[使用诊断日志对 Azure 流分析进行故障排除](stream-analytics-job-diagnostic-logs.md)。
...
2.  在“监视器”  选项卡上查看[“监控指标”  ](stream-analytics-monitoring.md)。由于值将进行聚合，因此指标会延迟几分钟。
   * 如果“输入事件数”大于 0，则作业可以读取输入数据。 如果“输入事件数”不大于 0，则作业的输入有问题。 若要了解如何排查输入连接问题，请参阅[排查输入连接问题](stream-analytics-troubleshoot-input.md)。
   * 如果“数据转换错误数”大于 0 并不断增大，请参阅 [Azure 流分析数据错误](data-errors.md)，详细了解数据转换错误。
   * 如果“运行时错误数”大于 0，则表示作业可以接收数据，但在处理查询时会遇到错误。 若要查找错误，请转到[审核日志](../azure-resource-manager/management/view-activity-logs.md)并筛选“失败”  状态。
   * 如果 InputEvents 大于 0 但 OutputEvents 等于 0，则存在下列情况之一：
      * 查询处理导致生成零个输出事件。
      * 事件或字段可能存在格式错误，导致完成查询处理后生成零个输出。
      * 由于连接或身份验证原因，作业无法将数据推送到输出接收器。

   对于前文所述的所有这些错误，操作日志消息会解释其他详细信息（包括发生了什么情况），但如果查询逻辑筛选掉了所有事件，则不提供这些详细信息。 如果处理多个事件时生成错误，则错误数每 10 分钟会聚合一次。
...
当使用原始兼容性级别 (1.0) 时，Azure 流分析过去常常将列名更改为小写。 此行为已在以后的兼容性级别中修复。 为了保留这种情况，我们建议客户迁移到兼容性级别 1.1 和更高版本。 有关详细信息，请参阅 [Azure 流分析作业的兼容性级别](/stream-analytics/stream-analytics-compatibility-level)。
