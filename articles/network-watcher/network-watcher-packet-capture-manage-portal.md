...
1. 在浏览器中导航到 [Azure 门户](https://portal.azure.cn)，选择“所有服务”，然后在“网络”部分选择“网络观察程序”。   
...
完成数据包捕获会话后，捕获文件将上传到 Blob 存储或虚拟机中的本地文件。 数据包捕获的存储位置是在创建数据包捕获期间定义的。 用于访问保存到存储帐户的捕获文件的便利工具是 Azure 存储资源管理器，可以[下载](http://storageexplorer.com/)该工具。
