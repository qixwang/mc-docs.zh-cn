...
ms.date: 11/04/2019
ms.openlocfilehash: c3edf516b15fb242acee8f4f0206f77f2d5e36d8
ms.sourcegitcommit: 623d64ef33e80d5f84b6dcf6d1ef4120fe4b8c08
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/02/2020
ms.locfileid: "75599618"
---
...
1. 从存储容器中选择一个数据集，或创建新的数据集。 可以基于本地文件、Web URL、数据存储或 Azure 开放数据集创建数据集。 
    ...
    1. 选择“下一步”，将数据集上传到在工作区中自动创建的默认存储容器，或选择要用于试验的存储容器。  
    ...
    1. “确认详细信息”窗体是前面在“基本信息”与“设置和预览”窗体中填充的信息的摘要。    还可以选择使用已启用分析的计算来分析数据集。 详细了解[数据分析](#profile)。
...
1. （可选）附加配置：可用于更好地控制训练作业的其他设置。 否则，将会根据试验选择和数据应用默认设置。 

    其他配置|说明
    ------|------
    主要指标| 用于对模型进行评分的主要指标。 [详细了解模型指标](how-to-configure-auto-train.md#explore-model-metrics)。
    自动特征化| 选择此项可以启用或禁用自动化机器学习执行的预处理。 预处理包括自动化数据清理、准备和转换，以生成合成特征。 [详细了解预处理](#preprocess)。
    ...
    并发| *最大并发迭代数*：要在训练作业中测试的最大管道（迭代）数。 作业运行的迭代数不会超过指定的数目。 <br> *每个迭代的最大核心数*：选择在使用多核计算时要使用的多核限制。
...
Max| 列的最大值。 
计数| 列中缺失和未缺失条目的总数。
未缺失计数| 列中未缺失的条目数。 空字符串和误差被视为值，因此它们不会计入“未缺少计数”。
...
配置试验时，可以启用高级设置 `Preprocess`。 这意味着，将自动执行以下数据预处理和特征化步骤。
...
选择“启动”以运行试验。  试验准备过程最长可能需要 10 分钟。 训练作业可能需要额外的 2-3 分钟，使每个管道完成运行。
...
“运行详细信息”屏幕中会打开“详细信息”选项卡。   此屏幕显示试验运行的摘要，包括“运行状态”。  
...
1. 可以使用多个部署选项。 
    ...
    + 选项 1：若要部署最佳模型（根据定义的指标条件），请从“详细信息”选项卡中选择“部署最佳模型”。
...
* 尝试学习端到端的[有关使用 Azure 机器学习创建第一个自动化 ML 试验的教程](tutorial-first-experiment-automated-ml.md)。 
* [详细了解自动化机器学习](concept-automated-ml.md)和 Azure 机器学习。
* [了解自动化机器学习结果](how-to-understand-automated-ml.md)。
* [了解如何使用 Web 服务](/machine-learning/service/how-to-consume-web-service)。
