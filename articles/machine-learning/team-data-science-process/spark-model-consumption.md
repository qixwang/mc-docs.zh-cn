...
还可以将此 Python 代码添加到 [Azure Functions](/functions/) 以触发一个 Spark 作业提交，该作业提交基于各种事件（如计时器、创建或 blob 更新）为 blob 评分。 

如果首选无代码客户端体验，请使用 [Azure 逻辑应用](/app-service/logic/)通过在**逻辑应用设计器**上定义一个 HTTP 操作并设置其参数来调用 Spark 批处理评分。 
