...
description: 了解如何启用 HTTPS，以保护通过 Azure 机器学习部署的 Web 服务。
...
3. 在启用 SSL 的情况下部署或更新 Web 服务。

4. 更新 DNS，使其指向该 Web 服务。

> [!IMPORTANT]
> 如果要部署到 Azure Kubernetes 服务 (AKS)，可以购买属于自己的证书或使用 Microsoft 提供的证书。 如果使用 Microsoft 的证书，则无需获取域名或 SSL 证书。 有关详细信息，请参阅本文的[启用 SSL 并进行部署](#enable)部分。
...
可通过多种方式获取 SSL 证书（数字证书）。 最常用的方式是从证书颁发机构 (CA) 购买  。 无论证书来自哪里，都需要以下文件：
...
enable_ssl 方法可以使用 Microsoft 提供的证书或你购买的证书  。
  ...
  * 使用你购买的证书 时，请使用 ssl_cert_pem_file、ssl_key_pem_file、ssl_cname 参数     。 下面的示例演示如何使用 .pem 文件创建使用所购买的 SSL 证书的配置  ：
...
+ **对于 AKS：**
  ...
  [![Azure 机器学习：使用 SSL 保护 Web 服务](./media/how-to-secure-web-service/aks-public-ip-address.png)](./media/how-to-secure-web-service/aks-public-ip-address-expanded.png)
...
若要为部署到 Azure Kubernetes 服务的模型禁用 SSL，请创建 `SslConfiguration` 并设置 `status="Disabled"`，然后执行更新：
