...
管道步骤可重用；如果该步骤的输出未更改，而无需重新运行后续步骤则可运行该步骤。 例如，如果数据未更改，则无需重新运行高开销的数据准备步骤，即可重新训练模型。 管道还使数据科学家能够展开协作，同时可以处理机器学习工作流的不同环节。
...
运行是训练脚本的单次执行。 Azure 机器学习记录所有运行并存储以下信息：
