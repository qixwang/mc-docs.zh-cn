...
Azure API 管理是一项 Azure 服务，支持通过定义用户访问权限、使用限制和仪表板监视管理 REST API 终结点。 有关更多详细信息，请参阅 [Azure API 管理站点](/api-management/)。 若要开始使用 Azure API 管理，请参阅[导入和发布指南](/api-management/import-and-publish)。 这份额外指南（作为本指南的依据）涵盖更多主题，包括通知配置、层定价、响应处理、用户身份验证、创建产品、开发人员订阅和使用仪表板。
