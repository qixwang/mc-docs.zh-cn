...
ms.author: keli19
ms.date: 10/16/2019
ms.openlocfilehash: 678631dc147ea0f80ec2b17e8b072d4f32bc5ad0
ms.sourcegitcommit: 623d64ef33e80d5f84b6dcf6d1ef4120fe4b8c08
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 01/02/2020
ms.locfileid: "75598380"
---
...
3. 将“创建训练器模式”选项设置为“参数范围”。   使用“范围生成器”指定要在参数扫描中使用的值范围。   

    几乎所有的分类和回归模块都支持集成式参数扫描。 对于不支持配置参数范围的学习器，只能测试可用的参数值。
...
7.  选择运行次数：
    ...
    2. **针对随机网格的最大运行次数**：此选项还会控制针对参数值随机采样的迭代次数，但这些值不是从指定的范围随机生成的。 模块会创建所有可能参数值组合的矩阵。 然后，它针对该矩阵执行随机采样。 此方法更高效，且不容易出现区域过采样或欠采样的情况。
...
+ 若要使用用于在其他管道中进行评分的模型且不必重复执行优化过程，请右键单击模型输出，然后选择“另存为已训练的模型”。  
