...
| 功能 | 说明 | 模块 |
| --- |--- | --- |
| 数据输入和输出 | 将数据从云源移动到管道中。 在运行管道时将结果或中间数据写入到 Azure 存储、SQL 数据库或 Hive，或者使用云存储空间在管道之间交换数据。  | [手动输入数据](enter-data-manually.md) <br/> [导出数据](export-data.md) <br/> [导入数据](import-data.md) |
