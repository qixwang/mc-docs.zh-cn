 ...
 * 入口点函数最多可以包含两个输入参数：`Param<dataframe1>` 和 `Param<dataframe2>`
   ...
   > [!NOTE]
    > 引用传递给“执行 R 脚本”模块的数据作为 `dataframe1` 和 `dataframe2`，这不同于 Azure 机器学习设计器（设计器引用作为 `dataset1`、`dataset2`）  。 请检查以确保在脚本中正确引用输入数据。  
...
1. 将第一个“执行 R 脚本”模块添加到管道，然后在“R 脚本”文本框中键入以下代码，创建一个序列化对象 `A` 作为模块输出数据表中的列：    
