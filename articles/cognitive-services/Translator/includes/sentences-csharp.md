...
在 `Program` 类中，创建一个名为 `BreakSentenceRequest()` 的新函数。 此函数采用四个参数：`subscriptionKey`、`endpoint`、`route` 和 `inputText`。
...
你会注意到，在 `Main` 中声明了 `subscriptionKey`、`endpoint`、`route` 以及要评估的文本 `breakSentenceText`。
