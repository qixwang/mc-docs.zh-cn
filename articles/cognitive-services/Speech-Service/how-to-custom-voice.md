...
[自定义语音](https://speech.azure.cn/customvoice)是一组在线工具，使用它可为自有品牌创建可识别的独一无二的语音。 只需准备好几个音频文件和关联的听录内容即可完全入门。 请遵循以下链接开始创建自定义文本转语音体验。

## <a name="whats-in-custom-voice"></a>什么是自定义语音？

在开始使用自定义语音之前，需要一个 Azure 帐户和一个语音服务订阅。 创建帐户后，可以准备数据、训练和测试模型、评估语音质量，并最终部署自定义语音模型。

下图突出显示了使用[自定义语音门户](https://speech.azure.cn/customvoice)创建自定义语音模型的步骤。 请使用链接了解详细信息。
...
> [!NOTE]
> Microsoft 承诺设计负责任的 AI，其中包括以保护个人和社会的权利、促进人机交互透明化为目标。 出于此原因，我们未向所有客户正式发布自定义神经语音。 仅当我们评审你的应用程序，并且你承诺在遵守我们的道德原则的条件下使用该应用程序后，你才会获得对该技术的访问权限。 详细了解我们的[应用程序把关流程](https://docs.azure.cn/cognitive-services/speech-service/concepts-gating-overview)。
...
2. 登录到[自定义语音门户](https://speech.azure.cn/customvoice)。
3. 选择你的订阅并创建语音项目。
...
> [!IMPORTANT]
> [“自定义语音”门户](https://speech.azure.cn/customvoice)最近已更新！ 如果以前已在 CRIS.ai 门户或使用 API 创建了数据、模型、测试并已发布了终结点，则需要在新门户中创建一个新项目以连接到这些旧实体。
