...
现在，使用之前创建的 `SpeechConfig` 和 `AudioConfig` 对象创建 `SpeechRecognizer` 对象。 此对象也是在 using 语句中创建的，以确保正确释放非托管资源。 将此代码插入 `RecognizeSpeechAsync()` 方法中，用于包装 ````AudioConfig```` 对象的 using 语句中。
...
3. **开始识别** - 音频文件将发送到语音服务，听录为文本，并在控制台中呈现。
