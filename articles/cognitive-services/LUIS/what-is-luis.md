...
* **预生成的模型** LUIS 具有许多预生成的域，其中包括用于完成常见使用方案的意向和实体模型，这些意向和实体模型协同工作。 这些域包括可进行检查和编辑的已标记言语，让你能够对其进行自定义。 预生成域模型为你提供了整个设计，是快速开始使用 LUIS 的好方法。 此外，还有一些预生成的实体，例如可独立于预生成的域使用的货币和数字。
...
使用[预生成](luis-get-started-create-app.md)域创作新的 LUIS 应用。 
