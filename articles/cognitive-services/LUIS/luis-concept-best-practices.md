...
发布应用后，使用开发生命周期添加特征、发布和从终结点进行测试。 下一个创作周期不要从添加更多示例言语开始，否则 LUIS 无法根据实际用户言语学习你的模型。 
...
定期使用主动学习的“查看终结点言语”功能，而不是将更多示例言语添加到意向  。 因为应用会不断接收终结点话语，所以此列表会不断变化。
