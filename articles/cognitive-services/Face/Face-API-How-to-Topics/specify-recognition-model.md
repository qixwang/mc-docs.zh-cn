...
**PersonGroup** 中的所有 **Person** 应有一个唯一的识别模型，在创建该组（使用 `recognitionModel`PersonGroup - Create[PersonGroup - 创建]LargePersonGroup - Create[LargePersonGroup - 创建] 参数指定此模型。 如果不指定此参数，则会使用原始的 `recognition_01` 模型。 某个组始终使用创建它时所用的识别模型，将新的人脸添加到该组时，它们将与此模型相关联；创建组后无法更改此行为。 若要查看 **PersonGroup** 是使用哪个模型配置的，请结合设置为 [PersonGroup - Get] 的 _returnRecognitionModel_ 参数使用 **PersonGroup - Get** API。
...
在此代码中，创建了 ID 为 **的**PersonGroup`mypersongroupid`，它设置为使用 _recognition_02_ 模型提取人脸特征。
...
还可以指定一个识别模型来进行类似性搜索。 可以在使用 `recognitionModel`FaceList - Create[FaceList - Create]LargeFaceList - Create[LargeFaceList - Create] 来分配模型版本。 如果不指定此参数，则会使用原始的 `recognition_01` 模型。 某个人脸列表始终使用创建它时所用的识别模型，将新的人脸添加到该组时，它们将与此模型相关联；创建后无法更改此行为。 若要查看人脸列表是使用哪个模型配置的，请结合设置为 [FaceList - Get] 的 _returnRecognitionModel_ 参数使用 **FaceList - Get** API。
...
此代码使用用于提取特征的 `My face collection`recognition_02_模型创建名为_ 的人脸列表。 在此人脸列表中搜索与新检测到的人脸类似的人脸时，该人脸必须已使用 [Face - Detect] 模型检测到 (_Face - Detect_)。 如上一部分所述，模型需要保持一致。
...
1. 分别使用 **recognition_01** 和 _recognition_02_ 创建两个 _PersonGroup_。
1. 使用图像数据检测人脸，并将其注册到两个 **PersonGroup** 的 **Person**，然后使用 [PersonGroup - 训练] API 触发训练过程。
1. 在两个 [Face - Identify] 中使用 **Face - Identify** 进行测试，然后比较结果。
