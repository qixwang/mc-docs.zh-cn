...
要使用文件上传功能，必须首先将 Azure 存储帐户链接到 IoT 中心。 可通过 Azure 门户完成此任务，或通过 [IoT 中心资源提供程序 REST API](https://docs.microsoft.com/en-us/rest/api/iothub/iothubresource) 以编程方式完成此任务。 将 Azure 存储帐户与 IoT 中心关联后，当设备启动文件上传请求时，此服务将向该设备返回 SAS URI。
