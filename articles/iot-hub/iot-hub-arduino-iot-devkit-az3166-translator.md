---
title: 使用 Azure Functions 和认知服务创建 IoT DevKit 翻译
...
* 遵循[将 IoT DevKit AZ3166 连接到云中的 Azure IoT 中心](/iot-hub/iot-hub-arduino-iot-devkit-az3166-get-started)中的步骤准备好 DevKit。
...
2. 转到刚才创建的语音服务，单击“Keys”部分进行复制，记下访问它的 DevKit 的 Key1   。
...
3. 在 IoT Workbench 的“示例”页中，找到 DevKit Translator 并单击“打开示例”   。 然后选择用于下载示例代码的默认路径。
...
1. 在 VS Code 中，单击 `F1`，键入并选择 **`Azure IoT Device Workbench: Provision Azure Services...`** 。

   ![预配 Azure 服务](media/iot-hub-arduino-iot-devkit-az3166-translator/provision.png)

2. 按照以下步骤完成 Azure IoT 中心和 Azure Functions 的预配。

   ![预配步骤](media/iot-hub-arduino-iot-devkit-az3166-translator/provision-steps.png)

   记录创建的 Azure IoT 中心设备的名称。

3. 打开 `Functions\DevKitTranslatorFunction.cs` 并使用记录的设备名称和语音服务密钥更新以下代码行。
...
4. 单击 `F1`，键入并选择 **`Azure IoT Device Workbench: Deploy to Azure...`** 。 如果 VS Code 要求对重新部署进行确认，请单击“是”  。
...
6. 在 Azure 门户中，转到 Function App 部分，找到刚刚创建的 Azure 函数应用  。 单击 `devkit_translator`，然后单击“</> 获取函数 URL”以复制 URL  。
   ![复制函数 URL](media/iot-hub-arduino-iot-devkit-az3166-translator/get-function-url.png)

7. 将 URL 粘贴到 `azure_config.h` 文件。
...
2. 单击 `F1`，键入并选择 **`Azure IoT Device Workbench: Configure Device Settings... > Config Device Connection String`** 。 选择 **`Select IoT Hub Device Connection String`** ，将其配置为 DevKit。
...
4. 再次单击 `F1`，键入并选择 **`Azure IoT Device Workbench: Upload Device Code`** 。 它启动编译，并将代码上传到 DevKit。
...
> [将 IoT DevKit AZ3166 连接到 Azure IoT 远程监视解决方案加速器](./iot-hub-arduino-iot-devkit-az3166-devkit-remote-monitoring.md)
