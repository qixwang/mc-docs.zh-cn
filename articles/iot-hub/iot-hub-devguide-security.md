...
* **IoT 中心级别的共享访问策略**。 共享访问策略可以授予任意[权限](#iot-hub-permissions)组合。 可使用 [IoT 中心资源 REST API](https://portal.azure.cn) 或使用 [az iot 中心策略](https://docs.microsoft.com/en-us/rest/api/iothub/iothubresource) CLI 以编程方式在 [Azure 门户](/cli/iot/hub/policy?view=azure-cli-latest)中定义策略。 新建的 IoT 中心有以下默认策略：
