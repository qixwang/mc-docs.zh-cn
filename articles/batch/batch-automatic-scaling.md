...
可以通过将计算节点池与你定义的自动缩放公式  关联，来启用该池的自动缩放。 Batch 服务会使用自动缩放公式确定执行工作负荷所需的计算节点数。 Batch 会响应定期收集的服务指标数据。 Batch 使用此指标数据，基于公式并按可配置的间隔来调整池中的计算节点数。
...
>
>如果使用用户订阅配置创建了帐户，则该帐户将共享订阅的核心配额。 有关详细信息，请参阅 [Azure 订阅和服务限制、配额和约束条件](../azure-resource-manager/management/azure-subscription-service-limits.md#virtual-machines-limits)中的[虚拟机限制](../azure-resource-manager/management/azure-subscription-service-limits.md)。
>
>
...
> [!NOTE]
> 还可以使用别名 `$TargetDedicatedNodes` 指定 `$TargetDedicated` 变量。 同样，可以使用别名 `$TargetLowPriorityNodes` 指定 `$TargetLowPriority` 变量。 如果全名变量及其别名都由公式设置，则分配给全名变量的值将优先。
>
>
...
为了在 CPU 使用率低时减少专用节点数，如果过去 60 分钟的平均 CPU 使用率低于 20%，则公式中的下一个语句会将同一  *变量设置为专用节点当前目标数的 90%。* `$totalDedicatedNodes` 否则，使用在以上语句中填充的 `$totalDedicatedNodes` 的当前值。
...
> [!IMPORTANT]
> 创建启用自动缩放的池时，请不要在 _CreatePool_ 调用中指定 _targetDedicatedNodes_ 参数或 **targetLowPriorityNodes** 参数。 应该指定池中的 **AutoScaleEnabled** 和**AutoScaleFormula** 属性。 这些属性的值确定每种类型的节点的目标数。 另请注意，若要手动调整启用自动缩放功能的池的大小（例如，使用 [BatchClient.PoolOperations.ResizePoolAsync][net_poolops_resizepoolasync] 来调整），则必须先**禁用**该池的自动缩放，并调整其大小。
>
>
...
* 如果池中的自动缩放目前已启用，则可指定自动缩放公式和/或评估间隔。 必须至少指定其中的一个属性。
  ...
  * 如果指定新的自动缩放评估时间间隔，将停止现有评估计划并开始新的计划。 新计划的开始时间是发出启用自动缩放的请求的时间。
...
该公式首先获取当前时间。 如果日期是工作日（周一到周五）且时间是工作时间（早 8 点到晚 6 点），则会将目标池大小设置为 20 个节点。 否则，它将设置为 10 个节点。
