...
1. [以交互方式登录](/cli/authenticate-azure-cli#az-authenticate-azure-cli-interactive-log-in)。 从命令行自行运行Azure CLI 命令时，请以交互方式登录。
...
## <a name="script-examples"></a>脚本示例 <a name="sample-shell-scripts"></a>

请参阅 [CLI 脚本示例](cli-samples.md)，了解用于完成常规任务的 Batch。 这些示例涵盖在用于 Batch 的 Azure CLI 中提供的许多命令，这些命令用于创建和管理帐户、池、作业和任务。 

## <a name="json-files-for-resource-creation"></a>用于创建资源的 JSON 文件 <a name="json-files"></a>
...
* 可以使用 **选项查看**JSON 格式的命令输出`--json`。 例如， `az batch pool show pool001 --json` 以 JSON 格式显示 pool001 的属性。 然后，可以复制并修改此输出，以便在 `--json-file` 中使用（请参阅本文前面的“JSON 文件”）。
