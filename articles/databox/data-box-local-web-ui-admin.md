---
title: 使用本地 Web UI 管理 Azure Data Box 和 Azure Data Box Heavy | Microsoft Docs
...
Data Box 和 Data Box Heavy 的本地 Web UI 用于设备的初始配置。 还可以使用本地 Web UI 关闭或重启设备、运行诊断测试、更新软件、查看复制日志以及生成供 Azure 支持部门使用的日志包。 在具有两个独立节点的 Data Box Heavy 设备上，可以访问对应于每个设备节点的两个独立本地 Web UI。
...
如果遇到任何设备问题，可通过系统日志创建支持包。 Azure 支持部门使用此包来解决此问题。 要生成支持包，请执行以下步骤：
