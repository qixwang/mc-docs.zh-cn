...
ms.locfileid: "63849966"
---
# <a name="tutorial-create-a-custom-role-for-azure-resources-using-azure-cli"></a>教程：使用 Azure CLI 为 Azure 资源创建自定义角色

如果 [Azure 资源的内置角色](built-in-roles.md)不能满足组织的特定需求，则可以创建自定义角色。 对于本教程，你将使用 Azure CLI 创建名为 Reader Support Tickets 的自定义角色。 该自定义角色允许用户查看订阅管理平面中的所有内容，以及开具支持票证。
...
登录到 [Azure CLI](/cli/authenticate-azure-cli)。
...
1. 查看适用于 [Microsoft.Support 资源提供程序的](resource-provider-operations.md#microsoftsupport)操作列表。 这有助于了解可用来创建你的权限的操作。
    ...
    | Microsoft.Support/supportTickets/write | 创建或更新支持票证。 可以针对技术、计费、配额或订阅管理相关问题创建支持票证。 可以更新现有支持票证的严重性、联系详细信息和通信。 |
...
1. 使用 [az account list](/cli/account#az-account-list) 命令获取你的订阅 ID。
...
1. 若要新建自定义角色，请使用 [az role definition create](/cli/role/definition#az-role-definition-create) 命令，并指定 JSON 角色定义文件。
...
- 若要列出所有自定义角色，请使用 [az role definition list](/cli/role/definition#az-role-definition-list) 命令及 `--custom-role-only` 参数。
...
1. 若要更新自定义角色，请使用 [az role definition update](/cli/role/definition#az-role-definition-update) 命令并指定更新后的 JSON 文件。
...
- 使用 [az role definition delete](/cli/role/definition#az-role-definition-delete) 命令并指定角色名称或角色 ID 来删除自定义角色。
