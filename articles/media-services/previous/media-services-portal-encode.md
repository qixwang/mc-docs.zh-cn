...
使用 Azure 媒体服务时最常见的情况之一是将自适应比特率流传送至客户端。 媒体服务支持以下自适应比特率流式处理技术：Apple HTTP Live Streaming (HLS)、Microsoft 平滑流式处理，以及经 HTTP 的动态自适应流式处理（DASH，也称 MPEG-DASH）。 要准备视频以便进行自适应比特率流式处理，首先要将源视频编码成多比特率文件。 可使用 Media Encoder Standard 对视频进行编码。  
