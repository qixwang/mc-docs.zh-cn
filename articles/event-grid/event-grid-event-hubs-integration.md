...
1. 通过运行以下 CLI 命令创建 Azure 资源组： 
    1. 将以下命令复制并粘贴到 Cloud Shell 窗口中
        ...
        ```azurecli
        az group create -l chinaeast -n <Name for the resource group>
        ```
    1. 为资源组指定名称 
    2. 按 **ENTER**。 
        ...
        ```azurecli
        user@Azure:~$ az group create -l chinaeast -n ehubegridgrp
        {
...
1. 在 Visual Studio 配置好配置文件后，选择“发布”  。
   ...
   ![选择“发布”](media/event-grid-event-hubs-integration/select-publish.png)
