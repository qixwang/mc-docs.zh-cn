...
虚拟机列表包含有用的数据：关联的资源组、以前的[备份预检查](/backup/backup-azure-manage-windows-server#backup-pre-check-status)、上次备份状态，以及最近还原点的日期。 单击最后一列中的省略号会打开用于触发常见任务的菜单。 列中提供的有用数据根据备份类型的不同而异。
