...
ms.topic: conceptual
ms.date: 06/09/2020
ms.openlocfilehash: e20641c5b61d4850108b1fd2840a6c91fb8b2efe
ms.sourcegitcommit: 285649db9b21169f3136729c041e4d04d323229a
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 06/11/2020
ms.locfileid: "84684025"
---
...
> [!NOTE]
>
> 此数据模型引用向 Log Analytics (LA) 发送诊断事件的 Azure 诊断模式。 若要了解新资源特定模式的数据模型，可以参阅以下文章：[Azure 备份诊断事件的数据模型](/backup/backup-azure-reports-data-model)
...
|CountOfAlertsConsolidated_s     |Number         |合并的警报数（如果为合并的警报）          |
|AlertRaisedOn_s     |文本         |引发警报的实体的类型         |
...
| ProtectedContainerUniqueId_s |文本 |与警报关联的受保护服务器的唯一标识符（为 ProtectedServerUniqueId_s in V1）|
...
| ProtectionGroupName_s |文本 | 保护备份项的保护组的名称（如 SC DPM 和 MABS，如果适用）|
...
| 资源 |文本 |这是正在收集其数据的资源，显示恢复服务保管库名称 |
| ProtectedContainerUniqueId_s |文本 |与备份项关联的受保护服务器的唯一标识符（为 ProtectedServerUniqueId_s in V1） |
...
|BackupManagementServerType_s     |文本         |备份管理服务器的类型（如 MABS 和 SC DPM）|
...
| RecoveryJobDestination_s |文本 | 已恢复数据的恢复作业的目标 |
...
| DiffBackupFormat_s |文本 |v2|Azure VM 备份中的 SQL 差异备份的格式 |
| DiffBackupTime_s |时间 |v2|Azure VM 备份中的 SQL 差异备份的时间|
| DiffBackupRetentionDuration_s |小数 |v2|Azure VM 备份中的 SQL 差异备份的保留持续时间|
| LogBackupFrequency_s |小数 |v2|SQL 日志备份的频率|
| LogBackupRetentionDuration_s |小数 |v2|Azure VM 备份中的 SQL 日志备份的保留持续时间|
| DiffBackupDaysofTheWeek_s |文本 |v2|Azure VM 备份中的 SQL 差异备份的星期日期|
| SourceSystem |文本 ||当前数据的源系统 - Azure |
| ResourceId |文本 ||正在收集其数据的资源标识符。 例如“恢复服务保管库资源 ID” |
| SubscriptionId |文本 ||正在收集其数据的资源（例如 恢复服务保管库）的资源组 |
| ResourceGroup |文本 ||正在收集其数据的资源（例如 恢复服务保管库）的资源组 |
...
此表提供有关受保护容器的基本字段。 （为 ProtectedServer in v1）
...
| ProtectedContainerWorkloadType_s |文本 |已备份的受保护容器的类型。 例如，IaaSVMContainer |
| ProtectedContainerLocation_s |文本 |受保护的容器是位于本地还是在 Azure 中 |
...
| 字段 | 数据类型 | 说明 |
| --- | --- | --- |
| CloudStorageInBytes_s |小数 |备份所用的云备份存储量，基于最新值进行计算（此字段仅适用于 v1 架构）|
...
| StorageTotalSizeInGBs_s |文本 |存储实体占用的总存储大小 (GB)|
...
| StorageConsumedInMBs_s |Number|相应存储中的相应备份项占用的存储大小 |
| StorageAllocatedInMBs_s |Number |磁盘类型的相应存储中的相应备份项分配的存储大小|
...
| --- | --- | --- |
| StorageUniqueId_s |文本 |用于标识存储实体的唯一 ID |
| BackupItemType_s |文本 |此卷是其首选存储的工作负载|
...
| 字段 | 数据类型 |适用版本 | 说明 |
| --- | --- | --- | --- |
| BackupItemUniqueId_s |文本 |v2|用于标识使用 DPM 和 MABS 备份的 VM 的备份项的唯一 ID|
| ProtectedContainerUniqueId_s |文本 |v2|用于标识使用 DPM 和 MABS 备份的 VM 以外的所有 VM 的受保护容器的唯一 ID|
| ProtectedInstanceCount_s |文本 |v2|关联的备份项或受保护的容器在该日期-时间的受保护实例计数|
...
| 字段 | 数据类型 | 说明 |
| --- | --- | --- |
| BackupItemUniqueId_s |文本 |用于标识使用 DPM 和 MABS 备份的 VM 的备份项的唯一 ID|
| OldestRecoveryPointTime_s |文本 |备份项的最早恢复点的日期和时间|
| OldestRecoveryPointLocation_s |文本 |备份项的最早恢复点的位置|
| LatestRecoveryPointTime_s |文本 |备份项的最新恢复点的日期和时间|
...
查看数据模型后，可以开始在 Azure Monitor 日志中[创建自定义查询](../azure-monitor/learn/tutorial-logs-dashboards.md)以生成自己的仪表板。
