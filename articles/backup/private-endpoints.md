...
通过 Azure 备份，你可以使用[专用终结点](https://docs.microsoft.com/azure/private-link/private-endpoint-overview)从恢复服务保管库安全地备份和还原数据。 专用终结点使用 VNet 中的一个或多个专用 IP 地址将服务有效接入 VNet 中。
...
- 如果在 2020 年 5 月 1 日之前注册了恢复服务资源提供程序，则需在订阅中重新注册它。 若要重新注册提供程序，请转到 Azure 门户中的订阅，导航到左侧导航栏上的“资源提供程序”，然后选择“Microsoft.RecoveryServices”，并单击“重新注册”。
...
请参阅[此部分](#create-a-recovery-services-vault-using-the-azure-resource-manager-client)，了解如何使用 Azure 资源管理器客户端创建保管库。 这会创建一个已启用托管标识的保管库。 有关恢复服务保管库的详细信息，请参阅[在此](/backup/backup-azure-recovery-services-vault-overview)。
...
- `privatelink.blob.core.chinacloudapi.cn`（用于备份/还原数据）
- `privatelink.queue.core.chinacloudapi.cn`（用于服务通信）

1. 在“所有服务”搜索栏中搜索“专用 DNS 区域”，然后从下拉列表中选择“专用 DNS 区域”  。

    ![选择“专用 DNS 区域”](./media/private-endpoints/private-dns-zone.png)

1. 显示“专用 DNS 区域”窗格后，单击“+添加”按钮以开始创建新区域 。
...
1. 在“创建专用 DNS 区域”窗格中，填写所需的详细信息。 订阅必须与将在其中创建专用终结点的位置相同。
    ...
    | ---------------------------------- | ----------- | ------------------------------------------------------------ |
    | `privatelink.blob.core.chinacloudapi.cn`  | Blob        | **订阅**：与需要创建专用终结点的位置相同  RG：VNET 或专用终结点的 RG |
    | `privatelink.queue.core.chinacloudapi.cn` | 队列       | RG：VNET 或专用终结点的 RG |
...
| **区域**                                                     | **服务** | **订阅和资源组详细信息**                  |
| ------------------------------------------------------------ | ----------- | ------------------------------------------------------------ |
| `privatelink.<geo>.backup.azure.cn`  <br><br>   请注意：此处的 geo 指的是地区代码。 例如，wcus 和 ne 分别表示美国中西部和中国北部 。 | Backup      | **订阅**：与需要创建专用终结点的位置相同  RG：订阅中的任何 RG |
...
<!--Removed in MC: For URL naming conventions in national geos:-->
...
1. 转到在上一步创建的 DNS 区域，并导航到左侧栏的“虚拟网络链接”。 转到那里后，单击“+添加”按钮。
...
1. 在“添加角色分配”中，选择“参与者”作为“角色”，然后使用保管库的“名称”作为“主体”    。 选择保管库，并在完成后单击“保存”。
...
1. 单击左侧导航栏中的“专用终结点”。 转到“专用终结点”窗格后，单击“+添加”以开始为保管库创建专用终结点 。
...
1. 在“创建专用终结点”过程中，需要指定用于创建专用终结点连接的详细信息。
    ...
    1. 输入详细信息后，继续“查看 + 创建”。 完成验证后，单击“创建”以创建专用终结点。
...
1. 转到“专用 DNS 区域”并导航到左侧栏上的“概述”选项 。 转到那里后，单击“+记录集”以开始添加记录。
...
1. 将以下代码粘贴到名为“dnszonerecordcreation.ps1”的本地文件中： 
```powershell
Param(
...
1. 将以下代码粘贴到名为“VaultMsiPrereqScript.ps1”的本地文件中：
...
| **区域**                                                     | **服务** |
| ------------------------------------------------------------ | ----------- |
| `privatelink.<geo>.backup.azure.cn`      | Backup      |
| `privatelink.blob.core.chinacloudapi.cn`                            | Blob        |
| `privatelink.queue.core.chinacloudapi.cn`                           | 队列       |

>[!NOTE]
>在上述文本中，geo 指地区代码。 例如，wcus 和 ne 分别表示美国中西部和中国北部 。
...
备份服务的 DNS 区域 (`privatelink.<geo>.backup.azure.cn`)：
...
Blob 服务的 DNS 区域 (`privatelink.blob.core.chinacloudapi.cn`)：
...
队列服务的 DNS 区域 (`privatelink.queue.core.chinacloudapi.cn`)：
