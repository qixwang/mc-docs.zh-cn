...
ms.author: v-johya
ms.date: 05/11/2020
ms.openlocfilehash: a25af98bc6d5c8410b273ba9373123d2ea9067b0
ms.sourcegitcommit: 08b42258a48d96d754244064d065e4d5703f1cfb
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/18/2020
ms.locfileid: "83445177"
---
...
**替换现有项** | 可以还原某个磁盘，并使用它来替换现有 VM 上的磁盘。<br/><br/> 当前 VM 必须存在。 如果已将其删除，则无法使用此选项。<br/><br/> Azure 备份将在替换磁盘前创建现有 VM 的快照，并将其存储在指定的暂存位置中。 连接到该 VM 的现有磁盘将替换为所选的还原点。<br/><br/> 该快照将复制到保管库，并根据保留策略予以保留。 <br/><br/> 替换磁盘操作完成后，原始磁盘会保留在资源组中。 如果不需要原始磁盘，可以选择手动删除它们。 <br/><br/>对于未加密的托管 VM（包括使用自定义映像创建的 VM），支持替换现有磁盘。 经典 VM 不支持此功能。<br/><br/> 如果还原点中的磁盘数多于或少于当前 VM 中的磁盘数，则还原点中的磁盘数只反映 VM 配置。<br><br> 包含链接资源（如[用户分配的托管标识](/active-directory/managed-identities-azure-resources/overview) 或 [Key Vault](/key-vault/key-vault-overview)）的 VM 不支持替换现有磁盘，因为备份客户端应用在执行还原时对这些资源没有权限。
**跨区域（次要区域）** | 跨区域还原可用于还原次要区域（[Azure 配对区域](/best-practices-availability-paired-regions#what-are-paired-regions)）中的 Azure VM。<br><br> 如果备份在次要区域中完成，则可针对所选恢复点还原所有 Azure VM。<br><br> 此功能适用于以下选项：<br> * [创建 VM](/backup/backup-azure-arm-restore-vms#create-a-vm) <br> * [还原磁盘](/backup/backup-azure-arm-restore-vms#restore-disks) <br><br> 当前不支持[替换现有磁盘](/backup/backup-azure-arm-restore-vms#replace-existing-disks)选项。<br><br> 权限<br> 次要区域中的还原操作可由备份管理员和应用管理员执行。
<!-- Video not suitable for China: https://azure.microsoft.com/resources/videos/create-a-custom-virtual-machine-image-in-azure-resource-manager-with-powershell/ -->
...
- [将已还原的磁盘附加](/virtual-machines/windows/attach-managed-disk-portal)到现有 VM。
- 使用 PowerShell 从已还原的磁盘[创建新的 VM](/backup/backup-azure-vms-automation#create-a-vm-from-restored-disks)。
...
作为[还原选项](/backup/backup-azure-arm-restore-vms#restore-options)之一，跨区域还原 (CRR) 允许你在某个次要区域（Azure 配对区域）中还原 Azure VM。

若要在预览版期间加入此功能，请阅读[“准备工作”部分](/backup/backup-create-rs-vault#set-cross-region-restore)。
...
- 若要还原和创建 VM，请参阅[创建 VM](/backup/backup-azure-arm-restore-vms#create-a-vm)。
- 若要还原为磁盘，请参阅[还原磁盘](/backup/backup-azure-arm-restore-vms#restore-disks)。
...
**还原单个域中的单个域控制器 VM** | 像还原其他任何 VM 一样还原该 VM。 请注意：<br/><br/> 从 Active Directory 的角度来看，Azure VM 与任何其他 VM 类似。<br/><br/> 还可使用目录服务还原模式 (DSRM)，因此所有 Active Directory 恢复方案都是可行的。 [详细了解](/backup/backup-azure-arm-restore-vms#post-restore-steps)虚拟化域控制器的备份和还原注意事项。
