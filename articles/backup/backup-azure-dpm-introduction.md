...
通过 Azure 备份来备份的每个计算机必须安装有备份代理，也称 Azure 恢复服务 (MARS) 代理。 按如下方式在 DPM 服务器上安装代理：
...
保管库凭据文件只能生效 48 小时（从将其从门户下载后算起）。 如果此屏幕中显示任何错误（例如“提供的保管库凭据文件已过期”），请登录到 Azure 门户，并再次下载保管库凭据文件。
...
如果遇到保管库凭据无效错误（例如“所提供的保管库凭据无效”），则表明该文件已损坏，或者没有与恢复服务关联的最新凭据。
