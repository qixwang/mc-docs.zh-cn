...
创建 Azure AD B2C 租户后，需要使用 [Azure 门户](https://portal.azure.cn)注册管理应用程序。 此外，需要向其授予代表自动化脚本或管理应用程序执行管理任务所需的权限。
...
1. 将 `<appSettings>` 节替换为以下 XML。 然后将 `{your-b2c-tenant}` 替换为你的租户名称，并将 `{Application ID}` 和 `{Client secret}` 替换为前面记下的值。
    ...
    ```xml
    <appSettings>
        <add key="b2c:Tenant" value="{your-b2c-tenant}.partner.onmschina.cn" />
...
如果生成成功，可以在 `B2CGraphClient\bin\Debug` 中找到 `B2C.exe` 控制台应用程序。
...
public B2CGraphClient(string clientId, string clientSecret, string tenant)
{
    ...
    this.authContext = new AuthenticationContext("https://login.partner.microsoftonline.cn/" + tenant);
...
public async Task<string> SendGraphGetRequest(string api, string query)
{
    ...
    AuthenticationResult result = authContext.AcquireToken("https://graph.chinacloudapi.cn", credential);
    ...
```
...
若要从 Azure AD Graph API 获取用户列表或获取特定用户，可以向 `/users` 终结点发送 HTTP `GET` 请求。 要求获取租户中所有用户的请求如下所示：

```HTTP
GET https://graph.chinacloudapi.cn/contosob2c.partner.onmschina.cn/users?api-version=1.6
...
* 使用 ADAL 获取的访问令牌将通过 `Bearer` 方案添加到 `Authorization` 标头。
...
public async Task<string> SendGraphGetRequest(string api, string query)
{
    ...
    string url = "https://graph.chinacloudapi.cn/" + tenant + api + "?" + "api-version=1.6";
...
在 B2C 租户中创建用户帐户时，可以向 `/users` 终结点发送 HTTP `POST` 请求。 以下 HTTP `POST` 请求显示了要在租户中创建的一个用户示例。

以下请求中的大多数属性都是创建使用者用户所必需的。 包含 `//` 注释是为了方便演示 -- 请不要将它们包含在实际请求中。

```HTTP
POST https://graph.chinacloudapi.cn/contosob2c.partner.onmschina.cn/users?api-version=1.6
Authorization: Bearer eyJhbGciOiJSUzI1NiIsIng1dCI6IjdkRC1nZWNOZ1gxWmY3R0xrT3ZwT0IyZGNWQSIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJod...
...
有关必填字段和可选字段的详细信息，请参阅[实体和复杂类型参考 | Graph API 参考](https://docs.microsoft.com/previous-versions/azure/ad/graph/api/entity-and-complex-type-reference)。
...
> [!NOTE]
> 如果要从现有用户存储迁移的帐户的密码强度低于 [Azure AD B2C 实施的强密码强度](active-directory-b2c-reference-password-complexity.md)，可以使用 `passwordPolicies` 属性中的 `DisableStrongPassword` 值禁用强密码要求。 例如，可按如下所示修改前面的创建用户请求：`"passwordPolicies": "DisablePasswordExpiration, DisableStrongPassword"`。
...
```HTTP
PATCH https://graph.chinacloudapi.cn/contosob2c.partner.onmschina.cn/users/<user-object-id>?api-version=1.6
Authorization: Bearer eyJhbGciOiJSUzI1NiIsIng1dCI6IjdkRC1nZWNOZ1gxWmY3R0xrT3ZwT0IyZGNWQSIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJod...
Content-Type: application/json
Content-Length: 37
...
```HTTP
DELETE https://graph.chinacloudapi.cn/contosob2c.partner.onmschina.cn/users/<user-object-id>?api-version=1.6
Authorization: Bearer eyJhbGciOiJSUzI1NiIsIng1dCI6IjdkRC1nZWNOZ1gxWmY3R0xrT3ZwT0IyZGNWQSIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJod...
```
...
除了用户管理之外，还可以使用 Azure AD 图形 API 执行许多其他操作。 [Azure AD 图形 API 参考](https://docs.microsoft.com/previous-versions/azure/ad/graph/api/api-catalog)提供了每个操作的详细信息以及示例请求。
