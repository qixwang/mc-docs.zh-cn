...
> [!NOTE]
> 如果要从现有用户存储迁移的帐户的密码强度低于 [Azure AD B2C 实施的强密码强度](user-flow-password-complexity.md)，可以使用 `passwordPolicies` 属性中的 `DisableStrongPassword` 值禁用强密码要求。 例如，可按如下所示修改前面的创建用户请求：`"passwordPolicies": "DisablePasswordExpiration, DisableStrongPassword"`。
