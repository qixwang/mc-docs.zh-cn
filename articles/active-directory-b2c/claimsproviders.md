...
| 元素 | 出现次数 | 说明 |
| ------- | ---------- | ----------- |
| 域 | 0:1 | 一个字符串，包含声明提供程序的域名。 此域名用于声明提供程序中定义的所有技术配置文件，除非被技术配置文件覆盖。 域名也可以在 **domain_hint** 中引用。 有关详细信息，请参阅**使用 Azure Active Directory B2C 设置直接登录**的[将登录重定向到社交提供者](direct-signin.md)部分。 |
