...
- “名称”  和“用户名”  - Name  属性包含用户的名和姓。 用户名  是用户登录时输入的标识符。 用户名包括完整域。 用户名的域名部分必须是初始默认域名 *your-domain.partner.onmschina.cn* 或已验证的非联合[自定义域名](../active-directory/fundamentals/add-custom-domain.md)（例如 *contoso.com*）。
