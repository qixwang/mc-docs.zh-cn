...
> [!NOTE]
> 这是幂等操作。 如果失败，请重试操作。 如果仍然失败，请创建支持票证或在 [Azure 支持](https://support.azure.cn/support/contact/)论坛上发帖。
>
>
...
| 经典表示形式 | Resource Manager 表示形式 | 注释 |
| --- | --- | --- |
...
| 可用性集属性 |可用性集资源 | 可用性集规范是经典部署模型中 VM 上的属性。 在迁移过程中，可用性集成为顶级资源。 以下配置不受支持：每个云服务包含多个可用性集，或者在一个云服务中有一个或多个可用性集以及不在任何可用性集中的 VM。 |
...
| 负载均衡的终结点集 |负载均衡器 |在经典部署模型中，平台已经为每个云服务分配一个隐式负载均衡器。 在迁移期间，创建新的负载均衡器资源，负载均衡终结点集成为负载均衡器规则。 |
