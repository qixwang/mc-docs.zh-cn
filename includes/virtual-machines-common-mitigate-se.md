...
> 
> 在 2019 年 5 月 14 日，[Intel 披露了](https://www.intel.com/content/www/us/en/security-center/advisory/intel-sa-00233.html)一组新的推理执行旁道漏洞，称为“微体系结构数据采样”（即 MDS，请参阅 Microsoft 安全指南 [ADV190013](https://portal.msrc.microsoft.com/zh-cn/security-guidance/advisory/ADV190013)），它已被分配多个 CVE： 
...
>
> 此漏洞影响 Intel® Core® 处理器和 Intel® Xeon® 处理器。  Azure 已发布了操作系统更新，并且正在将新的微代码（当它由 Intel 提供时）部署到我们的全套产品中，从而保护我们的客户免受这些新漏洞影响。   Azure 正在与 Intel 密切合作，以便在平台上正式发布新的微代码之前对其进行测试和验证。 
>
...
| 产品/服务 | 建议的操作  |
|----------|---------------------|
| Azure 云服务  | 启用[自动更新](/cloud-services/cloud-services-how-to-configure-portal)，或确保运行最新的来宾 OS。 |
...
**步骤 1：在 VM 上禁用超线程功能** - 在超线程 VM 上运行不受信任的代码的客户将需要禁用超线程功能或转换到非超线程的 VM 大小。 有关超线程 VM 大小（其中 vCPU 与核心的比率为 2:1）的列表，请参阅[此文档](/virtual-machines/windows/acu)。 若要从 VM 中使用 Windows 命令行检查 VM 是否启用了超线程功能，请参考下面的脚本。
...
**步骤 4**：对于正在使用[嵌套虚拟化](/virtual-machines/windows/nested-virtualization)的部署（仅限 D3 和 E3）：这些说明适用于用作 HYPER-V 主机的 VM。
...
**步骤 1：在 VM 上禁用超线程功能** - 在超线程 VM 上运行不受信任的代码的客户将需要禁用超线程功能或转换到非超线程VM。  有关超线程 VM 大小（其中 vCPU 与核心的比率为 2:1）的列表，请参阅[此文档](/virtual-machines/linux/acu)。 若要检查是否正在运行超线程 VM，请在 Linux VM 中运行 `lscpu` 命令。 
...
<!--Update_Description: wording update to follow global-->