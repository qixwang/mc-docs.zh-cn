...
| 大小             | vCPU  | 内存:GiB | 临时存储 (SSD) GiB | VM 的基本 CPU 性能 | VM 的最大 CPU 性能 | 初始信用额度 | 赚取的额度/小时 | 赚取的最大额度 | 最大数据磁盘数 | 最大缓存吞吐量和临时存储吞吐量：IOPS/Mbps | 最大非缓存磁盘吞吐量：IOPS/Mbps | 最大 NIC 数 |          
|---------------|-------------|----------------|----------------------------|-----------------------|--------------------|--------------------|--------------------|----------------|----------------------------------------|-------------------------------------------|-------------------------------------------|----------|
...
<!--MOONCAKE:  D16s_v3  CNY10.08 per hour (monthly CNY7499.52)-->
<!--MOONCAKE:  B16ms  CNY6.82 per hour (monthly CNY5071.10)-->
<!--MOONCAKE:  Save is 32% -->

对于 D16s_v3（有 16 个 vCPU 和 64 GiB 的内存）来说，小时费率为 10.08 元/小时（每月 7499.52 元）；对于 B16ms（有 16 个 vCPU 和 64 GiB 内存）来说，费率为 6.82 元/小时（每月 5071.10 元）。 <b>这样会节省 32%！</b>
...
**答**：我们将在未来几周引入 2 个新指标，**Credit** 指标将允许你查看 VM 已赚取多少额度，而 **ConsumedCredit** 指标将显示 VM 已消耗了多少累积的 CPU 额度。    你将能够从门户的指标窗格中查看这些指标，也可以编程方式通过 Azure Monitor API 查看这些指标。
...
### <a name="q-what-happens-if-i-run-out-of-credits"></a>问：如果用完了额度，会发生什么情况？
**答**：如果 VM 使用了所有可用额度，则只能使用基线 CPU，而不能突发超过该基线。
