...
2. 创建可用于[基于令牌的身份验证](https://docs.azure.cn/notification-hubs/notification-hubs-push-notification-http2-token-authentification)的 .p8  （较新的方法  ）。

如[基于令牌 (HTTP/2) 的 APNS 身份验证](https://docs.azure.cn/notification-hubs/notification-hubs-push-notification-http2-token-authentification)中所述，较新的方法（与使用证书相比）有很多好处。 不过，下面为这两种方法都提供了步骤。 
